...
- 生产 API 目录从现有 route source inventory 与授权矩阵派生；当前 21 份行为合同覆盖 19 个完整 endpoint 与 2 个 action-slice。新增八份 T09 合同覆盖药械/科研首发写入口并保存首次公共响应快照用于精确回放；T07 标准药械状态机保持独立。有界 receipt、进程锁与 SQLite CAS 不宣称跨实例 exactly-once。所有条目默认 `NO-GO`，静态幂等标记和仓库门禁不构成生产证据。
