...
- 在线 Playwright 继续在 29 + 13 项 context 中阻止 Service Worker；PWA 只在独立 3 项 context 中允许，
  使用同一 Chromium/动态端口策略并逐项注销 registration、删除 Cache Storage。v61 只缓存同源成功响应，
  仓库测试不替代真实 HTTPS、设备策略或现场安装验收。
- 急救生命链与医生工作台两个单页 controller 各自将 6 个 API 驱动 HTML sink 迁为显式
  DOM/text/class/dataset，Inventory v2 的 HTML occurrence 由 871 降至 859；两页恶意响应浏览器测试
  不替代真实托管头、渗透或现场验收。
