...
- 生产 API 目录从现有 route source inventory 与授权矩阵派生；当前 30 份行为合同覆盖 28 个完整 endpoint 与 2 个 action-slice。T09、T04/T05、T02/T06 共关闭冻结首发范围 17 个写入口的仓库行为复核，并保存首次公共响应或精确结果快照用于重放；T07 标准药械状态机保持独立。有界 receipt、进程锁与 SQLite CAS 不宣称跨实例 exactly-once。所有条目默认 `NO-GO`，静态幂等标记和仓库门禁不构成生产证据。
