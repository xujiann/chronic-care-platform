...
Accepted [AI/CDSS 边界 ADR](docs/adr/2026-09-05-ai-cdss-governance.md) 已落实规则治理、临床辅助端口、可信目录、人工复核及安全工作台。原始 state 入口收口，临床事实不复制；能力验证与生产授权继续分开。
...
- T07 医疗付费一件事复用既有金融事件、在线退款与日终对账事实源，通过机构范围只读投影和独立页面提供统一处理入口；不建立第二订单/支付账本，不改变按病种付费或外部 provider 协议，生产始终失败关闭。

- `main` 是唯一集成和发布主干。
- 开发组织采用 T01–T09 九个一级开发域，T00 仅作为集成治理单元；急救、血液、影像、体检、质量安全作为 T06 下五个嵌套开发子域。各单元可独立计划、工作树和测试，但继续使用单仓、共享运行时和模块化单体部署。
- T07 `insurance-payment` 已在同一组织合同中确立为独立开发产品线，拥有独立产品 Roadmap、Backlog、process 工作树和领域测试；Owner 仍来自 T07，发布仍经 T00/main，独立仓库、数据库、运行时、服务和生产部署未获授权。
...
- 生产 API 目录从现有 route source inventory 与授权矩阵派生；当前 36 份行为合同覆盖 34 个完整 endpoint 与 2 个 action-slice。T03 卫生监督四个写入口已形成完整 endpoint 证据，既有首发范围 17 个写入口的仓库行为复核继续闭合；合同保存首次公共响应或精确结果快照用于重放，T07 标准药械状态机保持独立。有界 receipt、进程锁与 SQLite CAS 不宣称跨实例 exactly-once。所有条目默认 `NO-GO`，静态幂等标记和仓库门禁不构成生产证据。
