...
地区配置准入和单地区投产档案已接入统一发布门禁。使用 `regional:configuration` 检查地区配置契约、能力映射和扩展覆盖，使用 `regional:dossier` 聚合组合发布、治理、运维、备份、证书与存储门禁；管理端可读取 `GET /api/regional/deployments/:regionCode/dossier` 的最小化结果。完整说明见 [地区配置准入与投产档案操作指南](docs/地区配置准入与投产档案操作指南-2026-08-07.md)。
