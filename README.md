...
The enforced rotation sequence is new active key, old grace key, cross-key audit smoke, cross-key callback smoke, then old-key expiry or revocation. An emergency revocation creates a P0 security-quarantine disposition; automatic resign and automatic recovery remain disabled. Run `npm.cmd run public-health:resilience-test` for the focused contract/resilience/modernization/UI regression and `npm.cmd run public-health:final-readiness` for the aggregate 80-check report: 59 T08 functional checks plus 21 T00 public integration and safety-boundary checks. The compatibility field `remainingT00Integration` now aliases `remainingProductionBoundaries`; completed public wiring is not reported as unfinished work. `endpointConnectivityReady` and `continuousConnectivityReady` are independent of production authorization, and all contract, cutover and endpoint views keep `productionReady=false`. Production remains blocked until real identity and institution directories, HIS/EMR/LIS/PACS, bed/roster/equipment/transfer sources, scanner and insurance callbacks, a production database, SIEM, alert duty coverage, P0/P1 closure, trusted site evidence, production handoff, launch approval and disaster-recovery evidence are connected and accepted.

T00 also registers the fifteenth-plan public-health modernization boundary. Commission users read minimized data-foundation, surveillance-model, surveillance and medical-prevention boards through `GET /api/public-health/data-foundation`, `GET /api/public-health/surveillance-model-governance`, `GET /api/public-health/surveillance-center` and `GET /api/public-health/medical-prevention-tasks`; governed writes use `POST /api/public-health/surveillance-signals`, shadow-run and model-validation routes, plus the signal, alert, validation and task `/:id/actions` routes. The public server supplies actor identity, organization scope, receive/action time and header-based idempotency, requires `expectedVersion`, delegates every transition to the T08 services and persists only their `nextData`. Thirteen collections share a SQLite transaction boundary with entity CAS and unique source-record/idempotency hashes; `/api/state` never exposes those raw collections. The public-health page provides data-foundation, source-operations, rule-governance, surveillance-model, monitoring-warning and medical-prevention workbenches while preserving human review. Model output is advisory-only, cannot mutate signal or alert state, requires independent performance review, expires into `review-due` after 90 days and never grants production authorization. Run `npm.cmd run public-health:modernization-readiness`; real source interfaces, sharing authorization, official receipts, staffed review, verified handoffs, approved datasets, sustained drift monitoring, site evidence and launch approval remain production blockers.
