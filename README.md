...
`digital-hospital-standard-platform/` is the public GitHub Pages entry for the v0.17 new-standard platform. It adds pilot operations, pilot collaboration, hospital onboarding, integration certification and execution control, pilot assessment, the evaluation assistant, rollout readiness, operational monitoring, 57 trial indicators, tiered review, scoring and rectification, and downloadable interface packages. The execution workbench now includes a task runtime center with capability-based Worker pools, one-time leases, heartbeat renewal, expired-lease recovery, exponential retry, dead-letter review and approved redrive. It continues to link environment verification, vault-reference-only credential governance, signed callback receipts, nonce replay isolation, production cutover gates and audited rollback records. Reusable domain logic in `digital-hospital-integration-execution.js` keeps raw credentials, lease tokens, signatures, nonces, payloads and idempotency keys out of persisted state. The focused design and acceptance boundary are documented in `docs/数智医院标准平台v0.17任务运行中心.md`.
