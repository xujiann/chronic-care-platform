...
- [架构与治理导航](./ARCHITECTURE.md)
- [全仓库工程体检](./ENGINEERING_HEALTH_CHECK.md)
- [工程治理体系](./ENGINEERING_GOVERNANCE.md)
- [工程治理路线图](./ROADMAP.md)
- [ADR 台账](./docs/adr/README.md)
...
3. 数据模型深化：已完成 SQLite schema v14 独立迁移注册表、历史指纹冻结、集合版本、乐观锁、持久化认证会话、恢复演练、P2 结构化镜像表、PostgreSQL 迁移包、事务 outbox、基线入队、只读影子核对及差异处置闭环；后续 migration 按内容 checksum 追加，并继续补生产主存储读路径、领域表转换、容量切换和原生备份证据。
...
The clinical-blood shared-runtime gate and the emergency-life-chain and imaging-cloud specialty gates are registered through `t10:clinical-blood:readiness`, `t10:clinical-blood:smoke`, `t10:emergency-module:smoke`, `imaging-cloud:readiness` and `imaging-cloud:test`. Clinical blood can be selected and rehearsed independently, but it is not an independently deployable service: `deploymentMode=shared-platform-node-runtime` and `independentDeploymentAuthorized=false`. Commission-only readiness summaries are exposed at `/api/t10-specialty/modules/clinical-blood/readiness` and `/api/t10-specialty/modules/emergency-life-chain/readiness`; the imaging workbench uses the nine role-guarded `/api/imaging-cloud/production/*` contracts. Module evidence can become complete without opening the platform: every public response keeps `productionReady=false` until trusted site identity/evidence, the T00 launch approval, duty/SIEM, disaster-recovery and rollback evidence are independently accepted.
