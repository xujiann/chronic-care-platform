...
`GET /api/digital-hospital/public-health/coordination` exposes the v0.20 public-health operations board and accepts `hospitalCode`, `laneId`, `level`, `status` and `overdueOnly` filters. `POST /api/digital-hospital/public-health/incidents` registers a minimized incident. `POST /api/digital-hospital/public-health/incidents/:id/actions` advances the optimistic-revision workflow through handling, review submission and independent closure, or records a separate `escalate-overdue` action without changing the lifecycle status. `GET /api/digital-hospital/public-health/incidents/export?format=json|csv` exports the filtered ledger and safe professional-system summaries. The persistence and response boundaries reject or omit sensitive security material and keep `productionReady=false` until field evidence and approvals are complete.
