...
`digital-hospital-standard-platform/` is the public GitHub Pages entry for the v0.16 new-standard prototype. It adds pilot operations, pilot collaboration, hospital onboarding, integration certification and execution control, pilot assessment, the evaluation assistant, rollout readiness, operational monitoring, 57 trial indicators, tiered review, scoring and rectification, and downloadable interface packages. The execution workbench now links environment verification, vault-reference-only credential governance, idempotent asynchronous jobs, signed callback receipts, nonce replay isolation, production cutover gates and audited rollback records. Reusable domain logic in `digital-hospital-integration-execution.js` keeps raw credentials, signatures, nonces and idempotency keys out of persisted state.
