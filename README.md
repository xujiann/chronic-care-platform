...
`GET /api/digital-hospital/public-health/coordination` exposes the v0.19 public-health coordination board. `POST /api/digital-hospital/public-health/incidents` registers a minimized incident, and `POST /api/digital-hospital/public-health/incidents/:id/actions` advances the optimistic-revision workflow through handling, review submission and independent closure. The persistence boundary rejects sensitive security material and keeps `productionReady=false` until field evidence and approvals are complete.
