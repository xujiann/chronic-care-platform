...
After `effectiveAt`, new enqueue resolves `currentContract` from the server snapshot and emits a signed v2 request with the matching request/receipt Schema. Compatibility-period v1 work can still be claimed, while sunset blocks old claims. Existing dead-letter recovery creates exactly one active-contract successor and seals the v1 predecessor, v2 successor, two audit relationships and coordination retry in the same durable state update. The operations board embeds `contractCutover`, reporting P1 compatibility backlog, P0 post-sunset backlog and P0 stale/missing successors while excluding delivered history and valid recovered predecessors.

The enforced rotation sequence is new active key, old grace key, cross-key audit smoke, cross-key callback smoke, then old-key expiry or revocation. An emergency revocation creates a P0 security-quarantine disposition; automatic resign and automatic recovery remain disabled. Run `npm.cmd run public-health:resilience-test` for the focused contract/resilience regression and `npm.cmd run public-health:final-readiness` for the aggregate 45-check report. All contract and cutover views keep `productionReady=false`. Production remains blocked until real identity and institution directories, HIS/EMR/LIS/PACS, bed/roster/equipment/transfer sources, scanner and insurance callbacks, a production database, SIEM, alert duty coverage and disaster-recovery evidence are connected and accepted.
