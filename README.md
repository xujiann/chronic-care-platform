...
## National 1+N+M Access Control Plane

- `national-access-service.js` provides the national hub, provincial/regional/sector node registry, institution registry, tenant boundary, six on-demand service packages, subscription lifecycle, resident authorization, existing-system adapter governance, contract conformance tests, callback receipts, certificate and developer-key governance, cross-province routing envelopes, automatic failover, SLA evaluation, standards conflict checks and access audit.
- `national-access.html` is the national/institution self-service portal for node registration, institution onboarding, package subscription, resident authorization, HIS/EMR/LIS/PACS adapter registration, contract testing, callback receipts, certificates, one-time developer keys, SDK contracts, LIS/PACS routing envelopes, SLA alerts, local standards extensions and cross-province route validation.
- `GET /api/national-access` returns the national access center. Commission users see the national view; institution users receive only their own organization, node and subscription scope.
- `GET /api/national-access/service-packages` publishes the selectable national service catalog.
- Node, institution and subscription write APIs use `/api/national-access/nodes`, `/api/national-access/institutions` and `/api/national-access/subscriptions`, with lifecycle actions under `/:id/actions`.
- `POST /api/national-access/node-health` records node health probes; `POST /api/national-access/routes/plan` validates both institutions, active package subscriptions, node certification and health before returning the source-node → national-hub → target-node route.
- `/api/national-access/certificates` and `/api/national-access/credentials` manage sandbox/pilot certificates and one-time developer secrets; production credentials remain hard-blocked until external acceptance evidence exists.
- `POST /api/national-access/sandbox/invoke` authenticates the one-time developer secret, validates package and contract scope, enforces minute/day quotas, and returns idempotent metadata-only acknowledgements without accepting clinical payloads.
- `POST /api/national-access/security/evaluate` expires invalid credentials and raises lifecycle alerts; `POST /api/national-access/certification-reports` scores institution, node, package, credential and security prerequisites while keeping production readiness false.
- `POST /api/national-access/consents` registers an authorization scope using tokenized resident references and an evidence digest; `/api/national-access/consents/:id/actions` revokes it without copying the authorization document into the control plane.
- `POST /api/national-access/routes/envelopes` validates that the authorization is active and matches the exact source, target, service package, exchange contract, purpose and validity period while keeping source clinical payloads outside the national control plane.
- `GET /api/national-access/sdk/manifest` publishes institution-scoped contracts, schemas, quota policies and the production block. `national-access-developer-sdk.js` is a browser/Node.js metadata-only client that drops unknown clinical fields.
- `/api/national-access/adapters` registers secret-free configuration references for existing HIS, EMR, LIS, PACS/RIS, EHR, public-health, insurance and pharmacy systems. Contract tests run under `/adapters/:id/contract-tests`; institutions may execute tests, while only the national administrator may verify an adapter after every declared contract has passed.
- `/api/national-access/callbacks` subscribes institutions to routing-envelope, consent-revocation and contract-test events. The control plane creates metadata-only delivery tasks, and `/callback-deliveries/:id/actions` records digest-backed receiver acknowledgements without making production network calls.
- `POST /api/national-access/operations/evaluate` generates SLA alerts and failover evidence; `/api/national-access/standards/extensions` enforces national-version and node-namespace compatibility before approval.
- Business data remains in source institutions or compliant nodes. The national layer manages metadata, routing, authorization and audit instead of requiring full raw-data centralization.
- Run `npm run national-access:test` for the domain and API regression suite.
- See `docs/国家全民健康数智化平台接入控制面实施状态.md` for the Chinese implementation status, interface matrix and next-stage work.
