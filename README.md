...
| P0 数据与恢复 | SQLite/JSON 双路径、schema v1-v8 幂等迁移、集合版本、乐观锁、409 冲突契约、PostgreSQL 事务 outbox 与影子同步、备份、恢复演练、脱敏快照、恢复指标验收 |
...
3. 数据模型深化：已完成 SQLite schema v8、集合版本、乐观锁、恢复演练、P2 结构化镜像表、PostgreSQL 迁移包和事务 outbox 影子同步；后续继续补生产主存储读路径、领域表转换、容量切换和原生备份证据。
