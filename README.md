...
The enforced rotation sequence is new active key, old grace key, cross-key audit smoke, cross-key callback smoke, then old-key expiry or revocation. An emergency revocation creates a P0 security-quarantine disposition; automatic resign and automatic recovery remain disabled. Run `npm.cmd run public-health:resilience-test` for the focused resilience regression and `npm.cmd run public-health:final-readiness` for the aggregate 40-check report. `productionReady` remains `false` until real managed keys, approved per-lane policies, HTTPS endpoints, service identity, load evidence, trusted site evidence, external joint tests, monitoring duty, SIEM, production database and disaster-recovery evidence are accepted.
