...
8. 机器 API 授权矩阵扫描全部模块化路由源码并加入一条显式外部回调认证合同，共 589 条声明；`production-api-catalog-v2` 与 372 个字面条件路由取并集，仍形成 594 个唯一接口条目（587 个字面路由、7 个运行时策略）。334 个写接口中仅 `POST /api/auth/sms-delivery-callback` 有机器校验的行为合同与既有可执行测试证据，333 个仍为 `behavior-proof-required`；源码标记只保留观察值，不能再解除复核阻断。13 个自定义鉴权仍未分类，339 项保持 `review-required`，594 项全部 `NO-GO`。
