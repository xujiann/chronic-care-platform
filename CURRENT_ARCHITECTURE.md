...
T00 新增独立 SQLite v1 checkpoint 技术账本，显式初始化绑定源/目标身份且目标批次账本须为空；推进前只读复核源批次及已绑定目标凭证，目标提交后进度未写可重放。不接自动 relay、HTTP、业务请求或生产；本地组件不是外部信任锚，生产 NO-GO。

PR #316 已在 `main@cf88124f` 集成，合并 tree 与冻结 `de23cf51` 一致；PR/main CI 九项及 main Pages 成功。GOV-020/OPS-046 仅关闭代码交付，OPS-046 能力保留真实 PG 新路径及运行观测缺口，生产 NO-GO。
...
T00 新增仅测试的真实 PostgreSQL primary 验证切片（OPS-044）；复用正式驱动、原始 DDL 和 SQLite 提交凭证。主服务、生产 schema、relay/checkpoint 均未改变。
...
- `clinical-ai-cdss.html` 进入分级左侧导航并只用 DOM/text 渲染。`J-CLIN-CDSS` 形成临床侧仓库证据，并由 T01 平台中心汇总最小治理元数据；自动诊断、医嘱、处方和生产激活均禁用。当前静态发布为 57 个入口、189 个显式资产，生产 API 为 637 项且写入口为 364 个。
...
- T07 与 T08 新页面均纳入分级左侧导航，并保持两个页面自身 Inventory v2 P0/P1 finding 为 0。能力注册表与招标追踪已覆盖统一订单、支付、退费和标准文档集成，但真实 provider、正式回执、生产数据库和现场验收仍使 `productionReady=false`。
...
- 当前对账值为 SQLite head v19/41 张非内部表、生产 API 637 项/364 个写入口/324 个行为证明缺口/326 个总复核项、首批范围 `FROZEN-NO-GO` 且范围内 API/集合复核与仓库迁移计划缺口均为 0、Markdown 292 份（222 current、68 snapshot、2 superseded）。
...
6. TEST-001 已建立统一的 `build`、`lint`、`typecheck`、`test:unit`、`test:integration`、`test:smoke` 入口；build 复用静态发布 allowlist 并默认输出到仓库外，unit/integration 完整分区根测试，smoke 独立启动临时 JSON 运行时。治理 CI 执行 `data:collection-governance:verify`，以源码、owner 和隔离清单漂移失败关闭；原 `server.js` c8 门禁保持 85/85/55，内部边界现以 10 个职责独立组锁定真实覆盖基线和直接负向矩阵：原 identity、audit、object storage、API governance 四组，加上 worker observability、区域共享命令、转诊 owner command、科研合规导出、浏览器响应头策略和 Safe URL 端口六组。所有报告只存在临时目录。TEST-006 已恢复全文件 `no-unreachable`；`internet-nursing.js` 与 `quality-safety.js` 的 16 个重复翻译键已按显式 shadow map 去重，保留原首次插入顺序和最终生效值，lint 不再有文件级规则例外。typecheck 去重后由 9 个唯一文件扩大到 13 个治理/安全边界文件。集成套件成员、顺序、断言和超时不变，但本机三次采样约 294–371 秒的 API 热点现在独立进程执行，并向 CI 日志输出无阈值的批次/套件耗时。
   ...
   在线 72 项并集/唯一性、浏览器策略和 runner 生命周期由专项测试锁定。PWA 专项另以 3 项独占套件允许
   Service Worker，验证居民登录后的 v63 安装、v62 及更早缓存清理、同源成功响应缓存边界、离线导航回退及最终
   ...
   当前根 60 + 居民 13 + PWA 3 = 76 项；新增居民服务评价投诉用例覆盖失败保留草稿、成功提交及防重复入口，入口用例直接覆盖 Pages 构建产物内 8 个入口的可访问性、授权与无脚本错误，其中 7 个管理页面同时锁定分级左侧导航；测试保留 `/chronic-care-platform/` 子路径并失败关闭未知入口和敏感源数据，标准在线 context 仍保持阻止 Service Worker。
...
`repository-governance-v1` 从 Git 路径派生；当前闭集为 292 份 Markdown：222 份 `current`、68 份
...
# 2026-09-23 增量：隔离显式单批次 relay

T00 新增仅由显式 rehearsal 调用的单批次协调器，串接已绑定源凭证、目标重放合同与独立 checkpoint；未挂载服务或 worker，生产 NO-GO。见 `docs/adr/2026-09-23-primary-single-batch-relay.md`。
