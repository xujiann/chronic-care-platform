...
8. 机器 API 授权矩阵现从路由扫描和小型认证证据合同派生 601 条声明；`production-api-catalog-v3` 与 371 个字面条件路由取并集，形成 593 个唯一接口条目（585 个字面路由、8 个运行时策略）。认证证据共 13 项且无未分类。幂等证据注册表现有 12 份直接行为合同：10 个完整 endpoint 与 2 个转诊 action-slice。T07 `POST /api/financial-gateways/dispatch` 继续以 canonical request digest、耐久 reservation、显式状态转换、SQLite CAS 和最终 event+audit 原子写入闭合。T07 `POST /api/disease-payment/formal-grouping/jobs` 现增加市级平台/医保资源范围、按幂等键单进程命令锁、锁内重读、稳定脱敏错误以及 queued job 与链式安全审计的单次状态提交；精确重放返回 200 且不二次写入。该锁和 SQLite collection-version CAS 均不被解释为跨实例 exactly-once，正式分组的真实适配器、生产 PostgreSQL 与现场证据仍外置。333 个写接口中 10 个 endpoint 为 `behavior-verified`，323 个仍为 `behavior-proof-required`；退款 endpoint 的 runtime-role variant 与两个通用 action endpoint 仍使当前 325 项需复核，593 项全部 `NO-GO`。T07 `reviewedProofRequired` 已归零。
