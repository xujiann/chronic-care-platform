...
| API authorization/catalog governance | T00 | B | v2 复用 route inventory、授权矩阵和小型 evidence registry 派生 594 项 endpoint 目录；SMS callback 为首个行为证明 pilot | 各 owner 逐项消除 runtime-policy、13 个自定义鉴权和 333 个行为证明缺口，不手工维护平行清单、不猜 owner |
