...
function renderIndustryGovernanceIndicatorCenter(center) {
  renderIndicatorCenter(center);
  const category = document.querySelector("#industry-indicator-category");
  const period = document.querySelector("#industry-indicator-period");
  const exportButton = document.querySelector("#industry-indicator-export");
  if (category) {
    const categories = center.categories || Array.from(new Set((center.indicators || []).map((item) => item.category).filter(Boolean)));
    category.innerHTML = [`<option value="all">全部分类</option>`, ...categories.map((item) => `<option value="${item.id || item}">${item.name || item}</option>`)].join("");
  }
  if (period && center.periodViews?.length) {
    period.innerHTML = center.periodViews.map((item) => `<option value="${item.id}">${item.label || item.id} ${item.period || ""}</option>`).join("");
  }
...
function renderBloodCoordination(coordination = {}) {
  ...
  lead.textContent = `区域血液事件 ${summary.events || 0}，治理指标投影 ${projections.length}，死信 ${summary.deadLetters || 0}`;
  target.append(lead);
  projections.slice(0, 6).forEach((item) => {
    const row = document.createElement("div");
    row.className = "rule-item";
    row.textContent = `${item.eventType || "blood.event"} / ${item.severity || "info"} / ${item.workflow || "aggregate-alert-review"}`;
    target.append(row);
  });
}

async function loadIndustryGovernanceIndicators() {
  if (!API_BASE) return buildStaticIndustryGovernanceIndicatorCenter({});
  const request = window.HealthCityAuth?.authFetch || fetch;
  const response = await request(`${API_BASE}/health-dashboard/industry-governance-indicators`);
...
function renderTemplates(applications) {
  const templateList = document.querySelector("[data-dashboard-template-list]");
  if (!templateList) return;
  templateList.innerHTML = applications.map((item) => `<div><strong>${item.conversationTitle || item.name}</strong><span>${item.functionalBoundary || item.boundary || "Detailed workflow remains in the source application."}</span><small>${(item.apiRoutes || []).join(" | ")} | ${(item.acceptanceEvidence || []).join(" | ")}</small></div>`).join("");
}
