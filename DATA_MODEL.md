...
Playwright E2E 只在操作系统临时目录复制开发种子，根 37 项和居民 13 项由不同服务进程与不同
...
当前 10 个组的新增范围只覆盖既有 Worker 脱敏投影、区域共享/转诊/科研命令逻辑和两个浏览器安全策略端口；它们不创建或修改任何业务数据。Safe URL 的 Node 直接测试可可信执行同一 UMD/CommonJS 端口，但其覆盖率不代表 44 个浏览器页面、836 个 HTML sink、45 个动态样式 sink 或真实 OHIF Origin 已被覆盖。急救生命链、医生工作台、血液上线看板、陪诊工作台、产品运行驾驶舱、产品区域运行驾驶舱、质量安全工作台、区域切换工作台与血液召回面板恶意响应 E2E 只验证临时测试数据的 DOM 表达，不创建业务事实或生产证据；血液接口/设备、要求、演练、迁移、审批、回滚与召回数据以及医生档案、申请、消息、临床辅助、公开备案、陪诊订单、服务主体、人员、风险、政策、产品运行、区域运行、质量安全与区域切换元数据仍使用原 API/集合事实源，本切片没有修改 schema、migration 或持久状态。
