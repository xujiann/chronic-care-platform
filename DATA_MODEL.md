...
Playwright E2E 只在操作系统临时目录复制开发种子，根 28 项和居民 13 项由不同服务进程与不同
...
当前 10 个组的新增范围只覆盖既有 Worker 脱敏投影、区域共享/转诊/科研命令逻辑和两个浏览器安全策略端口；它们不创建或修改任何业务数据。Safe URL 的 Node 直接测试可可信执行同一 UMD/CommonJS 端口，但其覆盖率不代表 44 个浏览器页面、865 个 HTML sink、45 个动态样式 sink 或真实 OHIF Origin 已被覆盖。急救生命链恶意响应 E2E 只验证临时测试数据的 DOM 表达，不创建业务事实或生产证据。
