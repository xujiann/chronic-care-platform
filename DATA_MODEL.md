...
API-002 的 existing-proof 扩展现使注册表达到 17 份合同。T04 `chronicManagementPlans` 更新与 `personalRecords` 随访反馈、T05 `referralTeleconsultations` 创建/动作均把只含 key hash、请求摘要、兼容响应快照和时间的有界 receipt 内嵌在既有 owner aggregate/record；普通端点投影删除内部 receipt。四条命令使用锁内最新 collection version，并把业务、既有 message/data-access/security audit 一次提交；SQLite 强制失败测试证明业务、receipt 与审计共同回滚且无 fallback 写。没有新增 collection、事实源、DDL 或 migration。T07 financial dispatch/formal grouping 继续保持既有 reservation/job、审计和 CAS 边界。T03 highlight signal intake 仍只使用既有 `publicHealthSignals` 与 `securityEvents`：新记录保存组织命名空间后的 `commandKeyHash` 和 canonical `requestDigest`，绝不保存原始幂等键；这两个字段是服务端持久化的幂等元数据，所有出站 highlight 响应均通过公开投影删除。signal 与链式审计在同一全状态快照中由一次 `writeDatabase` 提交，精确重放不写，ledger 保持既有 200 条上限。SQLite 继续消费 `storageMeta.collectionVersions` 做事务内乐观锁，JSON 兼容路径只受当前进程的按 key 命令尾保护。读取投影没有新增集合、表、字段、DDL、migration、outbox 或事实源，也不宣称跨实例 exactly-once；`publicHealthSignals` 仍为 review-required 数据 owner 状态，生产 PostgreSQL 状态未改变。
