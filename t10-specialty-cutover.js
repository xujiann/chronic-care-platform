...
function renderCutoverCommandCenter(commandCenter) {
  ...
  document.querySelector("#cutover-command-center").innerHTML = `
    <div class="cutover-kpis">
      ${kpi("指挥台状态", commandCenter.status || "command-center-ready-for-rehearsal", "warn")}
      ${kpi("值守窗口", summary.windows || windows.length, "ok")}
      ${kpi("责任席位", summary.rosterSeats || roster.length, "ok")}
      ${kpi("No-Go规则", summary.noGoRules || 0, "warn")}
    </div>
    <div class="cutover-card">
      <div class="badge-row">
        <span class="badge warn">${escapeHtml(commandCenter.primaryTrackName || "首个灰度增量")}</span>
        <span class="badge">旁路观察 ${(commandCenter.watchOnlyTrackIds || []).map(escapeHtml).join(" / ") || "无"}</span>
      </div>
      <p class="muted">切换指挥台把范围冻结、batch-1演练、T+1观察和旁路扩展收束到一张可追责的值守板。</p>
    </div>
    <div class="track-grid">
      ${windows.map((windowItem) => `
        <article class="cutover-card">
          ...
          <p><strong>进入：</strong>${escapeHtml(windowItem.entryGate)}</p>
          <p><strong>退出：</strong>${escapeHtml(windowItem.exitGate)}</p>
          <h4>产出材料</h4>
    ...
    <div class="track-grid">
      ${lanes.map((lane) => `
        <article class="cutover-card">
          ...
          <h4>Signals</h4>
          <ul class="evidence-list">${(lane.signals || []).map((signal) => `<li><strong>${escapeHtml(signal.id)}</strong> 路 ${escapeHtml(signal.metric)} <= ${escapeHtml(signal.threshold)} 路 ${escapeHtml(signal.severity)}</li>`).join("")}</ul>
          <p><strong>No-Go:</strong> ${escapeHtml(lane.noGoRule)}</p>
          <p class="muted">Artifact: ${escapeHtml(lane.evidenceArtifact)} 路 Scenarios: ${(lane.linkedScenarios || []).map(escapeHtml).join(" / ")}</p>
        </article>
      `).join("")}
    </div>
    <div class="control-grid">
      <article class="cutover-card">
        ...
        <ul class="blocker-list">${(board.decisionOutcomes || []).map((item) => `<li><strong>${escapeHtml(item.id)}</strong> 路 ${escapeHtml(item.when)} -> ${escapeHtml(item.nextStep)}</li>`).join("")}</ul>
      </article>
      <article class="cutover-card">
