...
Run `npm.cmd run quality-safety:interface-standard` before institution joint testing. The generated `release/quality-safety-interface-standard.md` and `release/quality-safety-interface-standard.json` define the hospital-facing interface document format: document control, source systems, HTTPS/JSON transport, HMAC-SHA256 signature, idempotency, message envelope, status codes, field dictionaries, sample payloads, error/retry rules, audit trace, and acceptance checklist. `GET /api/quality-safety/interface-standard` exposes the same standard to commission, institution, and county roles for controlled docking review.

Run `npm.cmd run quality-safety:joint-test` before hospital sample-message review. The generated `release/quality-safety-interface-joint-test-pack.md` and `release/quality-safety-interface-joint-test-pack.json` include signed sample requests, body hashes, idempotency keys, field dictionaries, positive validation results, and negative cases for missing required fields, invalid signatures, and duplicate replay. `GET /api/quality-safety/interface-joint-test-pack` returns the same pack, while `POST /api/quality-safety/interface-messages/validate` validates hospital sample messages and records the validation result in `securityEvents`.

Go-live readiness boundary: the module is considered usable for a controlled pilot only when boundary coverage, seed/reuse data, API operations, closed-loop evidence, critical-value/pathway loops, policy references, role boundaries, risk ranking, and action-plan evidence all pass. Full production cutover still requires site sign-off for live interfaces, routing, local dictionaries, negative lists, department attachments, and production audit retention.
