...
| SEC-004 | XSS / CSP 面 | 37 个可执行内联脚本、8 个样式块和 13 个静态样式属性已外移；血液主工作台 25 个、急救生命链 6 个、医生工作台 6 个、血液上线看板 8 个和陪诊工作台 8 个 HTML sink 已迁为 DOM/text/class/dataset，并分别补恶意 API 载荷 E2E。29 个原模板 URL occurrence 已由 28 个真实 DOM 绑定迁移和 1 个 `item.action` 扫描误报校正闭合；Inventory v2 对 145 个发布资产锁定 843 个 DOM HTML、6 个动态 URL 和 45 个动态样式风险。Safe URL port 拒绝 javascript/data/userinfo/协议相对/未批准 Origin；兼容 CSP 仍含 `unsafe-inline`，严格策略仅 Report-Only | 仅 2 个 OHIF 导航仍 `review-required`；清单不判断 HTML 输入可信度，仍须治理 843 个 HTML 与 45 个动态样式 sink，并补全角色浏览器回归。真实 OHIF/对象存储 Origin、托管头、独立扫描和渗透验收前继续 NO-GO |
...
| API-002 | 接口目录复核 | v3 仍为 593 项、13 项认证证据且未分类为 0。T01 security-control action 已以 commission/step-up、精确 control ID、exact replay/conflict、单进程串行、SQLite CAS、稳定脱敏错误和一次 control+receipt+audit 写入闭合，使注册表达到 9 份合同、7 个 endpoint verified；未接 PG adapter、无 outbox、无跨实例 exactly-once。326 个写接口仍缺 endpoint 级证明，328 项保持 review-required | 继续逐 owner 补证；financial dispatch 与 formal grouping job 保留 `reviewedProofRequired`。`securityAcceptanceLedger` data owner、真实多实例/PG 行为和现场证据仍未决，该 endpoint 保持 NO-GO |
...
| TEST-006 | 静态基线与测试性能 | `test/api.test.js` 的全文件 `no-unreachable` 已关闭，原 3 个 care 显式 skip 已由 T05 owner/route 独立特征测试逐块重验并恢复执行；陪诊引用挂号单的存在性/scope 缺口与 handoff `reject/return` 投影已最小修复。typecheck 为 13 个唯一文件；两个前端文件的 16 个重复翻译键已有逐键 shadow/final 值和真实调用保护，去重保持首次插入顺序及最终生效值，lint 文件级例外已归零；API 热点仍作为独立 integration 批次输出耗时。两个可逆夹具切片已分别提取临时 seed/env/server 生命周期和单个 HIS hospital mock 生命周期，并以 43 个子测试顺序摘要锁定 suite 成员、断言、超时、单进程语义、CI 预算与 required checks 不降低 | API 巨型测试仍集中约 8,300 行业务断言；financial/alert/storage mock 和其余共享状态仍内联。后续只按一个共享服务生命周期继续小步拆分，不得按耗时并行化共享状态或把 helper 变成生产接口。耗时先观察多次 CI 分布，不凭单机样本设门槛；不得恢复文件级 lint 豁免；外部护理/陪诊/HIS 投递与现场证据继续由生产 readiness 跟踪，不以本测试关闭 |
...
| API-IDEM-001 | 2026-08-22 | `production-api-catalog-v2/v3` 将 source marker 与行为证明分层；2026-08-23 已扩展为 7 个 endpoint 与 2 个 action-slice。T01 security-control action 在既有 Accepted 机制内复用 session-security-audit owner repository；其余 2 条 `reviewedProofRequired` 仍不得与合同同 key 并存 | 合同唯一性、身份/范围/step-up、重放/冲突、同/异载荷并发、SQLite CAS 稳定错误、原子审计、写失败不二次审计、生产晋升与 distributed exactly-once 否定断言；CI 执行全部登记专项行为测试 |
...
| TEST-005 | 2026-08-23 | 本地/CI 统一 Playwright Chromium；在线根 31 项与居民 13 项继续阻止 Service Worker；PWA 3 项使用独立允许策略、动态端口和临时数据 | 47 项唯一并集/漂移测试、居民同文件 13/13、PWA 重复 9/9、完整标准 E2E；不得把仓库浏览器测试解释为真实 HTTPS、托管安全头或现场验收 |
