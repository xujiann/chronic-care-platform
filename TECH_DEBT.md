...
| API-002 | 接口目录复核 | v3 当前 629 项、13 项认证证据且未分类为 0。T00 已登记 39 份幂等合同、37 个 endpoint verified；招标需求复核、脱敏批次登记和交付治理入口保持高风险授权、幂等键和 CAS 失败关闭，状态、回执与审计一次持久化。`reviewedProofRequired` 为 0。362 个写接口中 325 个仍缺 endpoint 级证明，327 项保持 review-required | 继续逐 owner 渐进补证。既有聚合 receipt 均有界，进程锁只覆盖单实例，JSON 路径和 SQLite CAS 均不是跨实例 exactly-once。`resourceDispatchRequests` 仍有 T05 owner 的既有跨域写债；PG 多实例、长期审计/归档和真实外部/现场证据均未决，相关 endpoint 保持 NO-GO |
