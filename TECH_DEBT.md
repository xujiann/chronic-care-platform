...
| API-002 | 接口目录复核 | v3 仍为 593 项、13 项认证证据且未分类为 0。T07 formal grouping create 已补市级平台/医保资源范围、精确重放/冲突、同键并发、稳定脱敏错误、SQLite CAS 映射和 queued job+audit 单次提交，使注册表达到 12 份合同、10 个 endpoint verified；`reviewedProofRequired` 归零。323 个写接口仍缺 endpoint 级证明，325 项保持 review-required | 继续逐 owner 补证。formal grouping 的单进程锁不等于跨实例 exactly-once；真实正式分组适配器、凭据/网络、PG 多实例、worker 运维和现场证据仍未决，该 endpoint 保持 NO-GO |
