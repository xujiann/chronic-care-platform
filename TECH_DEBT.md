...
| API-002 | 接口目录复核 | v3 仍为 593 项、13 项认证证据且未分类为 0。T07 financial reconciliation 已以身份/网关范围、digest replay/conflict、单进程串行、SQLite CAS、稳定错误和一次 run+audit 写入闭合，使注册表达到 8 份合同、6 个 endpoint verified；无 outbox、无跨实例 exactly-once。327 个写接口仍缺 endpoint 级证明，329 项保持 review-required | 继续逐 owner 补证；financial dispatch 与 formal grouping job 保留 `reviewedProofRequired`。`financialReconciliationRuns` data owner、真实多实例/PG 行为和现场证据仍未决，该 endpoint 保持 NO-GO |
...
| API-IDEM-001 | 2026-08-22 | `production-api-catalog-v2/v3` 将 source marker 与行为证明分层；2026-08-23 已扩展为 6 个 endpoint 与 2 个 action-slice。T07 对账合同在既有 Accepted 机制内替换对应拒绝记录；其余 2 条 `reviewedProofRequired` 仍不得与合同同 key 并存 | 合同唯一性、身份/范围、重放/冲突、并发/CAS、稳定错误、原子审计、写失败不二次审计、生产晋升与 distributed exactly-once 否定断言；CI 执行全部登记专项行为测试 |
