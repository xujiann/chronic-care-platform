...
| API-IDEM-001 | 2026-08-22 | `production-api-catalog-v2` 将 source marker 与行为证明分层；SMS callback 首个 pilot 已在 2026-08-23 按同一 Accepted 机制扩展为 5 个 endpoint 与 2 个 action-slice 合同，且 action-slice 不晋升通用入口；T07 退款 endpoint 仍保留 runtime-role 复核阻断 | 合同唯一性、owner/route/action/身份/重放/冲突/CAS/错误/审计/实现/测试锚点、T07 匿名/角色/机构边界与原子 command/outbox、伪造 marker 或 action-slice 晋升、生产晋升和 distributed exactly-once 否定断言；CI 执行全部登记专项行为测试 |
