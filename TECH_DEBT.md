...
| SEC-004 | XSS / CSP 面 | 37 个可执行内联脚本、8 个样式块和 13 个静态样式属性已外移；Inventory v2 对 144 个发布资产锁定 896 个 DOM HTML、69 个动态 URL 和 48 个动态样式风险，替换/增加失败关闭。兼容 CSP 仍含 `unsafe-inline`，严格策略仅 Report-Only | 清单不判断输入可信度；仍须按页面补恶意输入、角色浏览器和动态样式回归，再独立移除兼容 `unsafe-inline`。真实托管头、独立扫描和渗透验收前继续 NO-GO |
...
| TEST-001 | 2026-08-22 | 建立 build/lint/typecheck/unit/integration/smoke 标准入口并映射 CI/Pages；test:all 与原 server.js 85/85/55 覆盖门禁语义不变；governance-api 静态发布链包含 Browser Inventory v2 | 标准门禁契约、完整测试分区、隔离 smoke、静态发布链、Inventory v2 精确 occurrence/指纹与 synthetic 负向矩阵、CI 映射、独立内部边界覆盖和全量回归 |
