...
| SEC-004 | XSS / CSP 面 | 37 个可执行内联脚本、8 个样式块和 13 个静态样式属性已外移；血液主工作台 25 个、急救生命链 6 个、医生工作台 6 个、血液上线看板 8 个、陪诊工作台 8 个、血液创新指挥中心 10 个、体检风险卡 2 个、产品运行驾驶舱、产品区域运行驾驶舱与质量安全工作台各 1 个、区域切换工作台和血液召回面板各 2 个 HTML sink 已迁为 DOM/text/class/dataset，并分别补恶意 API 载荷 E2E。Inventory v2 对 145 个发布资产锁定 824 个 DOM HTML、6 个动态 URL 和 45 个动态样式风险。Safe URL port 拒绝 javascript/data/userinfo/协议相对/未批准 Origin；兼容 CSP 仍含 `unsafe-inline`，严格策略仅 Report-Only | 体检页面其余 25 个 HTML sink 与 2 个 OHIF 导航仍须分批治理/复核；全清单仍有 824 个 HTML 与 45 个动态样式 sink。真实 OHIF/对象存储 Origin、托管头、独立扫描和渗透验收前继续 NO-GO |
...
| API-002 | 接口目录复核 | v3 仍为 593 项、13 项认证证据且未分类为 0。注册表达到 18 份合同、16 个 endpoint verified；新增首发 T02 operations dispatch/reconciliation review 与 T06 quality-safety dispatch/feedback/review 五项的身份/组织或 assigned-resource scope、精确重放/冲突、expectedVersion、同资源并发、稳定错误及业务+既有审计单次提交证据。`reviewedProofRequired` 为零。317 个写接口仍缺 endpoint 级证明，319 项保持 review-required；首发范围 API review 由 17 降为 12，19 个 legacy 集合 owner review 已关闭，`collectionReviewRequired=0` | 继续逐 owner 补证。五项内部 receipt 每资源最多 50 条，进程锁只覆盖单实例，JSON 路径不具备多实例原子性；SQLite CAS 也不是跨实例 exactly-once。`resourceDispatchRequests` 仍为 T05 owner 的既有跨域写债，机构代码别名仍依赖遗留运行目录；PG 多实例、长期审计/归档、真实外部/现场证据均未决，相关 endpoint 保持 NO-GO |
...
- 仓库审查：32 个 scoped API 中 12 个仍需要 repository behavior review；首发 19 个 legacy 数据引用已关闭 owner/governance review，范围 `collectionReviewRequired=0`。它们与 T05 `referrals` exact source binding、T00 `operationsReadiness` derived read model 共 21 个引用仍不具备生产写资格，且全部 38 个引用禁止生产晋级。
