...
| SEC-004 | XSS / CSP 面 | 37 个可执行内联脚本、8 个样式块和 13 个静态样式属性已外移；血液主工作台 25 个 HTML sink、2 个 CSSOM sink 和 1 个模板 style sink 已迁为 DOM/text/class 并补恶意 API 载荷 E2E；Inventory v2 对 145 个发布资产锁定 871 个 DOM HTML、35 个动态 URL 和 45 个动态样式风险。Safe URL port 已迁移可证明的内部导航、对象存储、tel/blob sink，并拒绝 javascript/data/userinfo/协议相对/未批准 Origin；兼容 CSP 仍含 `unsafe-inline`，严格策略仅 Report-Only | 29 个模板 URL 与 2 个 OHIF 导航仍 `review-required`；清单不判断 HTML 输入可信度，仍须补恶意输入、全角色浏览器和动态样式回归。真实 OHIF/对象存储 Origin、托管头、独立扫描和渗透验收前继续 NO-GO |
