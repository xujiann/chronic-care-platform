...
function resolveClinicalOrganizationCode(data, row) {
  ...
  const explicit = row.orgCode || row.institutionCode;
  if (explicit) return directory.filter((item) => item.orgCode === explicit).length === 1 ? explicit : "";
  const name = row.institution || row.sourceInstitution;
...
module.exports = { createClinicalAssistRuntime, resolveClinicalOrganizationCode };
