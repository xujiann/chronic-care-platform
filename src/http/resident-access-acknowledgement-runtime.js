...
function createResidentAccessAcknowledgementRuntime(ports) {
  ...
  function writeAccessAcknowledgementState(data) {
    if (shouldUseSqlite()) {
      ...
      for (const key of keys) {
        ...
        // v17 intentionally removes this empty legacy collection from state storage.
        if (key === "secureAttachments" && data[key] === undefined
          && Array.isArray(normalized[key]) && normalized[key].length === 0) continue;
  ...
  const command = createAccessAcknowledgementCommand({
    queryResidentAccessEvent,
    validateAuthorization({ data, user, session }) {
      try {
        const current = ports.runtimeSessionStore().get(session?.sessionId);
        const live = ports.validateLiveSession(current, data).user;
        if (!current || current.sessionId !== session?.sessionId || current.user?.id !== user.id
          || current.user.residentId !== user.residentId || session.user?.id !== user.id
          || session.user.residentId !== user.residentId || live.id !== user.id
          || live.role !== "citizen" || live.residentId !== user.residentId) throw new Error("subject changed");
      } catch {
        throw Object.assign(new Error("当前会话或本人身份已失效"), { code: "CARE_ACCESS_ACK_FORBIDDEN", statusCode: 403 });
      }
    },
    ...
    getRuntimePolicy() {
      ...
      const allowed = ["", "development", "test"].includes(environment)
        && ["memory", "sqlite"].includes(ports.sessionStoreMode())
  ...
  return Object.assign(command, { readAuthorizationState: readAccessAcknowledgementState });
}
