...
function createClinicalDecisionSupport(ports = {}) {
  ...
  function buildOverview(data, user) {
    ...
    const rules = rows(data, "phase2ClinicalAssistRules", "rules").map((row) => {
      const projected = pick(row, ["id", "category", "name", "sourceSystem", "triggerCondition", "severity", "defaultAction", "requiredFields", "configStatus", "owner", "version"]);
      if (!policyFor(data, { ruleId: row.id }).decisionAvailable) projected.defaultAction = "";
      return projected;
    });
  ...
  async function commitReceipt({ alertId, payload = {}, user, idempotencyKey }) {
    ...
    if (key !== undefined) {
      ...
      if (replay) {
        ...
        const currentOverview = buildOverview(data, user);
        const savedOverview = replay.commandMetadata.response.overview;
        if (JSON.stringify(savedOverview.rules) !== JSON.stringify(currentOverview.rules)
          || JSON.stringify(savedOverview.ruleConfig) !== JSON.stringify(currentOverview.ruleConfig)) fail(409, "CDSS_REPLAY_SCOPE_CHANGED");
