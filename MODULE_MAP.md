...
新增 T01 `src/identity-security/ai-governance.js` 和 T06 `src/clinical-specialties/clinical-decision-support/index.js`。T00 `src/http/clinical-assist-runtime.js` 注入可信目录和治理策略；旧临床 overview/receipt 业务从 server/混合路由移入领域端口。T01/T06 与兼容配置共用临床辅助状态写锁，目录歧义失败关闭。
