...
| Playwright E2E 基础设施 | `playwright-browser-policy.v1`、`playwright-pwa-browser-policy.v1`、`playwright-port-policy`、三套 runner/config | 在线根 59 项（含生产 Go/No-Go、各治理中心恶意响应、四方审批角色回归及 8 个已发布入口直接验证）与居民 13 项继续阻止 Service Worker；独立 PWA 3 项只在专项 context 允许 Worker，三套共 75 项唯一并集，统一 Chromium、动态端口和临时数据，并验证缓存/注册清理 |
