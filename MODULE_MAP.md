...
T00 单写者拥有 fixture、两份 live 用例、配置保护负测及专用 runner；两个开发角色按文件分工，独立审查后由 T00 串行集成。未新增运行时模块。
...
| `citizen.js` | 6,096 | 居民端视图、状态和流程耦合；评价控制器已下沉到独立模块 |
...
| `test/repository-governance.test.js` | T00 / TEST-001 | 锁定当前 290 份 Markdown、三类边界、当前 main 流程和 3 个 PDF 负向漂移 | 不证明 PDF 内容正确或生产可用 |
