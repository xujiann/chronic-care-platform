...
| Playwright E2E 基础设施 | `playwright-browser-policy.v1`、`playwright-port-policy`、`scripts/playwright-e2e*.js`、居民独占 runner | 根 27 项与居民 13 项形成 40 项唯一并集；统一 Chromium、阻止 Service Worker、动态端口和独立临时数据，避免系统 Chrome/固定端口/共享服务状态漂移 |
...
- `scripts/playwright-e2e.js` 与 `scripts/resident-mini-program-e2e.js` 分别拥有根套件和居民套件的
  动态端口/进程生命周期；两份 Playwright 配置只消费公共 Chromium 策略。专项治理测试从 Playwright
  列表验证 27 + 13 = 40 且无重复，固定端口或系统 Chrome 选择回流会失败。
