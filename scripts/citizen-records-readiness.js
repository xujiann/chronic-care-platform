...
function assessCitizenRecordsReadiness(options = {}) {
  ...
  const softwareChecks = [
    ...
    { id: "portable-record-export", label: "居民健康档案最小化可携带副本", passed: /buildResidentPortableArchive/.test(v2) && /PORTABLE_RECORD_CATEGORIES/.test(v2) && /health-record-export-dialog/.test(html) && /renderCitizenHealthRecordExportPreview/.test(ui) && /resident-health-record-export-v1/.test(e2e) },
    ...
    { id: "acceptance-documentation", label: "居民验收标准与外部依赖", passed: /第十八增量/.test(documentation) && /## 外部依赖/.test(documentation) }
  ];
  ...
  const integrationChecks = [
    ...
    },
    {
      ...
      passed: /citizen-records-v2\.js\?v=20260724care14/.test(serviceWorker) && /citizen\.js\?v=20260724care14/.test(serviceWorker)
    }
  ];
