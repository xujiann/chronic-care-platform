...
function assessCitizenRecordsReadiness(options = {}) {
  ...
  const softwareChecks = [
    ...
    { id: "portable-record-export", label: "居民健康档案最小化可携带副本", passed: /buildResidentPortableArchive/.test(v2) && /sealResidentPortableArchive/.test(v2) && /verifyResidentPortableArchive/.test(v2) && /health-record-export-dialog/.test(html) && /renderCitizenHealthRecordExportPreview/.test(ui) && /integrity-only-not-source-signature/.test(e2e) },
    ...
    { id: "acceptance-documentation", label: "居民验收标准与外部依赖", passed: /第十九增量/.test(documentation) && /## 外部依赖/.test(documentation) }
  ];
  ...
  const integrationChecks = [
    ...
    },
    {
      ...
      passed: /citizen-records-v2\.js\?v=20260724care15/.test(serviceWorker) && /citizen\.js\?v=20260724care15/.test(serviceWorker)
    }
  ];
