...
function buildPublicHealthPriorityStandardReviewReadiness(options = {}) {
  ...
  const checks = [
    ...
    check("safety:trust-fields-bound-to-receipt", ["clean(evidence.status).toLowerCase()", "clean(attestation.attestationOrigin).toLowerCase()", "clean(attestation.verificationSource).toLowerCase()", "attestation.signatureVerified === true"].every((token) => serviceSource.includes(token)) && testSource.includes("post-signing promotion of status origin source and signature verification"), "all trust-decision fields are bound to the signed server receipt", "safety"),
