...
function buildPublicHealthPriorityStandardReviewReadiness(options = {}) {
  ...
  const checks = [
    ...
    check("launch:site-evidence-boundary", reviewedPack?.summary?.siteEvidencePending === 8 && reviewedPack?.summary?.formallyAccepted === 0 && reviewedPack?.productionReady === false, `${reviewedPack?.summary?.siteEvidencePending ?? 8} site evidence packets pending`, "launch"),
    ...
    check("safety:negative-tests", ["incomplete domain", "verified signed site evidence", "role, idempotency and version"].every((token) => testSource.includes(token)), "review completeness, signed evidence and concurrency safeguards are tested", "safety"),
