...
function hasMonotonicEventTimes(entity = {}) {
  const times = (entity.events || []).map((event) => Date.parse(event.at));
...
function buildDiseasePaymentReadiness() {
  ...
  supervisionState.cases = [
    { ...supervisionState.cases[2], id: "readiness-profile-first", residentId: "readiness-resident", institution: "测试甲医院", admissionDate: "2026-05-01", dischargeDate: "2026-05-03", otherDiagnoses: [] },
  ...
  const settlementCreated = Service.createSettlementBatch(specialApproved.state, { period: "2026-06" }, "readiness-settlement", { at: "2026-07-19T08:00:00.000Z" });
  ...
  let settlementCorrection = Service.reconcileBatch(structuredClone(settlementCreated.state), settlementCreated.batch.id, { action: "submit-core", externalRequestId: "READINESS-CORRECTION-REQUEST-1", idempotencyKey: "READINESS-CORRECTION-IDEM-1", at: "2026-07-20T08:00:00.000Z" }, "readiness-settlement");
  settlementCorrection = Service.applyInsuranceCoreSettlementCallback(settlementCorrection.state, settlementCreated.batch.id, { action: "core-returned", receiptId: "READINESS-CORE-RETURNED", returnCycleId: "readiness-core-return-cycle", reasonCode: "FIELD_VALIDATION", reason: "readiness field correction required", requirementDigest: "6".repeat(64), correctionWorkingDays: 5, at: "2026-07-21T08:00:00.000Z" }, "readiness-insurance-core");
  const settlementCorrectionOverdue = Service.buildSettlementCoreCorrectionOperations(settlementCorrection.state, { at: "2026-08-01T08:00:00.000Z" });
  settlementCorrection = Service.reconcileBatch(settlementCorrection.state, settlementCreated.batch.id, { action: "resubmit-core", returnCycleId: "readiness-core-return-cycle", externalRequestId: "READINESS-CORRECTION-REQUEST-2", idempotencyKey: "READINESS-CORRECTION-IDEM-2", correctionDigest: "5".repeat(64), at: "2026-07-24T08:00:00.000Z" }, "readiness-settlement");
  settlementCorrection = Service.applyInsuranceCoreSettlementCallback(settlementCorrection.state, settlementCreated.batch.id, { action: "core-accepted", receiptId: "READINESS-CORRECTION-ACCEPTED", at: "2026-07-24T09:00:00.000Z" }, "readiness-insurance-core");
  const settlementCorrectionOperations = Service.buildSettlementCoreCorrectionOperations(settlementCorrection.state, { at: "2026-07-24T09:00:00.000Z" });
  const settlementSubmitted = Service.reconcileBatch(settlementCreated.state, settlementCreated.batch.id, { action: "submit-core", externalRequestId: "READINESS-CORE-REQUEST", idempotencyKey: "READINESS-CORE-IDEM", at: "2026-07-20T08:00:00.000Z" }, "readiness-settlement");
  const settlementAccepted = Service.applyInsuranceCoreSettlementCallback(settlementSubmitted.state, settlementCreated.batch.id, { action: "core-accepted", receiptId: "READINESS-CORE-ACCEPTED", at: "2026-07-21T08:00:00.000Z" }, "readiness-insurance-core");
  const settlementReconciling = Service.reconcileBatch(settlementAccepted.state, settlementCreated.batch.id, { action: "start-reconciliation", idempotencyKey: "READINESS-RECON", providerSummaryDigest: "c".repeat(64), at: "2026-07-22T08:00:00.000Z" }, "readiness-finance");
  let settlementDifference = Service.reconcileBatch(structuredClone(settlementReconciling.state), settlementCreated.batch.id, { action: "record-difference", idempotencyKey: "READINESS-DIFFERENCE", at: "2026-07-23T08:00:00.000Z", differenceAmountFen: -100, reasonCode: "READINESS_DEDUCTION", evidenceDigest: "8".repeat(64), reviewWorkingDays: 5 }, "readiness-finance");
  settlementDifference = Service.reconcileBatch(settlementDifference.state, settlementCreated.batch.id, { action: "review-difference", idempotencyKey: "READINESS-DIFFERENCE-HOSPITAL", at: "2026-07-24T08:00:00.000Z", reviewDomain: "hospital-finance", approved: true, adjustedAmountFen: settlementCreated.batch.standardAmountFen - 100, resolutionDigest: "7".repeat(64) }, "readiness-hospital-finance");
  settlementDifference = Service.reconcileBatch(settlementDifference.state, settlementCreated.batch.id, { action: "review-difference", idempotencyKey: "READINESS-DIFFERENCE-INSURANCE", at: "2026-07-25T08:00:00.000Z", reviewDomain: "insurance-settlement", approved: true, adjustedAmountFen: settlementCreated.batch.standardAmountFen - 100, resolutionDigest: "7".repeat(64) }, "readiness-insurance");
  settlementDifference = Service.reconcileBatch(settlementDifference.state, settlementCreated.batch.id, { action: "resolve-difference", idempotencyKey: "READINESS-DIFFERENCE-RESOLVE", at: "2026-07-26T08:00:00.000Z", resolution: "readiness dual-domain difference resolution", resolutionDigest: "7".repeat(64), adjustedAmountFen: settlementCreated.batch.standardAmountFen - 100 }, "readiness-finance");
  const settlementDifferenceCase = settlementDifference.batch.reconciliation.differenceCase;
  const settlementReconciled = Service.reconcileBatch(settlementReconciling.state, settlementCreated.batch.id, { action: "confirm-matched", idempotencyKey: "READINESS-MATCHED", providerAmountFen: settlementCreated.batch.standardAmountFen, at: "2026-07-23T08:00:00.000Z" }, "readiness-finance");
  let settlementPaymentRetry = Service.reconcileBatch(structuredClone(settlementReconciled.state), settlementCreated.batch.id, { action: "request-payment", paymentRequestId: "READINESS-PAYMENT-RETRY-REQUEST-1", idempotencyKey: "READINESS-PAYMENT-RETRY-IDEM-1", at: "2026-07-24T08:00:00.000Z" }, "readiness-settlement");
  settlementPaymentRetry = Service.applyInsuranceCoreSettlementCallback(settlementPaymentRetry.state, settlementCreated.batch.id, { action: "payment-failed", receiptId: "READINESS-PAYMENT-FAILED", failureCycleId: "readiness-payment-failure-cycle", reasonCode: "ACCOUNT_VALIDATION", reason: "readiness payment account validation failed", failureEvidenceDigest: "4".repeat(64), resolutionWorkingDays: 2, at: "2026-07-25T08:00:00.000Z" }, "readiness-insurance-core");
  const settlementPaymentFailureOverdue = Service.buildSettlementPaymentFailureOperations(settlementPaymentRetry.state, { at: "2026-08-01T08:00:00.000Z" });
  settlementPaymentRetry = Service.reconcileBatch(settlementPaymentRetry.state, settlementCreated.batch.id, { action: "retry-payment", failureCycleId: "readiness-payment-failure-cycle", paymentRequestId: "READINESS-PAYMENT-RETRY-REQUEST-2", idempotencyKey: "READINESS-PAYMENT-RETRY-IDEM-2", resolution: "readiness account evidence corrected", resolutionDigest: "3".repeat(64), at: "2026-07-27T08:00:00.000Z" }, "readiness-settlement");
  settlementPaymentRetry = Service.applyInsuranceCoreSettlementCallback(settlementPaymentRetry.state, settlementCreated.batch.id, { action: "confirm-payment", receiptId: "READINESS-PAYMENT-RETRY-SUCCEEDED", paidAmountFen: settlementCreated.batch.standardAmountFen, at: "2026-07-27T09:00:00.000Z" }, "readiness-insurance-core");
  const settlementPaymentFailureOperations = Service.buildSettlementPaymentFailureOperations(settlementPaymentRetry.state, { at: "2026-07-27T09:00:00.000Z" });
  const settlementPaymentRequested = Service.reconcileBatch(settlementReconciled.state, settlementCreated.batch.id, { action: "request-payment", paymentRequestId: "READINESS-PAYMENT-REQUEST", at: "2026-07-24T08:00:00.000Z" }, "readiness-settlement");
  const settlementPaid = Service.applyInsuranceCoreSettlementCallback(settlementPaymentRequested.state, settlementCreated.batch.id, { action: "confirm-payment", receiptId: "READINESS-PAYMENT-RECEIPT", paidAmountFen: settlementCreated.batch.standardAmountFen, at: "2026-07-25T08:00:00.000Z" }, "readiness-insurance-core");
  const settlementTimelineValid = settlementReceipts.every((receipt) => Date.parse(receipt.signedAt) <= Date.parse(settlementCreated.batch.frozenAt))
    && [settlementCorrection.batch, settlementDifference.batch, settlementPaymentRetry.batch, settlementPaid.batch, settlementDifferenceCase].every(hasMonotonicEventTimes);
  ...
  const checks = [
    ...
    { id: "settlement-time-order", label: "正式回执、冻结、回调、对账与拨付证据时间单调", ok: settlementTimelineValid },
...
function renderMarkdown(report) {
  return [
    ...
    `- 病种监管档案：${report.summary.supervision.profiles}（跨机构重复住院${report.summary.supervision.crossInstitutionAdmissions}，编码复杂度跃升${report.summary.supervision.codingEscalations}，疑似基金影响${report.summary.supervision.estimatedFundImpact}元）`,
    "",
    "## 检查项",
    "",
