...
function buildDiseasePaymentReadiness() {
  ...
  const checks = [
    ...
    { id: "formal-grouping-async", label: "正式分组异步作业、幂等派发与关联回执", ok: formalJobDuplicate.idempotent && formalJobReceived.job.status === "completed" && formalJobReceived.run.succeeded === 1 && formalJobReceived.job.receiptCount === 1 && Intake.verifyFormalGroupingJobLedger(formalJobReceived.job) },
    ...
    { id: "formal-grouping-integrity", label: "正式分组作业/死信状态投影、哈希事件链与运维最小披露", ok: formalOperations.summary.invalidJobs === 0 && formalOperations.summary.invalidDeadLetters === 0 && formalOperations.jobs.every((item) => item.integrity && item.caseSnapshots === undefined) && formalOperations.deadLetters.every((item) => item.integrity && item.errorMessage === undefined && item.resolution === undefined) },
