...
function buildDiseasePaymentReadiness() {
  ...
  const checks = [
    ...
    { id: "formal-grouping-async", label: "正式分组异步作业、幂等派发、关联回执与结果账本交叉验真", ok: formalJobDuplicate.idempotent && formalJobReceived.job.status === "completed" && formalJobReceived.run.succeeded === 1 && formalJobReceived.job.receiptCount === 1 && Intake.verifyFormalGroupingJobLedger(formalJobReceived.job) && Intake.verifyFormalGroupingResultProjection(formalJobReceived.state, formalJobReceived.job) },
