...
function seedPublicHealthEvents() {
  return [
    { id: "phe-infectious-001", domain: "传染病防控", signal: "发热门诊聚集性呼吸道病例", institution: "区域中心医院", region: "中山区", sourceSystem: "EMR/LIS", status: "待疾控复核", priority: "high", reportedAt: "2026-07-08T08:30:00+08:00", commandAction: "市疾控发起流调任务并锁定实验室复核样本。", followupAction: "24 小时内回写流调结论和报告质量控制记录。", linkedStandardItems: ["病例报告", "流行病学调查", "呼吸道传染病实验室检测"] },
...
function buildPublicHealthReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    check("frontend:coordination-actions", ["handlePublicHealthCoordinationAction", "assign-coordination", "record-coordination-receipt", "retry-coordination", "close-coordination", "reopen-coordination", "/api/public-health/coordination/", "expectedVersion", "idempotencyKey"].every((token) => sources.js.includes(token)), "coordination actions are versioned and idempotent at the public route boundary", "frontend"),
