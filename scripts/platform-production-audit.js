...
const MVP_REQUIRED_MODULES = [
  ...
  },
  {
    ...
    priority: "P0",
    implemented: "支付、医保和电子证照通用出站网关，14 个受控操作，HMAC 签名、幂等、整数分金额、敏感字段拦截、有限重试、回执、死信、独立密钥回调验签、时间窗与 nonce 防重放、金额一致性、旧回执隔离、乱序终态保护、合法冲正、摘要级日终对账和运维中心",
    remainingCode: "按支付机构、医保核心和证照平台协议补充专有报文与签名映射、来源白名单、账单传输、长短款和差错补偿策略",
...
const ROADMAP = [
  {
    ...
    deliverables: ["已完成不可变部署包、密钥引用和进程模板；继续 Vault/KMS、TLS、制品仓库和环境级编排", "已完成 PostgreSQL 主读取演练与证据门禁生产适配器；继续领域转换、容量故障切换、原生备份恢复和正式回切", "已完成统一身份和短信通用适配基础；继续真实供应商联调和密钥托管", "已完成金融签名回调与摘要对账、医院出站连接器及 SIEM/Webhook 告警适配基础；继续厂商协议、目标接收端、网络环境和生产演练"],
...
function buildPlatformProductionAudit(options = {}) {
  ...
  const runtimeSources = options.runtimeSources || {
    ...
    siteAcceptanceDocumentation: read("docs/platform-site-acceptance-gate.md"),
    ...
    financialAdapter: read("financial-gateways.js"),
    financialDocumentation: read("docs/production-financial-certificate-gateways.md")
  };
  ...
  const checks = [
    ...
    check("platformAudit:capabilityOperationsCenter", runtimeSources.domain.includes("applyPlatformCapabilityReviewAction") && runtimeSources.domain.includes("applyPlatformProductionBlockerAction") && runtimeSources.domain.includes("record-site-acceptance") && runtimeSources.server.includes("/api/platform/capability-operations/blockers/") && runtimeSources.html.includes("platform-capability-operations-center") && runtimeSources.client.includes("renderPlatformCapabilityOperationsCenter") && runtimeSources.client.includes("runPlatformProductionBlockerAction") && runtimeSources.client.includes("record-site-acceptance") && runtimeSources.documentation.includes("reviewed-preproduction") && runtimeSources.documentation.includes("evidence-reviewed-site-pending") && runtimeSources.siteAcceptanceDocumentation.includes("site-accepted") && runtimeSources.siteAcceptanceDocumentation.includes("blocked-until-global-go-no-go"), "capability review, site acceptance, and production-blocker evidence workflows are runnable, audited and documented"),
  ...
  return {
    ...
    summary: {
      ...
      financialCallbackReconciliationAdapter: checks.some((item) => item.id === "platformAudit:financialCallbackReconciliation" && item.passed)
    },
    auditBasis: [
      "运行页面与角色入口",
      "Node API 与状态集合",
...
function renderMarkdown(report) {
  return [
    ...
    `P0-06 金融签名回调与摘要级日终对账已${report.summary.financialCallbackReconciliationAdapter ? "纳入" : "未纳入"}发布门禁；真实机构协议、来源白名单、账单传输、差错补偿和联合签字仍为上线阻断项。`,
    "",
    "## 二、审计范围与方法",
    "",
