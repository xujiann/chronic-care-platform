...
function buildInternetNursingReadinessReport(options = {}) {
  ...
  const mobilePreviewJs = options.mobilePreviewJs ?? (options.mobilePreview === undefined ? readText("mobile-preview.js") : "");
  const mobilePreviewRuntime = `${mobilePreview}\n${mobilePreviewJs}`;
  ...
  const checks = [
    ...
    { id: "nursing:accountProvisioning", passed: /data-account-provisioning/.test(login) && /data-provisioning-step="nurse"/.test(login) && /data-provisioning-step="audit"/.test(login) && /data-provisioning-owner/.test(login) && /医生和护理端账号需由机构开通/.test(login) && /执业资质/.test(login) && /资质核验/.test(login) && /账号审计留痕/.test(login) && /试点机构护理部/.test(login) && /平台账号管理员/.test(login) && /authUsers/.test(login) && /securityEvents/.test(login) && /dataAccessLogs/.test(login) && /nurseId/.test(login) && /accountType: "nurse"/.test(auth) && /nurseId: "inn-001"/.test(auth) && !/id="register-form"|\/api\/auth\/register/.test(`${login}\n${auth}\n${server}`), detail: "resident, doctor, and nurse accounts are provisioned through real-name binding, institution approval, qualification check, owner accountability, and account audit evidence, with no public self-registration route" },
    ...
    { id: "nursing:mobileWorkflow", passed: /nursing-mobile-workbench/.test(frontend) && /nursing-mobile-appointment/.test(frontend) && /nursing-nurse-mobile/.test(frontend) && /renderMobileAppointmentStatus/.test(frontend) && /renderMobileNurseCards/.test(frontend) && /internet-nursing-mobile/.test(frontend) && /src="\.\/mobile-preview\.js"/.test(mobilePreview) && /citizenPreviewSrc\(service\)/.test(mobilePreviewRuntime) && /data-preview-service="nursing"/.test(mobilePreview) && !/internet-nursing\.html\?preview=mobile-nursing/.test(mobilePreviewRuntime), detail: "citizen appointment and nurse response are available, and the mobile preview stays inside the citizen nursing service page" },
    ...
    { id: "nursing:authNavigation", passed: /"internet-nursing\.html": entry\("互联网护理", \["commission", "institution", "citizen", "county"\]/.test(accessPolicy) && /username: "nurse"/.test(auth) && /password: "123456"/.test(auth) && /nurseId: "inn-001"/.test(auth) && /\u4ec5\u67e5\u770b/.test(frontend) && /\u9700\u533b\u9662\u6d3e\u5355/.test(frontend), detail: "central route policy, nurse demo account, and role-scoped actions present" },
