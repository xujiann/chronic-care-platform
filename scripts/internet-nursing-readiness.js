...
function buildInternetNursingReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "nursing:accountProvisioning", passed: /data-account-provisioning/.test(login) && /护理端护士账号需由试点机构开通/.test(login) && /nurseId/.test(login) && /accountType: "nurse"/.test(auth) && /nurseId: "inn-001"/.test(auth) && !/id="register-form"|\/api\/auth\/register/.test(`${login}\n${auth}\n${server}`), detail: "resident and nurse accounts are provisioned through real-name binding and pilot institution approval, with no public self-registration route" },
