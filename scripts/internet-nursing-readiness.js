...
function buildInternetNursingReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "nursing:closedLoopSummary", passed: /buildNursingClosedLoopSummary/.test(frontend) && /renderNursingClosedLoopSummary/.test(frontend) && /nursing-loop-summary/.test(frontend) && /completionRate/.test(frontend) && /readyForDispatch/.test(frontend) && /waitingAssessment/.test(frontend) && /nursing-loop-summary/.test(portalCss), detail: "closed-loop summary exposes completion, dispatch, pending dispatch, and assessment or signature gaps" },
