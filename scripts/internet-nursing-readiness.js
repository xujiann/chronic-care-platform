...
function buildInternetNursingReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "nursing:frontend", passed: /nursing-appointment-form/.test(frontend) && /nursing-nurse-queue/.test(frontend) && /fetchInternetNursingDashboard/.test(frontend), detail: "citizen, hospital, and nurse work areas present" },
    { id: "nursing:authNavigation", passed: /"internet-nursing\.html": \["commission", "institution", "citizen", "county"\]/.test(auth) && /username: "nurse"/.test(auth) && /password: "123456"/.test(auth) && /nurseId: "inn-001"/.test(auth) && /view only/.test(frontend) && /hospital dispatch required/.test(frontend), detail: "route access, nurse demo account, and role-scoped actions present" },
