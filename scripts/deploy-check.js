...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "api:registrationJourney", ok: ["/api/registrations/orders/:id/actions", "/api/registrations/orders/:id/disruption", "registration-journey-action", "registration-disruption-action", "applyRegistrationJourneyAction", "applyRegistrationDisruptionAction"].every((marker) => serverSource.includes(marker)) && ["registration-journey-timeline", "runRegistrationJourneyAction", "runRegistrationDisruptionAction", "data-registration-journey-action", "data-registration-disruption-action"].every((marker) => citizenHtml.includes(marker) || citizenSource.includes(marker)) && ["registration-journey-workbench", "renderRegistrationJourneyWorkbench", "runInstitutionRegistrationDisruptionAction", "data-registration-institution-action", "data-registration-disruption-schedule"].every((marker) => institutionHtml.includes(marker) || institutionSource.includes(marker)) && ["payment", "refund", "HIS", "insurance", "resident-acceptance", "productionReady=false"].every((marker) => registrationJourneyDoc.includes(marker)), detail: "registration payment, confirmation, check-in, completion, refund and hospital disruption rescheduling are wired across resident and institution roles" },
