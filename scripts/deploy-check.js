...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "security:productionRuntimeBoundary", ok: ["LOCAL_PASSWORD_LOGIN_DISABLED", "assertProductionRuntimeSecurity", "PRODUCTION_SESSION_SECRET_INVALID", "PRODUCTION_SESSION_STORE_INVALID", "sessionStoreStatus", "script-src-attr 'none'", "applyResidentScope", "canManageResidentProfile"].every((marker) => serverSource.includes(marker)) && ["SqliteSessionStore", "auth_sessions", "revokeByUserIds", "crossProcess: true"].every((marker) => sessionStoreSource.includes(marker)) && platformSource.includes("同一主机共享数据目录可跨进程撤销") && envTemplateSource.includes("SESSION_STORE=sqlite") && authSource.includes("认证服务暂不可用，请稍后重试") && [bloodBusinessSource, bloodRecallSource].every((source) => source.includes("escapeHtml")) && ["生产安全边界", "集中式会话"].every((marker) => productionAdaptersDoc.includes(marker)), detail: "production local passwords, weak signing secrets and in-memory session stores are blocked; durable session revocation, operations visibility, browser auth, resident scoping, CSP and persistent HTML fields fail closed" },
