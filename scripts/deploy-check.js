...
function buildDeployCheckReport(options = {}) {
  ...
  const platformDevelopmentReportDoc = fs.readFileSync(path.join(ROOT, "docs", "数智医院标准平台开发报告与下一步计划-2026-07-12.md"), "utf8");
  ...
  const checks = [
    ...
    assertFile("docs/数智医院标准平台开发报告与下一步计划-2026-07-12.md"),
    ...
    { name: "docs:platformDevelopmentReport20260712", ok: ["本轮主要开发成果", "PostgreSQL 事务 outbox", "PostgreSQL 幂等影子同步", "正式生产前已实现的主要标准平台功能", "验证证据", "当前生产阻断项", "下一步开发计划", "P0：0-30 天", "P1：31-60 天", "P2：61-90 天", "82/82", "311/311", "251/252", "226/226"].every((marker) => platformDevelopmentReportDoc.includes(marker)), detail: "dated platform development report records transactional outbox, idempotent shadow sync, verification evidence, blockers and 30/60/90-day plan" },
    ...
    { name: "database:postgresTransactionalOutbox", ok: ["STORAGE_SCHEMA_VERSION = 8", "postgres_sync_outbox", "buildCollectionChanges", "enqueuePostgresSyncBatch", "readPostgresSyncStatus", "productionPrimary: false"].every((marker) => serverSource.includes(marker)) && ["buildPostgresSyncBatch", "validatePostgresSyncBatch", "previousChainHash", "chainHash"].every((marker) => postgresRuntimeSyncSource.includes(marker)), detail: "SQLite schema v8 atomically stores signed collection changes and health-visible queue state" },
