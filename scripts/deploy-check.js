...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "docs:citizenExternalDependencyOwners", ok: ["platform-ops", "identity-integration", "resident-master-index", "security-compliance", "mobile-release", "required-before-production", "上线前必须归档的证据"].every((marker) => citizenProductionRequirementsDoc.includes(marker)), detail: "citizen launch requirements document production dependency owners and evidence" },
