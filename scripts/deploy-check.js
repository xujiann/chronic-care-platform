...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "api:publicHealthLaunchCommandBriefs", ok: ["/api/public-health/launch-command-briefs", "/api/public-health/launch-command-briefs/:id/actions", "public-health-launch-command-brief-action", "seedPublicHealthLaunchCommandBriefs"].every((marker) => serverSource.includes(marker)) && ["public-health-launch-command-briefs", "renderLaunchCommandBriefs", "data-public-health-launch-command-brief"].every((marker) => fs.readFileSync(path.join(ROOT, "public-health.js"), "utf8").includes(marker) || fs.readFileSync(path.join(ROOT, "public-health.html"), "utf8").includes(marker)), detail: "public health launch command brief API and board are wired" },
    ...
    { name: "api:registrationJourney", ok: ["/api/registrations/orders/:id/actions", "registration-journey-action", "applyRegistrationJourneyAction"].every((marker) => serverSource.includes(marker)) && ["registration-journey-timeline", "runRegistrationJourneyAction", "data-registration-journey-action"].every((marker) => citizenHtml.includes(marker) || citizenSource.includes(marker)) && ["registration-journey-workbench", "renderRegistrationJourneyWorkbench", "data-registration-institution-action"].every((marker) => institutionHtml.includes(marker) || institutionSource.includes(marker)) && ["payment", "refund", "HIS", "insurance", "productionReady=false"].every((marker) => registrationJourneyDoc.includes(marker)), detail: "registration payment, hospital confirmation, check-in, completion and refund journey is wired across resident and institution roles" },
    ...
    { name: "docs:publicHealthLaunchCommandBriefs", ok: ["/api/public-health/launch-command-briefs/:id/actions", "publicHealthLaunchCommandBriefs", "launch command brief"].every((marker) => publicHealthDoc.includes(marker)), detail: "public health system report documents launch command brief broadcasts" },
