...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "api:registrationIntegration", ok: ["appointment-order-v1", "/api/registrations/integration-center", "applyRegistrationIntegrationCallback", "landAppointmentIntegrationEvent"].every((marker) => serverSource.includes(marker)) && ["registration-integration-center", "registration-integration-events", "renderRegistrationIntegrationCenter"].every((marker) => institutionHtml.includes(marker) || institutionSource.includes(marker)) && ["HMAC-SHA256", "idempotencyKey", "refund-failed", "Production boundary"].every((marker) => registrationIntegrationDoc.includes(marker)), detail: "signed appointment callbacks, order landing, reconciliation, dead-letter handling and institution UI are wired" },
