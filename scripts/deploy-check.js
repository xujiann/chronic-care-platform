...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "security:productionRuntimeBoundary", ok: ["LOCAL_PASSWORD_LOGIN_DISABLED", "assertProductionRuntimeSecurity", "PRODUCTION_SESSION_SECRET_INVALID", "PRODUCTION_SESSION_STORE_INVALID", "PRODUCTION_SESSION_RETENTION_INVALID", "sessionStoreStatus", "cleanupRuntimeSessions", "scheduleSessionCleanup", "/api/auth/sessions/cleanup", "SESSION_CLEANUP_CONFIRMATION_REQUIRED", "script-src-attr 'none'", "applyResidentScope", "canManageResidentProfile"].every((marker) => serverSource.includes(marker)) && ["SqliteSessionStore", "auth_sessions", "revokeByUserIds", "cleanup(options", "deletedExpired", "deletedRevoked", "crossProcess: true"].every((marker) => sessionStoreSource.includes(marker)) && ["同一主机共享数据目录可跨进程撤销", "会话保留"].every((marker) => platformSource.includes(marker)) && ["SESSION_STORE=sqlite", "SESSION_EXPIRED_RETENTION_DAYS=", "SESSION_REVOKED_RETENTION_DAYS=", "SESSION_CLEANUP_INTERVAL_MS="].every((marker) => envTemplateSource.includes(marker)) && authSource.includes("认证服务暂不可用，请稍后重试") && [bloodBusinessSource, bloodRecallSource].every((source) => source.includes("escapeHtml")) && ["生产安全边界", "集中式会话", "POST /api/auth/sessions/cleanup"].every((marker) => productionAdaptersDoc.includes(marker)), detail: "production local passwords, weak signing secrets and in-memory session stores are blocked; durable session revocation, retention cleanup, operations visibility, browser auth, resident scoping, CSP and persistent HTML fields fail closed" },
