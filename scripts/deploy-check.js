...
function buildDeployCheckReport(options = {}) {
  const sqliteMigrationRegistry = validateSqliteMigrationRegistry();
  ...
  const regionalSharingReadModelSource = fs.readFileSync(path.join(ROOT, "src", "platform", "governance", "regional-sharing-read-model.js"), "utf8");
  const browserSecurityPolicySource = fs.readFileSync(path.join(ROOT, "browser-security-policy.json"), "utf8");
  const sqliteMigrationSource = fs.readFileSync(path.join(ROOT, "src", "platform", "storage", "sqlite-migrations.js"), "utf8");
  const sqliteRuntimeSource = `${serverSource}\n${sqliteMigrationSource}`;
  ...
  const postgresTransitionReadiness = fs.readFileSync(path.join(ROOT, "scripts", "postgres-primary-transition-readiness.js"), "utf8");
  const preproductionControlSource = fs.readFileSync(path.join(ROOT, "scripts", "platform-preproduction-control.js"), "utf8");
  const postgresRuntimeSyncDoc = fs.readFileSync(path.join(ROOT, "docs", "postgresql-runtime-sync.md"), "utf8");
  const postgresPrimaryStorageDoc = fs.readFileSync(path.join(ROOT, "docs", "postgresql-primary-storage-contract.md"), "utf8");
  const productionAdaptersEnvTemplate = fs.readFileSync(path.join(ROOT, "deploy", "platform-production-adapters.env.template"), "utf8");
  ...
  const checks = [
    ...
    assertFile("scripts/postgres-primary-transition-readiness.js"),
    assertFile("scripts/chronic-followup-dispatch-worker.js"),
    assertFile("src/citizen-chronic/followup-dispatch-outbox.js"),
    assertFile("src/citizen-chronic/followup-dispatch-worker.js"),
    assertFile("src/citizen-chronic/followup-dispatch-activation-provider.js"),
    assertFile("deploy/chronic-followup-dispatch-worker.service.template"),
    assertFile("deploy/chronic-followup-dispatch-worker.timer.template"),
    assertFile("deploy/chronic-followup-dispatch-worker.env.template"),
    ...
    assertFile("src/platform/storage/postgres-primary-storage-contract.js"),
    assertFile("src/platform/storage/postgres-primary-driver.js"),
    assertFile("deploy/postgres-primary-storage-schema.sql"),
    assertFile("docs/postgresql-primary-storage-contract.md"),
    assertFile("src/platform/regional/site-deployment-plan.js"),
    assertFile("src/platform/regional/regional-release-governance.js"),
    assertFile("src/platform/regional/multi-region-operations.js"),
    assertFile("src/platform/regional/regional-configuration-readiness.js"),
    assertFile("src/platform/regional/regional-site-evidence-trust.js"),
    assertFile("src/platform/regional/regional-site-evidence-lifecycle.js"),
    assertFile("src/platform/regional/regional-site-evidence.js"),
    assertFile("src/platform/regional/regional-cutover-dossier.js"),
    assertFile("src/platform/regional/regional-cutover-workbench.js"),
    assertFile("src/platform/regional/regional-pilot-program.js"),
    assertFile("scripts/regional-configuration-readiness.js"),
    assertFile("scripts/regional-site-evidence-readiness.js"),
    assertFile("scripts/regional-site-evidence-lifecycle.js"),
    assertFile("scripts/regional-cutover-dossier.js"),
    assertFile("scripts/regional-pilot-readiness.js"),
    assertFile("regional-cutover-workbench-ui.js"),
    assertFile("config/regional-pilot-program.json"),
    assertFile("src/http/platform-runtime-composition.js"),
    assertFile("src/http/platform-request-handler.js"),
    assertFile("src/platform/data/collection-governance.js"),
    assertFile("scripts/data-collection-governance.js"),
    assertFile("src/platform/governance/release-evidence-freshness.js"),
    assertFile("scripts/release-evidence-provenance.js"),
    assertFile("src/platform/governance/platform-nonfunctional-readiness.js"),
    assertFile("scripts/platform-nonfunctional-readiness.js"),
    assertFile("config/platform-nonfunctional-budgets.json"),
    assertFile("docs/平台下一阶段六迭代交付报告-2026-08-16.md"),
    assertFile("deploy/regional-site-evidence.template.json"),
    assertFile("deploy/regional-site-evidence.env.template"),
    assertFile("deploy/regional-site-evidence-trust-registry.template.json"),
    assertFile("deploy/regional-site-evidence-lifecycle-registry.template.json"),
    assertFile("config/regional-replication-sites.json"),
    ...
    assertFile("production-preflight-decision-receipt.js"),
    ...
    assertFile("config/platform-productization-program.json"),
    assertFile("config/p0-data-promotions.json"),
    assertFile("config/platform-work-item-sources.json"),
    assertFile("config/institution-integration-catalog.json"),
    assertFile("config/regional-capability-bundles.json"),
    assertFile("src/platform/productization/runtime.js"),
    ...
    { name: "package:chronicFollowupDispatchWorker", ok: Boolean(pkg.scripts?.["chronic:followup-dispatch-worker"] && pkg.scripts?.["chronic:followup-dispatch-preflight"] && pkg.scripts?.["chronic:followup-dispatch-status"]), detail: `${pkg.scripts?.["chronic:followup-dispatch-worker"] || "missing"} / durable SQLite worker remains production NO-GO` },
    ...
    { name: "api:financialGateways", ok: ["dispatchFinancialRequest", "financialGatewayCenter", "verifyFinancialCallback", "applyFinancialCallback", "createFinancialReconciliationRun", "FINANCIAL_CALLBACK_REPLAY_DETECTED", "amount-mismatch", "superseded-receipt", "FORBIDDEN_PAYLOAD_KEYS", "FINANCIAL_GATEWAY_MAX_ATTEMPTS"].every((marker) => financialGatewaysSource.includes(marker)) && ["/api/financial-gateways", "/api/financial-gateways/dispatch", "/api/financial-gateways/callbacks/", "/api/financial-gateways/operations", "/api/financial-gateways/reconciliation-runs", "financialReconciliationRuns", "adapterType: \"financial\"", "retrySource?.adapterType === \"financial\""].every((marker) => serverSource.includes(marker)) && ["financial-gateway-operations-center", "financial-gateway-callback-events", "financial-reconciliation-runs"].every((marker) => platformHtml.includes(marker)) && ["renderFinancialGatewayOperationsCenter", "loadFinancialGatewayOperationsCenter"].every((marker) => platformSource.includes(marker)) && ["FINANCIAL_CALLBACK_SECRET=", "FINANCIAL_CALLBACK_MAX_SKEW_SECONDS=", "FINANCIAL_IDEMPOTENCY_LEDGER_MAX_EVENTS=", "FINANCIAL_IDEMPOTENCY_LEDGER_MAX_EVENTS_PER_SCOPE=", "FINANCIAL_CALLBACK_LEDGER_MAX_EVENTS_PER_REQUEST="].every((marker) => envTemplateSource.includes(marker)) && financialGatewayReadiness.includes("financialGateway:operationsUi") && ["适配器基础通过不等于支付、医保或电子证照已经正式验收", "签名回调代码就绪", "摘要级日终对账", "现场联合测试回执"].every((marker) => financialGatewaysDoc.includes(marker)), detail: "payment insurance and certificate gateways are runtime-wired with outbound signing, minimized payloads, guarded idempotency reservations, signed replay-safe amount-aware callbacks, digest-only daily reconciliation, operations UI and explicit site boundaries" },
    ...
    { name: "snapshot:regionalHandoffRuntime", ok: ["/api/regional-data-sharing/handoff-report", "regionalSharingReadModel.buildRegionalHandoffReport"].every((marker) => serverSource.includes(marker)) && ["buildRegionalHandoffReport", "renderRegionalHandoffMarkdown"].every((marker) => regionalSharingReadModelSource.includes(marker)) && ["regional-handoff-report-action", "regional-handoff-report"].every((marker) => regionalDataSharingHtml.includes(marker)) && ["generateRegionalHandoffReport", "/api/regional-data-sharing/handoff-report"].every((marker) => regionalDataSharingSource.includes(marker)), detail: "regional referral handoff report API, role-scoped checklist UI and runtime no-merge boundary are wired" },
    ...
    { name: "api:digitalHospitalSelfAssessment", ok: ["/api/digital-hospital/self-assessments", "/api/digital-hospital/self-assessments/:id/actions", "buildDigitalHospitalSelfAssessmentBoard", "createDigitalHospitalSelfAssessment", "normalizeDigitalHospitalSelfAssessmentAction", "digitalHospitalSelfAssessments"].every((marker) => serverSource.includes(marker)) && ["DIGITAL_HOSPITAL_SELF_ASSESSMENT_INDICATORS", "assign-assessment", "save-draft", "submit-assessment", "start-preliminary-review", "escalate-expert-review", "record-expert-opinion", "request-correction", "accept-assessment"].every((marker) => digitalHospitalSelfAssessmentSource.includes(marker)) && digitalHospitalSelfAssessmentHtml.includes('src="./page-auth-bootstrap.js" data-roles="commission,institution"') && digitalHospitalSelfAssessmentUi.includes("recordDigitalSelfAssessmentAction") && digitalHospitalSelfAssessmentUi.includes("digital-self-assessment-dispute-indicators"), detail: "institution-scoped self-assessment, correction and tiered independent expert review workflow is wired" },
    { name: "ui:digitalHospitalPilotEvaluation", ok: ["pilot-operations", "pilot-issues", "catalog", "collection", "evidence", "preassessment", "rectification", "boundary"].every((marker) => digitalHospitalEvaluationHtml.includes(`data-digital-evaluation-section="${marker}"`)) && digitalHospitalEvaluationHtml.includes('src="./page-auth-bootstrap.js" data-roles="commission,institution"') && digitalHospitalEvaluationHtml.includes("digital-evaluation-issue-action-no-pii") && digitalHospitalEvaluationUi.includes("run-preassessment") && digitalHospitalEvaluationUi.includes("renderPilotIssueActions") && digitalHospitalEvaluationUi.includes("refreshBoard"), detail: "four-pack evaluation and role-scoped pilot issue closure workbench are visible" },
    ...
    { name: "database:sqliteMigrationRegistry", ok: sqliteMigrationRegistry.head === SQLITE_SCHEMA_HEAD && /^[a-f0-9]{64}$/.test(sqliteMigrationRegistry.registryFingerprint) && serverSource.includes("STORAGE_SCHEMA_VERSION = SQLITE_SCHEMA_HEAD"), detail: `SQLite schema v${SQLITE_SCHEMA_HEAD} / registry ${sqliteMigrationRegistry.registryFingerprint.slice(0, 12)}` },
    { name: "database:postgresTransactionalOutbox", ok: ["STORAGE_SCHEMA_VERSION = SQLITE_SCHEMA_HEAD", "postgres_sync_outbox", "buildCollectionChanges", "enqueuePostgresSyncBatch", "readPostgresSyncStatus", "productionPrimary: false"].every((marker) => sqliteRuntimeSource.includes(marker)) && ["buildPostgresSyncBatch", "validatePostgresSyncBatch", "previousChainHash", "chainHash"].every((marker) => postgresRuntimeSyncSource.includes(marker)), detail: `SQLite schema v${SQLITE_SCHEMA_HEAD} retains atomic signed collection changes and health-visible queue state` },
    { name: "database:postgresIdempotentWorker", ok: ["runtime_sync_batches", "ON CONFLICT (batch_id) DO NOTHING", "FOR UPDATE", "POSTGRES_SYNC_BATCH_ID_CONFLICT", "runtime_collection_state", "source_version < EXCLUDED.source_version", "POSTGRES_SYNC_VERSION_CONFLICT", "markPostgresSyncBatch"].every((marker) => postgresRuntimeSyncSource.includes(marker)) && postgresRuntimeSyncWorker.includes("runPostgresSyncWorker") && ciSource.includes("test/postgres-runtime-sync.test.js"), detail: "worker applies ordered batches idempotently with immutable batch identity, equal-version digest conflicts, stale-version protection, retries and CI contract tests" },
    ...
    { name: "database:postgresTransitionReadiness", ok: ["buildPostgresPrimaryStorageConfig", "buildTransitionAssessment", "MAX_INPUT_BYTES", "isSymbolicLink", "TOP_LEVEL_FIELDS", "SECTION_FIELDS", "readyForControlledRehearsal", "activationAuthorized: false", "productionReady: false", "productionPrimary: false", "runtimeCutoverEnabled: false"].every((marker) => postgresTransitionReadiness.includes(marker)) && Boolean(pkg.scripts?.["postgres:transition-readiness"] && pkg.scripts?.["postgres:transition-readiness:test"]) && ["POSTGRES_PRIMARY_TRANSITION_INPUT_FILE=", "POSTGRES_PRIMARY_TRANSITION_INPUT_SHA256=", "POSTGRES_PRIMARY_STORAGE_MODE=disabled", "POSTGRES_SYNC_MODE=disabled", "POSTGRES_PRIMARY_READ_MODE=disabled", "POSTGRES_ADAPTER_MODE=disabled", "POSTGRES_PRODUCTION_WRITE_MODE=disabled", "POSTGRES_SCHEMA=health_platform", "POSTGRES_RECONCILIATION_AGE_SLO_SECONDS=600"].every((marker) => productionAdaptersEnvTemplate.includes(marker)) && productionDeploymentPackage.processContract.databaseTransition?.productionReady === false && productionDeploymentPackage.processContract.databaseTransition?.productionPrimary === false && productionDeploymentPackage.processContract.databaseTransition?.runtimeCutoverEnabled === false && productionDeploymentPackage.processContract.databaseTransition?.activationAuthorized === false && productionDeploymentPackage.processContract.backgroundJobs.some((item) => item.id === "postgres-shadow-sync" && item.productionReady === false) && productionDeploymentPackage.processContract.backgroundJobs.some((item) => item.id === "postgres-shadow-reconciliation" && item.productionReady === false) && productionDeploymentPackage.secretContract.variables.some((item) => item.name === "DATABASE_URL" && !("value" in item)) && !JSON.stringify(productionDeploymentPackage.processContract).includes("DATABASE_URL") && ["metadata-only", "readyForControlledRehearsal", "productionPrimary"].every((marker) => postgresPrimaryStorageDoc.includes(marker)), detail: "bounded metadata-only seven-gate transition assessment and deployable shadow jobs remain fixed NO-GO" },
    { name: "deployment:preproductionOnsiteControl", ok: ["environment", "joint-test", "monitoring", "rehearsal", "candidate"].every((command) => productionDeploymentPackage.processContract.preproductionControls?.some((item) => item.id === command && item.readOnly === true && item.externalEvidenceRequired === true && item.executionAuthorized === false && item.productionPrimary === false && item.productionReady === false)) && ["platform:preproduction:environment", "platform:preproduction:joint-test", "platform:preproduction:monitoring", "platform:preproduction:rehearsal", "platform:preproduction:candidate", "platform:preproduction:test"].every((name) => Boolean(pkg.scripts?.[name])) && ["COMMAND_FLAGS", "OPTION_DUPLICATE", "serializeControlError", "require-ready", "require-go-candidate"].every((marker) => preproductionControlSource.includes(marker)) && ["PLATFORM_PREPRODUCTION_ENVIRONMENT_EVIDENCE_FILE=", "PLATFORM_PILOT_CUTOVER_REHEARSAL_SESSION_FILE=", "PLATFORM_PILOT_CUTOVER_AUTHORIZATION_REPORT_FILE="].every((marker) => productionAdaptersEnvTemplate.includes(marker)), detail: "five read-only onsite evaluation commands use closed arguments, deployable metadata references and fixed non-authorization flags" },
    { name: "database:postgresReconciliationCaseWorkflow", ok: ["syncPostgresReconciliationCases", "applyPostgresReconciliationCaseAction", "RECONCILIATION_CLEARANCE_REQUIRED", "auto-reopen"].every((marker) => postgresRuntimeSyncSource.includes(marker)) && ["postgres_sync_reconciliation_cases", "postgres_sync_reconciliation_case_actions", "/api/production-database/shadow-reconciliations", "/api/production-database/reconciliation-cases", "postgres-reconciliation-case-action", "/api/metrics/prometheus", "health_platform_postgres_sync_slo_breaches"].every((marker) => sqliteRuntimeSource.includes(marker)) && ["postgres-reconciliation-cases", "postgres-reconciliation-history"].every((marker) => platformHtml.includes(marker)) && ["runPostgresReconciliationCaseAction", "data-postgres-reconciliation-action"].every((marker) => platformSource.includes(marker)), detail: "payload-free differences have commission-only operational UI, ownership, verified clearance, evidence-backed resolution, immutable actions and scrapeable SLOs" },
    { name: "database:postgresReconciliationDeployment", ok: ["Type=oneshot", "NoNewPrivileges=true", "ProtectSystem=strict", "User=__SERVICE_USER__", "Group=__SERVICE_GROUP__", "WorkingDirectory=__APP_DIR__", "EnvironmentFile=__SECRET_ENV_FILE__", "__NODE_BINARY__", "__DATA_DIR__", "__LOG_DIR__"].every((marker) => postgresShadowReconcileService.includes(marker)) && ["OnUnitActiveSec=5min", "Persistent=true", "Unit=postgres-shadow-reconcile.service"].every((marker) => postgresShadowReconcileTimer.includes(marker)), detail: "hardened five-minute shadow reconciliation timer uses the unified deployment placeholders" },
    ...
    { name: "api:productionGoNoGo", ok: ["/api/production-go-no-go/center", "/api/production-go-no-go/approvals/:id/actions", "/api/production-go-no-go/decision", "production-go-no-go-decision", "assessProductionPreflightDecisionReceipt"].every((marker) => serverSource.includes(marker)) && ["evidenceFingerprint", "staleApprovals", "APPROVE PRODUCTION GO LIVE", "goNoGo:drSignoff", "goNoGo:trustedPreflightDecision"].every((marker) => productionGoNoGoSource.includes(marker)) && platformHtml.includes('data-platform-section="production-go-no-go"') && ["dataset.goNoGoApproval", "dataset.goNoGoDecision", "dataset.goNoGoDrift", "dataset.goNoGoApprovalDrift"].every((marker) => productionGoNoGoUi.includes(marker)), detail: "P0-10 global prerequisites, trusted preflight receipt, evidence fingerprint, stale approval drift, four-party approvals, command decision and trusted DOM UI are wired" },
    ...
    { name: "manifest:platformProductizationReadiness", ok: manifestSource.includes("platform-productization-readiness.json") && manifestSource.includes("productization:readiness") && manifestSource.includes("/api/platform/productization/center"), detail: "platform productization readiness artifact is indexed" },
