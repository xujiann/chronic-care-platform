...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "api:productionIdentityMessageAdapters", ok: ["fetchOidcUserInfo", "refreshOidcAccessToken", "revokeOidcToken", "fetchIdentityDirectory", "sendSmsVerificationCode", "digestPhoneVerificationCode", "identityLifecycleReady", "productionAdapterCenter"].every((marker) => productionAdaptersSource.includes(marker)) && ["/api/auth/oidc/exchange", "/api/auth/oidc/refresh", "/api/auth/oidc/revoke", "/api/auth/identity-lifecycle", "/api/auth/identity-directory/preview", "/api/auth/identity-directory/bind", "/api/auth/identity-directory/apply", "BIND EXTERNAL IDENTITY", "IDENTITY_BINDING_SUBJECT_CONFLICT", "IDENTITY_BINDING_REASSIGNMENT_BLOCKED", "IDENTITY_DIRECTORY_SELF_DEACTIVATION_BLOCKED", "IDENTITY_DIRECTORY_LAST_COMMISSION_BLOCKED", "APPLY IDENTITY DIRECTORY DEACTIVATIONS", "codeDigest"].every((marker) => serverSource.includes(marker)) && platformHtml.includes("identity-lifecycle-center") && ["renderIdentityLifecycleCenter", "runIdentityBindingAction", "runIdentityDirectoryAction"].every((marker) => platformSource.includes(marker)) && ["OIDC_TOKEN_URL=", "OIDC_REVOCATION_URL=", "IDENTITY_DIRECTORY_URL=", "IDENTITY_DIRECTORY_TOKEN="].every((marker) => envTemplateSource.includes(marker)) && ["受控本地账号绑定", "不按同名用户名自动回退", "不自动开户", "不自动提权", "不自动复活", "最终送达回调", "现场联合测试回执"].every((marker) => productionAdaptersDoc.includes(marker)), detail: "OIDC subject binding, refresh, revocation, safe SCIM deactivation, operations UI and SMS runtime are wired with explicit provider joint-test boundaries" },
    { name: "security:productionRuntimeBoundary", ok: ["LOCAL_PASSWORD_LOGIN_DISABLED", "assertProductionRuntimeSecurity", "PRODUCTION_SESSION_SECRET_INVALID", "script-src-attr 'none'", "applyResidentScope", "canManageResidentProfile"].every((marker) => serverSource.includes(marker)) && authSource.includes("认证服务暂不可用，请稍后重试") && [bloodBusinessSource, bloodRecallSource].every((source) => source.includes("escapeHtml")) && ["生产安全边界", "集中式会话"].every((marker) => productionAdaptersDoc.includes(marker)), detail: "production local passwords and weak signing secrets are blocked; browser auth, resident scoping, CSP and persistent HTML fields fail closed" },
    ...
    { name: "database:postgresTransactionalOutbox", ok: ["STORAGE_SCHEMA_VERSION = 10", "postgres_sync_outbox", "buildCollectionChanges", "enqueuePostgresSyncBatch", "readPostgresSyncStatus", "productionPrimary: false"].every((marker) => serverSource.includes(marker)) && ["buildPostgresSyncBatch", "validatePostgresSyncBatch", "previousChainHash", "chainHash"].every((marker) => postgresRuntimeSyncSource.includes(marker)), detail: "SQLite schema v10 atomically stores signed collection changes and health-visible queue state" },
