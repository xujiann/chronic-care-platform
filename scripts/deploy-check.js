...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "docs:platformDevelopmentReport20260713", ok: ["本轮主要开发成果", "安全基线入队", "PostgreSQL 只读影子核对", "验证证据", "当前生产阻断项", "下一步开发计划", "P0：0-30 天", "P1：31-60 天", "P2：61-90 天", "86/86", "311/311", "254/255", "233/233"].every((marker) => platformDevelopmentReportDoc.includes(marker)), detail: "dated platform development report records baseline bootstrap, read-only shadow reconciliation, verification evidence, blockers and 30/60/90-day plan" },
    ...
    { name: "database:postgresTransactionalOutbox", ok: ["STORAGE_SCHEMA_VERSION = 9", "postgres_sync_outbox", "buildCollectionChanges", "enqueuePostgresSyncBatch", "readPostgresSyncStatus", "productionPrimary: false"].every((marker) => serverSource.includes(marker)) && ["buildPostgresSyncBatch", "validatePostgresSyncBatch", "previousChainHash", "chainHash"].every((marker) => postgresRuntimeSyncSource.includes(marker)), detail: "SQLite schema v9 atomically stores signed collection changes and health-visible queue state" },
