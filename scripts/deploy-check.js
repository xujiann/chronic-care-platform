...
function buildDeployCheckReport(options = {}) {
  ...
  const checks = [
    ...
    { name: "snapshot:publicHealth", ok: (data.publicHealthStandards || []).length === 21 && (data.publicHealthInstitutionScopes || []).length >= 7 && (data.publicHealthEvents || []).length >= 6 && (data.publicHealthExchangeTasks || []).length >= 6 && (data.publicHealthExchangeRuns || []).length >= 6 && (data.publicHealthInstitutionTasks || []).length >= 7 && (data.publicHealthOnsiteAcceptances || []).length >= 6, detail: `${data.publicHealthStandards?.length || 0} standards, ${data.publicHealthInstitutionScopes?.length || 0} scopes, ${data.publicHealthEvents?.length || 0} events, ${data.publicHealthExchangeTasks?.length || 0} exchange tasks, ${data.publicHealthExchangeRuns?.length || 0} runs, ${data.publicHealthInstitutionTasks?.length || 0} institution tasks, ${data.publicHealthOnsiteAcceptances?.length || 0} onsite rows` },
    ...
    { name: "api:publicHealthAdvancedActions", ok: ["/api/public-health/exchange-tasks/:id/runs", "/api/public-health/institution-tasks/:id/actions", "/api/public-health/onsite-acceptances/:id/actions", "public-health-exchange-run", "public-health-institution-task-action", "public-health-onsite-acceptance"].every((marker) => serverSource.includes(marker)), detail: "public health exchange, institution and onsite action APIs are wired with audit evidence" },
    { name: "docs:publicHealth", ok: ["21/125/421", "平战结合", "医防融合", "/api/public-health/system", "/api/public-health/events/:id/actions", "/api/public-health/exchange-tasks/:id/runs", "publicHealthExchangeRuns", "publicHealthInstitutionTasks", "publicHealthOnsiteAcceptances", "public-health:readiness"].every((marker) => publicHealthDoc.includes(marker)), detail: "public health system report documents source-derived scope, action APIs, exchange runs, institution collaboration, onsite acceptance, and release evidence" },
