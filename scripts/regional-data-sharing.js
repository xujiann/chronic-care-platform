...
function buildRegionalDataSharingReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "regional:siteIntegrationUi", passed: /data-regional-section="site-integration"/.test(html) && /regional-site-integration/.test(html) && /renderRegionalSiteIntegration/.test(client) && /身份与权限/.test(client) && /监控灾备/.test(client) && /联调样例/.test(client) && /验收证据/.test(client) && /下一步/.test(client), detail: "site integration checklist present" },
    { id: "regional:functionRoadmapUi", passed: /data-regional-section="function-roadmap"/.test(html) && /regional-function-roadmap/.test(html) && /renderRegionalFunctionRoadmap/.test(client) && /优先级排序：P0 共享包编目、角色权限裁剪；P1 联调检查与交接清单；P2 调阅审计与发布证据/.test(client) && /责任部门/.test(client) && /下一步计划开发功能/.test(client) && /P0 上线前必须/.test(client) && /P1 试点联调/.test(client) && /P2 生产加固/.test(client) && /市级平台数据治理组/.test(client) && /接口联调组/.test(client), detail: "main page function owners, priorities and planned features present" },
    ...
    { id: "regional:aboutFunctionRoadmap", passed: /data-regional-about-section="function-roadmap"/.test(about) && /现有功能及责任部门/.test(about) && /优先级排序：P0 共享包编目、角色权限裁剪；P1 联调检查与交接清单；P2 调阅审计与发布证据/.test(about) && /展示编目完整度/.test(about) && /下一步计划开发功能/.test(about) && /优先级：P0 上线前必须/.test(about) && /优先级：P1 试点联调/.test(about) && /优先级：P2 生产加固/.test(about) && /市级平台数据治理组/.test(about) && /安全管理岗/.test(about), detail: "implemented functions, owners, priorities and planned features documented" },
