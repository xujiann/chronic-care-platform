...
function buildRegionalDataSharingReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "regional:functionRoadmapUi", passed: /data-regional-section="function-roadmap"/.test(html) && /regional-function-roadmap/.test(html) && /renderRegionalFunctionRoadmap/.test(client) && /责任部门/.test(client) && /下一步计划开发功能/.test(client) && /P0 上线前必须/.test(client) && /P1 试点联调/.test(client) && /P2 生产加固/.test(client) && /市级平台数据治理组/.test(client) && /接口联调组/.test(client), detail: "main page function owners, priorities and planned features present" },
