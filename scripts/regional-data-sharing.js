...
function buildRegionalDataSharingReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "regional:siteIntegrationUi", passed: /data-regional-section="site-integration"/.test(html) && /regional-site-integration/.test(html) && /renderRegionalSiteIntegration/.test(client) && /身份与权限/.test(client) && /监控灾备/.test(client), detail: "site integration checklist present" },
