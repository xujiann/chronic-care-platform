#!/usr/bin/env node
const assert = require("node:assert/strict");
const {
  MODULE_ID,
  digest,
  validateBagId,
  validateMasterData,
  validateReceipt,
  validateColdChainEvidence,
  validateAcceptanceScenario,
  evaluateProductionReadiness
} = require("../blood-clinical-production");

const bagId = "A123456789012-RBCS-01";
const sha = digest({ run: "shared-runtime-smoke" });
assert.equal(validateBagId(bagId).valid, true);
assert.equal(validateMasterData({ organizationCode: "913301", system: "BIS", code: "RBCS", name: "红细胞", version: "1", effectiveAt: new Date().toISOString() }).valid, true);
assert.equal(validateReceipt({ evidenceRef: "site://receipt/1", evidenceDigest: sha, organizationCode: "913301", signedBy: "site-owner", signedAt: new Date().toISOString(), verifiedBy: "independent-reviewer", verifiedAt: new Date().toISOString(), correlationId: "corr-1", idempotencyKey: "idem-1" }).valid, true);
assert.equal(validateColdChainEvidence({ deviceId: "FRIDGE-01", serialNumber: "SN01", calibrationCertificateRef: "site://cal/1", calibrationDigest: sha, calibratedAt: "2026-01-01", calibrationExpiresAt: "2027-01-01", alarmTestResult: "passed", alarmEvidenceRef: "site://alarm/1", performedBy: "engineer-a", verifiedBy: "engineer-b", componentCode: "RBC", phase: "storage", minimumTemperature: 2.8, maximumTemperature: 5.4, sampleCount: 1440 }, new Date("2026-07-24")).valid, true);
assert.equal(validateAcceptanceScenario({ scenarioId: "S1", type: "crossmatch-operation-review", patientId: "P1", bagId, evidenceRef: "site://scenario/1", performedBy: "A", reviewedBy: "B", releasedBy: "C", performedAt: "2026-07-24T01:00:00Z", reviewedAt: "2026-07-24T01:10:00Z", releasedAt: "2026-07-24T01:20:00Z", result: "compatible" }).valid, true);
const readiness = evaluateProductionReadiness({});
assert.equal(readiness.standalone, false);
assert.equal(readiness.deploymentMode, "shared-platform-node-runtime");
assert.equal(readiness.independentDeploymentAuthorized, false);
assert.equal(readiness.productionReady, false);
assert.equal(readiness.formalGoLiveState, "NO-GO");
process.stdout.write(`${JSON.stringify({
  moduleId: MODULE_ID,
  result: "passed",
  deploymentMode: readiness.deploymentMode,
  independentDeploymentAuthorized: readiness.independentDeploymentAuthorized,
  formalGoLiveState: readiness.formalGoLiveState
})}\n`);
