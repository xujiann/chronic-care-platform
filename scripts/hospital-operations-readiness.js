...
function buildHospitalOperationsReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "hospitalOps:goLiveGates", passed: /buildOperationsGoLiveGates/.test(serverSource) && /\/api\/operations\/go-live-gates/.test(serverSource) && /\/api\/operations\/go-live-gates\/actions/.test(serverSource) && /operation-go-live-gates/.test(operationsHtml) && /renderGoLiveGates/.test(operationsJs) && /reviewGoLiveGate/.test(operationsJs), detail: "go-live gate checklist API, review action, and panel cover payload, audit, monitoring, and DR evidence" },
