...
function renderMarkdown(report) {
  return [
    ...
    "## Joint-test samples",
    "",
    "| Interface | Owner | Method | Endpoint | Idempotency | Retry | Status |",
    "|---|---|---|---|---|---|---|",
    ...report.interfaceSamples.map((item) => `| ${clean(item.name)} | ${clean(item.owner)} | ${item.method} | ${item.endpoint} | ${item.idempotencyKey} | ${item.retryPolicy} | ${item.status} |`),
    "",
