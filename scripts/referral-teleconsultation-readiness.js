...
function buildReferralTeleconsultationReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "referral:jointTestPack", passed: /joint-test-pack/.test(server) && /buildReferralTeleconsultationJointTestPack/.test(server) && /taskReceipts/.test(server) && /exportSummary/.test(server), detail: "runtime joint-test pack exposes callback samples, checklist, task receipts, export summary, and signoff roles" },
