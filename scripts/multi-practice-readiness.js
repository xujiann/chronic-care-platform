...
function buildMultiPracticeReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "multiPractice:doctorAccountGuard", passed: hasAll(doctorPortal, ["requireRole([\"institution\"])", "requireAccountType([\"doctor\"])"]) && hasAll(readText("auth.js"), ["function requireAccountType", "normalizeAccountType(user)"]) && accessPolicy.includes('"doctor.html": entry("医生工作站", ["institution"], { group: "机构工作", accountTypes: ["doctor"]'), detail: "doctor workbench is limited to institution-domain doctor accounts through the central policy" },
