...
function buildMultiPracticeReadinessReport(options = {}) {
  ...
  const accessPolicy = options.accessPolicy ?? readText("access-control-policy.js");
  ...
  const confirmationRequired = applications.filter((item) => !/已建档|草稿|draft/i.test(String(item.status || "")));
  const confirmationPendingDrafts = applications.filter((item) => !confirmationRequired.includes(item) && !item.primaryPracticeConfirmation?.signatureNo);
  ...
  const checks = [
    ...
    { id: "multiPractice:firstPracticeConfirmation", passed: confirmationRequired.every((item) => item.primaryPracticeConfirmation?.status && item.primaryPracticeConfirmation?.signatureNo && item.documentChecks?.firstPracticeConsent === true) && confirmationPendingDrafts.every((item) => (item.riskFlags || []).includes("first-practice-consent-missing")), detail: `${confirmationRequired.length} submitted applications signed / ${confirmationPendingDrafts.length} draft gaps retained in risk queue` },
    ...
    { id: "multiPractice:doctorAccountGuard", passed: hasAll(doctorPortal, ["src=\"./page-auth-bootstrap.js\" data-roles=\"institution\" data-account-types=\"doctor\""]) && hasAll(readText("auth.js"), ["function requireAccountType", "normalizeAccountType(user)"]) && accessPolicy.includes('"doctor.html": entry("医生工作站", ["institution"], { group: "机构工作", parent: "institution.html", accountTypes: ["doctor"]'), detail: "doctor workbench is limited to institution-domain doctor accounts through the central policy" },
    ...
    { id: "multiPractice:doctorReceiptMessages", passed: hasAll(doctorPortal, ["multiPracticeMessages", "sourceId", "医院消息"]) && hasAll(server, ["targetRole", "doctor", "多点执业医院端已处理"]), detail: "doctor portal renders scoped hospital receipts for multi-practice applications" },
    ...
    { id: "multiPractice:lifecycleActions", passed: hasAll(server, ["resolveMultiPracticeLifecyclePatch", "return-correction", "suspend", "withdraw", "terminate"]) && hasAll(institution, ["退回补正", "暂停执业", "撤回申请", "终止备案"]), detail: "return, suspend, withdraw, and terminate actions are available through workflow-actions" },
    { id: "multiPractice:auditEvidence", passed: hasAll(server, ["securityEvents", "/api/audit/export", "multiPracticeApplications"]) && hasAll(functionReport, ["workflow-actions", "审计导出"]), detail: "workflow-actions write security audit evidence and docs cover audit export" },
    { id: "multiPractice:publicQuery", passed: hasAll(server, ["/api/public/multi-practice-ledger", "doctorName", "institution", "publicLedger"]), detail: "public multi-practice ledger can be queried without exposing management fields" },
    { id: "multiPractice:externalSync", passed: hasAll(server, ["buildMultiPracticeExternalSync", "electronicRegistration", "eSignature", "hisHr"]) && hasAll(institution, ["外部同步", "HIS/HR"]), detail: "electronic registration, e-signature, and HIS/HR sync evidence are carried in responses" },
  ...
  return {
    ...
    summary: {
      ...
      messageLoopReady: hasAll(server, ["buildMultiPracticeTaskMessage", "multiPracticeMessages"]),
      scheduleConflictReady: hasAll(server, ["detectMultiPracticeScheduleConflicts", "scheduleConflictEvidence"]),
      publicLedgerReady: hasAll(server, ["/api/public/multi-practice-ledger"]),
      externalSyncReady: hasAll(server, ["buildMultiPracticeExternalSync", "hisHr"]),
      auditEvidenceReady: hasAll(server, ["securityEvents", "/api/audit/export", "multiPracticeApplications"]),
...
function renderMarkdown(report) {
  return [
    ...
    `- Doctor-hospital message loop: ${report.summary.messageLoopReady ? "ready" : "missing"}`,
    `- Schedule conflict detection: ${report.summary.scheduleConflictReady ? "ready" : "missing"}`,
    `- Public ledger query: ${report.summary.publicLedgerReady ? "ready" : "missing"}`,
    `- External sync evidence: ${report.summary.externalSyncReady ? "ready" : "missing"}`,
    `- Workflow audit evidence: ${report.summary.auditEvidenceReady ? "ready" : "missing"}`,
