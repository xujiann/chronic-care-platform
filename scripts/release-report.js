...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:modernizationRoutes", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-routes" && item.passed), "commission-only data foundation, surveillance and medical-prevention routes use server actor, time and idempotency context", "error", "public-health"),
    check("publicHealthFinal:modernizationPersistence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-persistence" && item.passed), "nine modernization collections use nextData-only SQLite transaction CAS and unique source/idempotency hashes", "error", "public-health"),
    check("publicHealthFinal:modernizationUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-ui-release" && item.passed), "three redacted executable public-health workbenches are registered without granting production readiness", "error", "public-health"),
    check("publicHealthFinal:campaignContinuityGap", publicHealthFinalReadiness.summary?.t08FunctionalChecks === 53 && publicHealthFinalReadiness.summary?.t08FunctionalPassed === 53 && publicHealthFinalReadiness.summary?.t00BoundaryChecks === 18 && publicHealthFinalReadiness.summary?.t00BoundaryPassed === 18 && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.continuousConnectivityReady === false && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.continuityBreak?.code === "campaign-verification-failed" && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.productionReady === false, "53 T08 functional checks include modernization and fail-closed continuity while 18 T00 public boundary checks remain separately visible", "error", "public-health"),
