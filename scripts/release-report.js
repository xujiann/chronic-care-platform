...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:endpointProbeCampaign", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-endpoint-probe-campaign" && item.passed) && publicHealthFinalReadiness.endpointProbeCampaignRegistry?.continuousConnectivityReady === true && publicHealthFinalReadiness.endpointProbeCampaignRegistry?.productionReady === false, "three independent signed eight-lane campaigns prove continuity without asserting production readiness", "error", "public-health"),
    check("publicHealthFinal:endpointConnectivityUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-endpoint-connectivity-ui" && item.passed), "commission UI exposes only redacted endpoint and continuity summaries with fail-closed states and server-owned production authorization", "error", "public-health"),
    check("publicHealthFinal:campaignContinuityGap", publicHealthFinalReadiness.summary?.t08FunctionalChecks === 49 && publicHealthFinalReadiness.summary?.t08FunctionalPassed === 49 && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.continuousConnectivityReady === false && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.continuityBreak?.code === "campaign-verification-failed" && publicHealthFinalReadiness.endpointProbeCampaignFailureRegistry?.productionReady === false, "49 T08 functional checks include a fail-closed rejected campaign window while 15 T00 public boundary checks remain separately visible", "error", "public-health"),
