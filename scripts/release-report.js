...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:modernizationRoutes", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-routes" && item.passed), "commission-only data foundation, shadow-model governance, surveillance and medical-prevention routes use server actor, time and idempotency context", "error", "public-health"),
    check("publicHealthFinal:modernizationPersistence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-persistence" && item.passed), "thirteen modernization collections use nextData-only SQLite transaction CAS and unique source/idempotency hashes", "error", "public-health"),
    check("publicHealthFinal:modernizationUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-ui-release" && item.passed), "six redacted executable public-health workbenches are registered without granting production readiness", "error", "public-health"),
