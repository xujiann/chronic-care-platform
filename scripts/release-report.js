...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:endpointConnectivityUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-endpoint-connectivity-ui" && item.passed), "commission UI exposes redacted summaries and controlled lane/campaign actions with fail-closed states, server-owned security inputs and server-owned production authorization", "error", "public-health"),
