...
function healthDashboardChecks(healthDashboard) {
  ...
  return [
    ...
    check("healthDashboard:indicatorCenter", indicatorRows >= 8 && indicatorDimensions >= 7 && indicatorCategories === 7 && indicatorEntrypoints === 3 && (healthDashboard.indicatorCenter?.indicators || []).every((item) => item.definition && item.source && item.owner && item.reformCategory && item.drilldown?.href), `${indicatorRows} indicators, ${indicatorDimensions} dimensions, ${indicatorCategories} categories, ${indicatorEntrypoints} entrypoints`, "error", "health-dashboard"),
