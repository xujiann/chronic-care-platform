...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:modernizationRoutes", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-routes" && item.passed), "commission-only data foundation, shadow-model, aggregate respiratory-pathogen, respiratory-network evidence, surveillance and medical-prevention routes use server actor, time and idempotency context", "error", "public-health"),
    check("publicHealthFinal:modernizationPersistence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-persistence" && item.passed), "twenty modernization collections use SQLite transaction CAS, including atomic respiratory publication, signed network evidence and dual-control lifecycle audit", "error", "public-health"),
    check("publicHealthFinal:modernizationUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-ui-release" && item.passed), "eight redacted executable public-health workbenches are registered without granting production readiness", "error", "public-health"),
