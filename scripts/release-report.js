...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:modernizationPersistence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-persistence" && item.passed), "seventeen modernization collections use SQLite transaction CAS, including atomic respiratory publication and signed network-evidence audit", "error", "public-health"),
    ...
    check("publicHealthFinal:respiratoryNetworkEvidence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:respiratory-network-evidence-documented" && item.passed) && publicHealthFinalReadiness.modernizationReadiness?.respiratoryNetworkReadiness?.technicalLaunchReady === true && publicHealthFinalReadiness.modernizationReadiness?.respiratoryNetworkReadiness?.productionReady === false, "six trusted evidence tracks and three quality days per acceptance sentinel prove technical readiness while central production gates remain closed", "error", "public-health"),
