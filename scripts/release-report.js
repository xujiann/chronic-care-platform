...
function validateProductionConfig(options = {}) {
  ...
  if (strict) {
    ...
    checks.push(
      ...
      check("env:OBJECT_STORAGE.adapter", secureObjectStorage.adapterReady, secureObjectStorage.adapterReady ? "object storage gateway trust v1 adapter is configured" : secureObjectStorage.blockers.join("; "), "error", "environment"),
      ...
      check("env:OBJECT_STORAGE.trustContract", secureObjectStorage.configuration.trustContractReady, secureObjectStorage.configuration.trustContractReady ? "response signature, exact-origin URL and explicit receipt contract configured" : "object-storage-gateway-trust-v1 is incomplete", "error", "environment"),
      check("env:OBJECT_STORAGE.receiptSecretQuality", secretQuality(env.OBJECT_STORAGE_RECEIPT_SIGNING_SECRET).strongEnough, "object storage receipt signing secret must be non-placeholder and at least 32 chars", "error", "environment"),
      check("env:OBJECT_STORAGE.directionKeys", secureObjectStorage.configuration.distinctDirectionKeys, "request and response signing keys must be distinct", "error", "environment"),
...
function regionalProductLineChecks(regionalConfigurationReadiness, regionalSiteEvidenceReadiness, regionalCutoverDossier) {
  ...
  return [
    check(
      ...
      regionalConfigurationReadiness.ok
        && regionalConfigurationReadiness.summary?.technicalReady === regionalConfigurationReadiness.summary?.regions,
      `${regionalConfigurationReadiness.summary?.technicalReady || 0}/${regionalConfigurationReadiness.summary?.regions || 0} regional configurations technically ready`,
      "error",
    ...
    check(
      ...
      regionalCutoverDossier.ok
        && regionalCutoverDossier.summary?.localControlReady === regionalCutoverDossier.summary?.regions,
      `${regionalCutoverDossier.summary?.localControlReady || 0}/${regionalCutoverDossier.summary?.regions || 0} regional cutover dossiers locally controlled`,
      "error",
      "regional-product-line"
    ),
    check(
      ...
      dossierBoundary,
      `${regionalCutoverDossier.summary?.candidateReady || 0} candidate-ready; external authorization is still required`,
      "error",
      "regional-product-line"
    )
  ];
...
function publicHealthFinalReadinessChecks(publicHealthFinalReadiness) {
  return [
    ...
    check("publicHealthFinal:modernizationRoutes", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-routes" && item.passed), "commission-only workbenches and authenticated server-only official exchange callbacks use server actor, time, managed trust and idempotency context", "error", "public-health"),
    check("publicHealthFinal:modernizationPersistence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-persistence" && item.passed), "twenty-two modernization collections use SQLite transaction CAS, including atomic respiratory and official receipt-alert-audit boundaries", "error", "public-health"),
    check("publicHealthFinal:modernizationUi", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:t00-modernization-ui-release" && item.passed), "redacted executable public-health workbenches include the trusted official report-feedback chain without granting production readiness", "error", "public-health"),
    check("publicHealthFinal:respiratoryNetworkEvidence", publicHealthFinalReadiness.checks?.some((item) => item.id === "integration:respiratory-network-evidence-documented" && item.passed) && publicHealthFinalReadiness.checks?.some((item) => item.id === "modernization:respiratory-network-evidence-lifecycle" && item.passed) && publicHealthFinalReadiness.checks?.some((item) => item.id === "modernization:respiratory-network-lifecycle-boundary" && item.passed) && publicHealthFinalReadiness.modernizationReadiness?.respiratoryNetworkReadiness?.technicalLaunchReady === true && publicHealthFinalReadiness.modernizationReadiness?.respiratoryNetworkReadiness?.productionReady === false, "six trusted evidence tracks, three quality days, signed lifecycle events and the 30-day renewal gate prove technical readiness while central production gates remain closed", "error", "public-health"),
...
function productionDbReadinessChecks(productionDbReadiness) {
  return [
    ...
    check("productionDb:sqliteMigrationRegistry", productionDbReadiness.checks?.some((item) => item.id === "production-db:sqliteMigrationRegistry" && item.passed) && Number.isInteger(productionDbReadiness.sqliteMigrationRegistry?.head) && /^[a-f0-9]{64}$/.test(productionDbReadiness.sqliteMigrationRegistry?.registryFingerprint || ""), `SQLite v1-v${productionDbReadiness.sqliteMigrationRegistry?.head || 0} registry is continuous and content-fingerprinted`, "error", "production-db"),
    ...
    check("productionDb:transactionalOutbox", productionDbReadiness.postgresRuntimeSync?.transactionalOutbox && productionDbReadiness.postgresRuntimeSync?.batchIntegrity && productionDbReadiness.postgresRuntimeSync?.healthStatus, `SQLite schema v${productionDbReadiness.sqliteMigrationRegistry?.head || 0} retains atomic signed PostgreSQL change batches and health status`, "error", "production-db"),
    ...
    check("productionDb:primaryStorageContract", productionDbReadiness.checks?.some((item) => item.id === "production-db:primaryStorageContract" && item.passed), "formal PostgreSQL storage modes, committed-outbox boundary, serializable CAS driver and schema are present", "error", "production-db"),
    check("productionDb:primaryStorageBoundary", productionDbReadiness.postgresPrimaryStorage?.modeReady
      && productionDbReadiness.postgresPrimaryStorage?.capabilities?.requestPathWrite === false
      && productionDbReadiness.postgresPrimaryStorage?.productionPrimary === false
      && productionDbReadiness.postgresPrimaryStorage?.runtimeCutoverEnabled === false,
    `mode=${productionDbReadiness.postgresPrimaryStorage?.mode || "disabled"}; request-path writes blocked; production primary false`, "error", "production-db"),
...
function buildReleaseReport(options = {}) {
  ...
  const checks = [
    ...
    check("specialtyCutover:clinicalBloodSharedRuntimeGate", t10ClinicalBloodReadiness.standalone === false && t10ClinicalBloodReadiness.deploymentMode === "shared-platform-node-runtime" && t10ClinicalBloodReadiness.independentDeploymentAuthorized === false && t10ClinicalBloodReadiness.productionReady === false && (t10ClinicalBloodReadiness.blockers?.length || 0) === 8, `${t10ClinicalBloodReadiness.blockers?.length || 0} clinical-blood local/platform blockers; shared runtime and platform gate closed`, "error", "cutover"),
