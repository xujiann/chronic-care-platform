...
function buildCitizenLaunchFoundationReadiness(options = {}) {
  ...
  const checks = [
    ...
    },
    {
      ...
      passed: hasAll(citizenJs + mobilePreview, [/citizenClientChannels/, /mini-program/, /clientChannelEntry/, /copyClientEntry/, /launchChecklist/, /productionMaterials/, /client-production-materials/, /SMS_GATEWAY_URL/, /OIDC/, /previewParams\.get\("client"\)/]),
      detail: "mini-program and app channel routing, copyable entry links, production-material prompts, and preview query handling are present"
    },
    {
      id: "citizen-foundation:mobile-preview-service-switch",
      passed: hasAll(mobilePreview, [/preview-service-select/, /preview-service-stepper/, /preview-prev-service/, /preview-next-service/, /preview-service-position/, /previewServicePosition\.setAttribute\("aria-label"/, /previewPrevService\.title = previousService/, /previewNextService\.title = nextService/, /preview-focus-mode/, /进入手机验收模式/, /preview-swipe-hint/, /滑动切换服务/, /preview-readiness-summary/, /preview-readiness-method/, /preview-priority/, /下一步优先级/, /P0/, /P1/, /P2/, /P3/, /验收摘要/, /验收方式：点击、方向键、滑动、复制入口/, /alignFocusPreview/, /window\.scrollTo\(\{ top: Math\.max\(0, targetTop\), left: 0, behavior: "auto" \}\)/, /handlePreviewSwipe/, /bindPreviewSwipeTarget/, /frame\.contentDocument/, /touchstart/, /touchend/, /data-preview-service="escort"/, /data-preview-service="registration"/, /citizenPreviewSrc\(service\)/]) &&
        hasAll(mobilePreviewCss, [/body\.preview-focus-mode \.preview-shell/, /body\.preview-focus-mode \.preview-copy/, /body\.preview-focus-mode \.preview-device/, /body\.preview-focus-mode \.phone-frame/, /preview-swipe-hint/, /#preview-readiness-summary/, /#preview-readiness-method/, /preview-priority/, /scroll-margin-top: 12px/, /touch-action: pan-y/]) &&
