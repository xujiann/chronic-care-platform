...
function buildEscortServiceReadinessReport(options = {}) {
  ...
  const writePathSource = options.writePathSource ?? readText("escort-service.js");
  ...
  const checks = [
    ...
    { id: "escort:providerAdmissionCatalog", passed: providers.every((item) => Array.isArray(item.serviceItems) && item.serviceItems.length > 0 && item.admissionReview?.policyVersion === "escort-provider-admission-v1") && providers.filter((item) => item.published).every((item) => item.admissionReview?.status === "approved" && Number.isFinite(Date.parse(item.admissionReview?.validUntil))), detail: "provider publication is bound to a policy-versioned admission review and explicit service catalog" },
    ...
    { id: "escort:guardedWritePath", passed: /createEscortOrder/.test(writePathSource) && /ESCORT_DELEGATION_EVIDENCE_REQUIRED/.test(writePathSource) && /validateProviderAndCatalog/.test(writePathSource) && /ESCORT_EVIDENCE_BYPASS_FORBIDDEN/.test(writePathSource) && /transitionEscortOrder/.test(writePathSource) && /canAccessOrder/.test(writePathSource), detail: "resident scope, family authorization, provider catalog, access decisions, evidence gates, and command idempotency are enforced by the escort write adapter" },
    { id: "escort:eventOutbox", passed: /escortServiceOutbox/.test(writePathSource) && /escort-event-outbox-v1/.test(writePathSource) && /validateOutboxEvent/.test(writePathSource) && /recordEscortNotificationReceipt/.test(writePathSource) && /payloadDigest/.test(writePathSource), detail: "order mutations and notification receipts produce integrity-checked atomic outbox events" },
    { id: "escort:reliableRuntime", passed: policy.careServiceRuntimeVersion === "care-service-runtime-v1" && /executeTransactionalCommand/.test(runtimeSource) && /claimOutboxEvents/.test(runtimeSource) && /leaseExpiresAt/.test(runtimeSource) && /retryDelaySeconds/.test(runtimeSource) && /dead-letter/.test(runtimeSource) && /requeueDeadLetter/.test(runtimeSource) && /buildOutboxHealth/.test(runtimeSource) && /可靠投递与死信补偿/.test(frontend) && /care-service-production-readiness\.js/.test(responsibilityDoc), detail: "transaction execution, exclusive leases, bounded retry, dead-letter compensation and queue health are release-gated" },
    ...
    { id: "escort:developmentReport", passed: /陪诊信息平台研发报告/.test(developmentReport) && /政策文件依据/.test(developmentReport) && /已实现功能/.test(developmentReport) && /下一步开发计划/.test(developmentReport) && /沪民养老发〔2025〕2 号/.test(developmentReport) && /中共中央 国务院关于深化养老服务改革发展的意见/.test(developmentReport) && /个人信息保护法/.test(developmentReport) && /居民端陪诊预约/.test(developmentReport) && /HIS\/导诊台/.test(developmentReport) && /npm\.cmd run escort:readiness/.test(developmentReport), detail: "policy basis, implemented functions, next development plan, and verification commands are consolidated into the escort R&D report" },
    ...
    { id: "escort:writePathAcceptance", passed: /data-escort-write-path-acceptance/.test(frontend) && /服务商准入与目录/.test(frontend) && /幂等建单与事务外盒/.test(frontend) && /证据化状态迁移/.test(frontend), detail: "guarded escort write-path acceptance boundary is visible to operators" },
