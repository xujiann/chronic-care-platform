...
function buildEscortServiceReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    { id: "escort:appointmentFieldGuard", passed: /hospital is required/.test(server) && /appointmentAt is required/.test(server) && /appointmentAt cannot be in the past/.test(server) && /isPastEscortAppointmentDate/.test(server), detail: "resident escort requests require hospital and non-past appointment date before dispatch" },
