...
function writeOutput(center, options = {}) {
  const output = path.resolve(ROOT, options.output || "release/research-expert-consultation.json");
  const markdown = path.resolve(ROOT, options.markdown || "release/research-expert-consultation.md");
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.mkdirSync(path.dirname(markdown), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify(center, null, 2)}\n`, "utf8");
  fs.writeFileSync(markdown, renderExpertConsultationMarkdown(center), "utf8");
...
function main() {
  ...
  const files = writeOutput(center, parseArgs());
  process.stdout.write(`${JSON.stringify({ ok: center.ok, summary: center.summary, aggregate: center.aggregate, files }, null, 2)}\n`);
}
