...
function buildPublicHealthFinalReadiness(options = {}) {
  ...
  const workerSource = options.workerSource ?? fs.readFileSync(path.join(ROOT, "public-health-external-worker.js"), "utf8");
  ...
  const checks = [
    ...
    check("modernization:data-source-operations", modernizationReadiness.summary.freshSources === 2 && modernizationReadiness.summary.noDataSources === 6 && modernizationReadiness.dataSourceOperations.productionReady === false, "source operations distinguishes observed clinical and laboratory freshness from six registered sources without data", "modernization"),
    ...
    check("modernization:respiratory-pathogen-network", modernizationReadiness.summary.respiratoryCatalogPathogens === 18 && modernizationReadiness.summary.respiratoryObservedPathogens === 18, "the respiratory sentinel catalog and observed panel cover 18 pathogens", "modernization"),
    ...
    check("modernization:respiratory-network-evidence-lifecycle", modernizationReadiness.summary.respiratoryNetworkLifecycleEvents === 1 && modernizationReadiness.summary.respiratoryNetworkSupersededEvidence === 1 && modernizationReadiness.summary.respiratoryNetworkRenewalDueEvidence === 0, "one independently approved lifecycle event renews a trusted track without active duplicates or renewal debt", "modernization"),
    check("modernization:respiratory-network-lifecycle-boundary", modernizationReadiness.respiratoryNetworkReadiness.evidenceLifecycle.ok === true && modernizationReadiness.respiratoryNetworkReadiness.evidenceLifecycle.productionReady === false && modernizationReadiness.respiratoryNetworkReadiness.summary.minimumEvidenceValidityDaysAtLaunch === 30, "append-only lifecycle integrity and the 30-day validity window remain fail-closed and cannot assert production readiness", "modernization"),
    ...
    check("modernization:official-exchange-receipt-chain", modernizationReadiness.summary.trustedOfficialReports === 1 && modernizationReadiness.summary.trustedOfficialFeedbacks === 1 && modernizationReadiness.summary.officialExchangeReceiptFindings === 0 && modernizationReadiness.summary.officialExchangeReceiptKeyringReady === true, "official report and feedback are accepted only through a verified purpose-bound predecessor receipt chain", "modernization"),
    check("modernization:official-exchange-production-boundary", modernizationReadiness.surveillanceCenter.officialExchangeReceipts.productionReady === false && modernizationReadiness.surveillanceCenter.productionReady === false && modernizationReadiness.checks.find((item) => item.id === "safety:official-receipts-not-production")?.passed === true, "trusted exchange receipts prove business delivery without asserting production readiness", "modernization"),
    ...
    check("integration:t00-public-routes", [
      ...
      "/api/public-health/external/outbox/due",
      "publicHealthExternalClaimMatch",
    ...
    check("integration:t00-contract-chain-persistence", ["assertPublicHealthContractAttestationChain", "persistedAttestations", "incomingAttestations", "contract-transition-conflict"].every((token) => contractIntegrationSource.includes(token)) && ["assertPublicHealthContractAttestationInsert", "publicHealthExternalContractInsert", "signingMaterial: context.signingMaterial"].every((token) => serverSource.includes(token)), "the durable writer locks an append-only lane chain and verifies every signed predecessor in the attestation transaction", "integration"),
    check("integration:t00-endpoint-verification", [
      ...
      "/api/public-health/external/endpoints/summary",
      "/api/public-health/external/endpoints/receipts"
    ].every((token) => serverSource.includes(token)) && ["expectedEndpoint", "expectedContract", "seenReceiptIds", "seenNonces"].every((token) => endpointProbeSource.includes(token)), "commission-only redacted endpoint summaries and trusted receipts use server configuration with durable receiptId and nonce uniqueness", "integration"),
    check("integration:t00-active-endpoint-probe", [
    ...
    check("integration:t00-modernization-routes", [
      ...
      "publicHealthSafeOfficialExchangeReceiptRegistry",
    ...
    ].every((token) => serverSource.includes(token)), "all twenty-two modernization collections use SQLite transaction CAS; respiratory publication, signed network evidence, dual-control lifecycle events and official exchange receipt/alert/audit updates each commit their complete boundary atomically", "integration"),
    check("integration:t00-modernization-ui-release", [
      ...
      "test/public-health-official-exchange-api.test.js",
      "test/public-health-official-exchange-receipt-service.test.js"
    ].every((token) => packageSource.includes(token)), "responsive workbenches expose only minimized summaries, advisory shadow output, aggregate respiratory counts, six-track network readiness, dual-control lifecycle actions and a redacted trusted official exchange chain while release scripts preserve productionReady=false", "integration"),
    ...
    check("integration:respiratory-network-evidence-documented", ["public-health-respiratory-network-readiness-service.js", "technicalLaunchReady=true", "public-health-respiratory-network-evidence", "/api/public-health/respiratory-network-readiness", "supersede", "renewalDueEvidence", "两阶段受控动作"].every((token) => modernizationDoc.includes(token)), "respiratory network evidence service, purpose-bound keyring, dual-control lifecycle, renewal window, public read route and production boundary are documented", "integration"),
  ...
  return {
    ...
    formalGoLiveState: "blocked-until-production-key-service-endpoints-and-trusted-site-evidence-verified",
    summary: {
      ...
      modernizationRespiratoryCatalogPathogens: modernizationReadiness.summary.respiratoryCatalogPathogens,
      modernizationRespiratoryObservedPathogens: modernizationReadiness.summary.respiratoryObservedPathogens,
      modernizationRespiratoryBatches: modernizationReadiness.summary.respiratoryBatches,
      ...
      modernizationRespiratoryNetworkLifecycleEvents: modernizationReadiness.summary.respiratoryNetworkLifecycleEvents,
      modernizationRespiratoryNetworkSupersededEvidence: modernizationReadiness.summary.respiratoryNetworkSupersededEvidence,
      modernizationRespiratoryNetworkRenewalDueEvidence: modernizationReadiness.summary.respiratoryNetworkRenewalDueEvidence,
      ...
      modernizationTrustedOfficialReports: modernizationReadiness.summary.trustedOfficialReports,
      modernizationTrustedOfficialFeedbacks: modernizationReadiness.summary.trustedOfficialFeedbacks,
      modernizationOfficialExchangeReceiptFindings: modernizationReadiness.summary.officialExchangeReceiptFindings,
      modernizationOfficialExchangeReceiptKeyringReady: modernizationReadiness.summary.officialExchangeReceiptKeyringReady,
    ...
    artifacts: {
      ...
      publicHealthOfficialExchangeReceipts: "public-health-official-exchange-receipt-service.js",
...
function renderMarkdown(report) {
  return [
    ...
    `- Modernization respiratory pathogens catalogued/observed: ${report.summary.modernizationRespiratoryCatalogPathogens}/${report.summary.modernizationRespiratoryObservedPathogens}`,
    ...
    `- Modernization respiratory network lifecycle events: ${report.summary.modernizationRespiratoryNetworkLifecycleEvents}`,
    `- Modernization respiratory network superseded/renewal due evidence: ${report.summary.modernizationRespiratoryNetworkSupersededEvidence}/${report.summary.modernizationRespiratoryNetworkRenewalDueEvidence}`,
    ...
    `- Modernization trusted official report/feedback receipts: ${report.summary.modernizationTrustedOfficialReports}/${report.summary.modernizationTrustedOfficialFeedbacks}`,
    `- Modernization official exchange receipt findings: ${report.summary.modernizationOfficialExchangeReceiptFindings}`,
    `- Modernization official exchange receipt keyring ready: ${report.summary.modernizationOfficialExchangeReceiptKeyringReady ? "yes" : "no"}`,
