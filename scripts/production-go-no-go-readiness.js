...
function buildProductionGoNoGoReadiness(options = {}) {
  ...
  const checks = [
    ...
    check("goNoGoReadiness:ui", html.includes('data-platform-section="production-go-no-go"') && html.includes("production-go-no-go-ui.js") && ui.includes("data-go-no-go-approval") && ui.includes("data-approval-role") && ui.includes("dataset.approvalRole") && !ui.includes('data-role="${escapeHtml(item.role)}"') && ui.includes("data-go-no-go-decision"), "platform command workbench wired with business responsibility isolated from the auth role namespace"),
