...
function buildProductionGoNoGoReadiness(options = {}) {
  ...
  const checks = [
    ...
    check("goNoGoReadiness:ui", html.includes('data-platform-section="production-go-no-go"') && html.includes("production-go-no-go-ui.js") && ui.includes("dataset.goNoGoApproval") && ui.includes("dataset.approvalRole") && ui.includes("dataset.goNoGoDecision") && !/innerHTML|insertAdjacentHTML/.test(ui), "platform command workbench uses trusted DOM rendering with business responsibility isolated from the auth role namespace"),
    ...
    check("goNoGoReadiness:evidenceDrift", model.includes("staleApprovals") && model.includes("evidenceCurrent") && ui.includes("dataset.goNoGoDrift") && docs.includes("evidence drift"), `${center.summary.staleApprovals || 0} stale approvals against current evidence fingerprint`),
