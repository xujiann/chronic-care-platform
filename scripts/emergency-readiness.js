...
function buildEmergencyReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    check("production:site-signoff", productionCenter.summary.cutoverBlockers === 5 && ["REQUIREMENT_CONFIRMATION","submit-evidence","verify-evidence","evidenceDigest","submittedBy"].every((marker)=>sources.production.includes(marker)) && ["requirement-evidence-form","requirement-verify-form","VERIFY EMERGENCY SITE EVIDENCE"].every((marker)=>sources.html.includes(marker)) && sources.apiTest.includes("independentCommissionToken"), "site evidence requires external provenance plus independent verification before a blocker is released", "production"),
