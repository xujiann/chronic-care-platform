...
const EmergencyDeviceGateway = require("../emergency-device-gateway");
...
function buildEmergencyReadinessReport(options = {}) {
  ...
  const deviceGatewayCenter = EmergencyDeviceGateway.center(EmergencyDeviceGateway.seed());
  const checks = [
    ...
    check("life-chain:safety-controls", ["revokeAuthorization","requestAutomaticSosCancellation","resolveAutomaticSosCancellation","suppressed-duplicate","cancellationReviews"].every((marker)=>sources.lifeService.includes(marker)) && ["/authorizations/:id/revoke","automatic-sos-cancellation-request","automatic-sos-cancellation-resolve"].every((marker)=>sources.server.includes(marker)) && ["Duplicate device signal was suppressed","data-revoke-authorization","data-cancellation-event","data-cancellation-review-event"].every((marker)=>sources.lifeUi.includes(marker)) && sources.docs.includes("120-only cancellation resolution") && sources.lifeTest.includes("suppressed-duplicate"), "deduplication, consent withdrawal, human cancellation review, and scoped hospital confirmation are release-wired", "safety"),
    check("life-chain:trusted-device-gateway", ["createHmac","timingSafeEqual","credentialRef","certificateFingerprint","monotonic-device-counter","rawSecretsPersisted:false","rotate-credential","manageDevice"].every((marker)=>sources.deviceGateway.includes(marker)) && sources.deviceGatewayTest.includes("independent attestation verification") && sources.lifeService.includes("createAutomaticSosFromTrustedSignal") && sources.lifeTest.includes("trusted gateway signal") && EmergencyDeviceGateway.ROUTE_CONTRACTS.length === 6 && deviceGatewayCenter.formalGoLiveState === "blocked-until-device-registered", "device identity, HMAC verification, clock window, monotonic counter, authenticated idempotency, four-eyes attestation, suspension, retirement, credential rotation and verified-signal life-chain bridging are implemented without persisting secrets", "security"),
    ...
    check("production:site-signoff", productionCenter.summary.cutoverBlockers === 6 && ["REQUIREMENT_CONFIRMATION","submit-evidence","verify-evidence","evidenceDigest","submittedBy","EMG-SITE-06","regional-platform"].every((marker)=>sources.production.includes(marker)) && ["requirement-evidence-form","requirement-verify-form","VERIFY EMERGENCY SITE EVIDENCE"].every((marker)=>sources.html.includes(marker)) && sources.apiTest.includes("independentCommissionToken"), "six site gates cover every required integration and require external provenance plus independent verification before a blocker is released", "production"),
