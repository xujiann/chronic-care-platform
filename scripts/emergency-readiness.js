...
function buildEmergencyReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    check("life-chain:safety-controls", ["revokeAuthorization","requestAutomaticSosCancellation","resolveAutomaticSosCancellation","suppressed-duplicate"].every((marker)=>sources.lifeService.includes(marker)) && ["/authorizations/:id/revoke","automatic-sos-cancellation-request","automatic-sos-cancellation-resolve"].every((marker)=>sources.server.includes(marker)) && ["Duplicate device signal was suppressed","data-revoke-authorization","data-cancellation-event"].every((marker)=>sources.lifeUi.includes(marker)) && sources.docs.includes("120-only cancellation resolution") && sources.lifeTest.includes("suppressed-duplicate"), "deduplication, consent withdrawal, human cancellation review, and scoped hospital confirmation are release-wired", "safety"),
