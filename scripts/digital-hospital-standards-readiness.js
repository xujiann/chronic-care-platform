...
const REQUIRED_API_MARKERS = [
  ...
  "/api/digital-hospital/policy-register/:id/change-actions",
...
function buildDigitalHospitalStandardsReadiness(options = {}) {
  ...
  const checks = [
    ...
    check("digitalHospital:apiContract", apiMarkersPresent.length === REQUIRED_API_MARKERS.length && js.includes("HealthCityAuth?.authFetch") && js.includes("Array.isArray(payload.policyRegister)"), `${apiMarkersPresent.length}/${REQUIRED_API_MARKERS.length} API markers, frontend auth fetch and non-ready remediation state loading`),
    check("digitalHospital:policyGovernance", policyRecordCount >= 18 && controlCount >= 12 && html.includes("data-digital-hospital-section=\"policy-register\"") && html.includes("data-digital-hospital-section=\"control-matrix\"") && html.includes("digital-hospital-policy-change-form") && js.includes("recordDigitalHospitalPolicyReview") && js.includes("recordDigitalHospitalPolicyChange") && governance.includes("normalizeDigitalHospitalPolicyChangeAction") && governance.includes("historical-planning") && controlDoc.includes("2026 年现行基线"), `${policyRecordCount} policy records / ${controlCount} controls with lifecycle, version impact and review workflow`),
