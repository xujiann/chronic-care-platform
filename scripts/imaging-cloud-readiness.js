...
function buildImagingCloudReadinessReport(options = {}) {
  ...
  const checks = [
    ...
    check("spec:imaging-teleconsultation", sources.server.includes("/api/imaging-cloud/studies/:id/teleconsultations") && sources.server.includes("rawDicomIncluded: false") && sources.html.includes('data-imaging-section="remote-teleconsultation"') && sources.pageJs.includes("startImagingTeleconsultation") && sources.docs.includes("studies/:id/teleconsultations"), "authorized imaging studies can create a minimized remote-teleconsultation work order without exporting original DICOM or patient identifiers", "integration"),
    ...
    check("security:diagnostic-viewer-boundary", sources.server.includes('requireApiRole(req, res, ["commission", "institution", "county"], "/api/imaging-cloud/studies/:id/viewer")') && sources.server.includes("diagnostic-level series unavailable") && sources.pageJs.includes("canOpenDiagnosticViewer") && sources.docs.includes("OHIF 诊断级调阅仅面向"), "diagnostic OHIF links require an authorized clinical role and diagnostic-level series; citizens retain browser-level preview only", "security"),
  ...
  return {
    ...
    summary: {
      ...
      requiredCapabilities: ["hospital ingest", "patient mobile viewing", "EMR compatibility", "FHIR writeback retry", "imaging teleconsultation", "authorization sharing", "quality control", "mutual-recognition appeal", "security baseline", "development plan", "synthetic acceptance", "formal production gate"],
