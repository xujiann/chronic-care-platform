...
function buildPublicHealthCoordinationReadiness(options = {}) {
  ...
  const contractDoc = options.contractDoc ?? read("docs/public-health-external-contract-governance.md");
  ...
  const checks = [
    ...
    check("adapter:sequential-contract-chain", ["transitions", "contract-transition-disconnected", "contract-transition-approval-order-invalid", "contract-transition-window-overlap"].every((token) => contractSource.includes(token)) && ["transitionTracks", "transitionIndex", "allowedSuccessorContracts"].every((token) => contractCutoverSource.includes(token)), "continuous signed version chains retain each historical cutover track and reject branches, gaps, approval-order errors and overlapping windows", "adapter"),
    ...
    check("docs:t00-boundary", ["T00", "server.js", "package.json", "现场证据", "八领域", "productionReady"].every((token) => doc.includes(token)) && ["requestKeyring", "receiptKeyring", "receiptReplayKeyHash", "legacy-static"].every((token) => keyringDoc.includes(token)) && ["expectedLaneControlVersion", "maxPending", "lane-circuit-open"].every((token) => resilienceDoc.includes(token)) && ["runtimeReleaseDigest", "contract-transition-conflict", "contract-governance-mismatch"].every((token) => contractDoc.includes(token)), "completion scope, key lifecycle, resilience, contract governance and remaining T00 integration are documented", "docs")
  ];
  return {
