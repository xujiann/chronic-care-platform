...
function buildInsurancePaymentAcceptance(options = {}) {
  ...
  const workflows = [
    ...
    { id: "drg-dip", label: "DRG/DIP分组测算", ready: ["dual-mode", "official-receipt-contract", "formal-grouping-async", "formal-grouping-compensation", "formal-grouping-integrity", "parameter-dual-review"].every((id) => checkById(diseasePayment, id)), evidence: ["signed-grouper-contract", "simulation-isolation", "formal-job-state-projection", "job-result-ledger-cross-check", "dead-letter-state-projection", "redacted-operations", "versioned-parameters", "impact-analysis"] },
