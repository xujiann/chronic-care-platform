...
function buildHospitalOperationsReleaseReport(options = {}) {
  ...
  const checks = [
    ...
    check("release:siteJointPatrol", /\/api\/operations\/site-joint-patrol/.test(serverSource) && /buildOperationsSiteJointPatrol/.test(serverSource) && /operations-site-joint-patrol/.test(operationsHtml) && /submitSiteJointPatrol/.test(operationsJs) && /operations-site-joint-patrol/.test(serverSource), "daily site joint-test patrol and audit action are present"),
    ...
    check("release:cutoverCommand", /\/api\/operations\/cutover-command/.test(serverSource) && /buildOperationsCutoverCommand/.test(serverSource) && /operation-cutover-command/.test(operationsHtml) && /signoffCutoverCommand/.test(operationsJs) && /operations-cutover-signoff/.test(serverSource), "production cutover command and signoff audit are present"),
    check("release:postCutoverObservation", /\/api\/operations\/post-cutover-observation/.test(serverSource) && /buildOperationsPostCutoverObservation/.test(serverSource) && /operation-post-cutover-observation/.test(operationsHtml) && /submitPostCutoverObservation/.test(operationsJs) && /operations-post-cutover-observation/.test(serverSource), "post-cutover observation panel and audit action are present"),
    check("release:postCutoverSignoffGate", /signoffReady/.test(serverSource) && /signoffStatus/.test(serverSource) && /signoffBlockers/.test(serverSource) && /signoffReadyWindows/.test(serverSource) && /signoffStatus/.test(operationsJs) && /可签收/.test(operationsJs), "post-cutover observation signoff gate is present"),
    check("release:launchReadiness", /buildOperationsLaunchReadiness/.test(serverSource) && /launchReadiness/.test(serverSource) && /operation-launch-readiness/.test(operationsHtml) && /renderLaunchReadiness/.test(operationsJs) && /operation-launch-readiness-summary/.test(portalCss), "launch readiness decision card is present"),
    check("release:goLiveGates", /\/api\/operations\/go-live-gates/.test(serverSource) && /\/api\/operations\/go-live-gates\/actions/.test(serverSource) && /buildOperationsGoLiveGates/.test(serverSource) && /operations-go-live-gate-review/.test(serverSource) && /operation-go-live-gates/.test(operationsHtml) && /renderGoLiveGates/.test(operationsJs) && /reviewGoLiveGate/.test(operationsJs) && /operation-go-live-gate-card/.test(portalCss), "go-live gate checklist API, review action, and panel are present"),
    ...
    check("release:resourcePool", /\/api\/operations\/resource-pool/.test(serverSource) && /buildOperationsResourcePool/.test(serverSource) && /operation-resource-pool/.test(operationsHtml) && /renderResourcePool/.test(operationsJs), "cross-hospital resource pool is present"),
    check("release:emergencyDispatchLoop", /\/api\/operations\/emergency-dispatch-loop/.test(serverSource) && /buildOperationsEmergencyDispatchLoop/.test(serverSource) && /operations-emergency-loop-panel/.test(operationsHtml) && /renderEmergencyDispatchLoop/.test(operationsJs) && /operation-emergency-loop/.test(portalCss), "emergency congestion dispatch loop, action API, panel, and audit evidence are present"),
    check("release:mobileDuty", /\/api\/operations\/mobile-duty/.test(serverSource) && /buildOperationsMobileDuty/.test(serverSource) && /operation-mobile-duty/.test(operationsHtml) && /renderMobileDuty/.test(operationsJs) && /operations-mobile-duty-reminder/.test(serverSource), "mobile duty command panel and message audit loop are present"),
