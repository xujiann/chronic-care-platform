...
function buildHospitalOperationsReleaseReport(options = {}) {
  ...
  const checks = [
    ...
    check("release:goLiveGates", /\/api\/operations\/go-live-gates/.test(serverSource) && /buildOperationsGoLiveGates/.test(serverSource) && /operation-go-live-gates/.test(operationsHtml) && /renderGoLiveGates/.test(operationsJs) && /operation-go-live-gate-card/.test(portalCss), "go-live gate checklist API and panel are present"),
