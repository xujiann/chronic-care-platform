...
function writeOutput(center, options = {}) {
  const output = path.resolve(ROOT, options.output || "release/research-rater-consistency.json");
  const markdown = path.resolve(ROOT, options.markdown || "release/research-rater-consistency.md");
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.mkdirSync(path.dirname(markdown), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify(center, null, 2)}\n`, "utf8");
  fs.writeFileSync(markdown, renderRaterConsistencyMarkdown(center), "utf8");
