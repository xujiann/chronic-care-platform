...
const REQUIREMENTS = [
  ...
  },
  {
    ...
    acceptance: ["resident phone-code login sends through the production gateway", "failed login lockout and audit evidence are reproducible", "delivery callback and throttling proof are archived before opening phone-code login to residents"],
    evidence: ["release/citizen-launch-foundation-readiness.md", "SMS delivery log", "masked resident SMS receipt", "platform-ops signoff"]
  },
  {
    id: "OSL-05",
  ...
  },
  {
    ...
    acceptance: ["bottom navigation, secondary page return, weak-network prompt, and 44px touch targets pass on site devices", "signed package, push certificate, crash monitoring, and staged upgrade channel are verified on site"],
    evidence: ["release/citizen-launch-foundation-readiness.md", "mobile acceptance screenshots", "mobile-release signoff", "crash and upgrade monitoring dashboard"]
  },
  {
    id: "OSL-13",
    domain: "gray-release",
...
function buildOnsiteLaunchRequirements(options = {}) {
  ...
  const checks = [
    ...
    { id: "onsite:citizen-dependency-owners", passed: REQUIREMENTS.some((item) => item.id === "OSL-04" && item.owner === "platform-ops" && item.evidence.includes("platform-ops signoff")) && REQUIREMENTS.some((item) => item.id === "OSL-12" && item.owner === "mobile-release" && item.evidence.includes("mobile-release signoff")), detail: "citizen launch onsite requirements align to production dependency owners" },
