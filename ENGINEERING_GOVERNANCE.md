...
T07 financial dispatch 后续切片只登记 `POST /api/financial-gateways/dispatch`。session/RBAC 与 gateway/institution scope 必须先于 ledger 读取；同一幂等键在当前进程串行，不同键只在短状态写锁内串行并允许下游调用并行，锁内重读并用 canonical request digest 区分精确重放和载荷冲突。外调前必须先把 `dispatching` reservation 写入既有 integration event ledger；外调后从新快照替换最终事件并在同一次最终写入中追加安全审计。最终写失败保留 reservation，重放不得二次外调；adapter 错误只允许稳定脱敏原因，失败重放保持 502 结构。SQLite CAS 冲突稳定脱敏，未知写失败不得 fallback 写。不得把进程锁、SQLite CAS 或 synthetic provider 测试解释为跨实例 exactly-once，真实 provider、凭据网络、多实例耐久性、reservation 对账和现场证据继续失败关闭。
...
T02 quality-governance 后续切片只登记 `POST /api/quality-operations-governance/items/:id/actions`。session/RBAC 先于 body/state，action 与 institution scope 在 receipt replay 前重新核验；不存在或无法映射的精确 record ID 返回 404 且不写 receipt/audit。既有 owner 状态机和 adapter 继续拥有 expectedVersion、command fingerprint、来源状态、receipt 及 governance/platform/security 三类审计；同 record 命令只在当前进程串行，非重放结果只调用一次 `writeDatabase`。SQLite CAS 与未知写失败必须稳定脱敏且不得 fallback audit；无新 schema/outbox/外部依赖，所有目录项继续 `NO-GO`，不得宣称跨实例 exactly-once。
...
TEST-005 规定标准 E2E 只能使用 Playwright Chromium，不得按本地操作系统自动切换浏览器。在线根 60 项
（含急救生命链、医生工作台、血液上线看板、陪诊工作台、产品运行驾驶舱、产品区域运行驾驶舱、质量安全工作台、区域切换工作台、血液召回面板、血液创新指挥中心和体检风险卡恶意响应回归）与
居民 13 项继续阻止 Service Worker；PWA 3 项只能在独立允许策略、context、动态回环端口和临时数据中运行。
三套必须形成 76 项不重叠并集，PWA 每项结束注销 registration 并删除 Cache Storage；runner 不得复用或
停止其他任务的服务。该门禁不证明真实 HTTPS、OS 安装策略、外部 Origin 或现场浏览器验收。

required `test` 聚合以 `always()` 检查地区矩阵、真实 PostgreSQL 合同、治理/API、浏览器 E2E 和发布
就绪五个上游结果，任一失败、取消或跳过均不得合并；`complete-unit-test` 继续作为独立 required check。
