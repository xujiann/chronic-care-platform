...
TEST-005 规定标准 E2E 只能使用 Playwright Chromium，不得按本地操作系统自动切换浏览器。根 27 项与
居民 13 项必须形成 40 项不重叠并集，并分别拥有动态回环端口和临时数据生命周期；通用在线 E2E 阻止
Service Worker，PWA/离线缓存必须在独立 context 与专项套件验证。runner 不得复用或停止其他任务的服务。
