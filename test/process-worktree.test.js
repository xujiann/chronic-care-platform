...
test("process manifest assigns every domain route exactly once", () => {
  ...
  assert.equal(resolveProtectedOwner("config/api-authentication-evidence.json", manifest), "T00");
  assert.equal(resolveProtectedOwner("test/api-authentication-evidence.test.js", manifest), "T00");
...
test("CI isolates browser E2E and release readiness behind the required test aggregate", () => {
  ...
  assert.match(governanceJob, /Verify object storage architecture decision remains fail-closed[\s\S]*npm run object-storage:architecture-governance:verify/);
