...
test("final readiness accepts every planned T08 functional increment", () => {
  ...
  assert.equal(report.summary.checks, 80);
  assert.equal(report.summary.passed, 80);
  assert.equal(report.summary.t08FunctionalChecks, 59);
  assert.equal(report.summary.t08FunctionalPassed, 59);
  ...
  assert.equal(report.checks.find((item) => item.id === "integration:t00-modernization-source-operations").passed, true);
  assert.equal(report.checks.find((item) => item.id === "integration:t00-surveillance-rule-governance").passed, true);
  assert.equal(report.checks.find((item) => item.id === "integration:t00-surveillance-rule-governance-ui-release").passed, true);
...
test("final readiness renders and writes machine and human reports", () => {
  ...
  assert.match(fs.readFileSync(markdown, "utf8"), /T08 functional checks: 59\/59/);
