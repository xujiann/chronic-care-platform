...
test("API authentication, scoping and governance regression suite", async (t) => {
  ...
  await t.test("keeps health, authentication and error response contracts stable", async () => {
    ...
    assert.equal(healthDashboard.body.scope.role, "health-administration-management-service-system");
    ...
    assert.equal(healthDashboard.body.populationServiceBoard.periods.find((item) => item.id === "month").metrics.some((item) => item.id === "visits" && item.value === 92800), true);
    assert.equal(healthDashboard.body.populationServiceBoard.insights.length, 4);
    assert.equal(healthDashboard.body.populationServiceBoard.insights.some((item) => item.id === "medical-service-signal" && item.status === "ready"), true);
    ...
    assert.equal(healthDashboard.body.functionalReport.functions.length, 15);
    ...
    assert.equal(healthDashboard.body.indicatorCenter.dimensions.length, 8);
    assert.equal(healthDashboard.body.indicatorCenter.reformCategories.length, 7);
    assert.equal(healthDashboard.body.indicatorCenter.aggregationEntrypoints.length, 3);
    assert.equal(healthDashboard.body.indicatorCenter.aggregationEntrypoints.some((item) => item.id === "smart-hospital"), true);
    assert.equal(healthDashboard.body.indicatorCenter.indicators.some((item) => item.dimension === "performance" && item.name.includes("绩效考核")), true);
    assert.equal(healthDashboard.body.indicatorCenter.indicators.some((item) => item.dimension === "interface-launch" && item.drilldown.href === "#production-readiness-board"), true);
    assert.equal(healthDashboard.body.indicatorCenter.indicators.every((item) => item.definition && item.source && item.owner && item.currentValue && item.targetValue && item.reformCategory && item.confidence && item.drilldown.href), true);
    ...
    assert.equal(healthDashboard.body.checks.some((item) => item.id === "dashboard:indicator-center" && item.passed), true);
    ...
    assert.equal(productionReadiness.body.acceptanceRouting.length, 5);
    assert.equal(productionReadiness.body.acceptanceRouting.some((item) => item.id === "identity" && item.receiver.includes("统一认证管理员")), true);
    assert.equal(productionReadiness.body.acceptanceRouting.some((item) => item.id === "audit-retention" && item.failedAction.includes("生产上线阻断")), true);
    assert.equal(productionReadiness.body.acceptanceRouting.some((item) => item.id === "monitoring-dr" && item.passCondition.includes("RTO/RPO")), true);
    assert.equal(productionReadiness.body.backendGoLiveChecklist.items.length, 8);
    assert.equal(productionReadiness.body.backendGoLiveChecklist.items.some((item) => item.id === "backend-runtime" && item.capability.includes("Node.js")), true);
    assert.equal(productionReadiness.body.backendGoLiveChecklist.items.some((item) => item.id === "backend-audit-retention" && item.evidence.includes("AUDIT_EXPORT_PATH")), true);
    assert.equal(productionReadiness.body.indicatorCenterSummary.indicators, 8);
    assert.equal(productionReadiness.body.indicatorCenterEvidence.some((item) => item.evidence === "docs/health-dashboard-indicator-center-report.md"), true);
