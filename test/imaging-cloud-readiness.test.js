...
test("imaging cloud readiness validates province-spec capabilities", () => {
  ...
  assert.equal(report.checks.some((item) => item.id === "security:diagnostic-viewer-boundary" && item.passed), true);
  ...
  assert.equal(report.checks.some((item) => item.id === "production:deployment-contract" && item.passed), true);
  ...
  assert.equal(report.checks.some((item) => item.id === "governance:catalog-operations" && item.passed), true);
  ...
  assert.equal(report.artifacts.deploymentReadiness, "scripts/imaging-cloud-deployment-readiness.js");
