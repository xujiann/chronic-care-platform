...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.summary.total, 902);
  assert.equal(inventory.summary.byPriority.P0, 857);
  assert.equal(inventory.summary.byPriority.P1, 45);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 64);
  ...
  assert.equal(inventory.summary.byType["dom-inner-html"], 846);
  ...
  assert.equal(new Set(inventory.findings.filter((item) => ["dom-inner-html", "dom-insert-adjacent-html"].includes(item.type)).map((item) => item.asset)).size, 40);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 902);
