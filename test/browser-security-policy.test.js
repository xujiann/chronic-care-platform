...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.summary.total, 894);
  assert.equal(inventory.summary.byPriority.P0, 849);
  assert.equal(inventory.summary.byPriority.P1, 45);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 62);
  ...
  assert.equal(inventory.summary.byType["dom-inner-html"], 839);
  ...
  assert.equal(new Set(inventory.findings.filter((item) => ["dom-inner-html", "dom-insert-adjacent-html"].includes(item.type)).map((item) => item.asset)).size, 39);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 894);
