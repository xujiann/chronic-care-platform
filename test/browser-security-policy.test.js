...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.summary.total, 887);
  assert.equal(inventory.summary.byPriority.P0, 842);
  assert.equal(inventory.summary.byPriority.P1, 45);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 57);
  ...
  assert.equal(inventory.summary.byType["dom-inner-html"], 832);
  ...
  assert.equal(new Set(inventory.findings.filter((item) => ["dom-inner-html", "dom-insert-adjacent-html"].includes(item.type)).map((item) => item.asset)).size, 34);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 887);
