...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.summary.total, 985);
  assert.equal(inventory.summary.byPriority.P0, 940);
  assert.equal(inventory.summary.byPriority.P1, 45);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 89);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 985);
