...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.summary.total, 847);
  assert.equal(inventory.summary.byPriority.P0, 805);
  assert.equal(inventory.summary.byPriority.P1, 42);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 53);
  ...
  assert.equal(inventory.summary.byType["dom-inner-html"], 795);
  ...
  assert.equal(new Set(inventory.findings.filter((item) => ["dom-inner-html", "dom-insert-adjacent-html"].includes(item.type)).map((item) => item.asset)).size, 32);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 847);
