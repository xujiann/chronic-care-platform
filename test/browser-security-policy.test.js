...
test("published browser assets match the exact Inventory v2 fail-closed baseline", () => {
  ...
  assert.equal(inventory.assetsScanned, 182);
  assert.equal(inventory.summary.total, 841);
  assert.equal(inventory.summary.byPriority.P0, 799);
  assert.equal(inventory.summary.byPriority.P1, 42);
  assert.equal(inventory.summary.byPriority.P2, 0);
  assert.equal(inventory.findings.length, 51);
  ...
  assert.equal(inventory.summary.byType["dom-inner-html"], 791);
  ...
  assert.equal(inventory.summary.byType["dynamic-html-style-attribute"], 29);
  ...
  assert.equal(new Set(inventory.findings.filter((item) => ["dom-inner-html", "dom-insert-adjacent-html"].includes(item.type)).map((item) => item.asset)).size, 31);
  ...
  assert.equal(policy.riskBaseline.findings.reduce((sum, item) => sum + item.count, 0), 841);
  ...
  assert.equal(policy.riskBaseline.findings.some((item) => item.asset === "blood-innovation.js"), false);
  assert.equal(policy.riskBaseline.findings.some((item) => item.asset === "blood-recall.js"), false);
  ...
  assert.equal(policy.riskBaseline.findings.some((item) => item.asset === "production-go-no-go-ui.js"), false);
