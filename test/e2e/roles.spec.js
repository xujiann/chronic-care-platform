...
test("commission health dashboard filters live source actions and drills into source app", async ({ page }) => {
  ...
  await expect(page.locator(".dashboard-section-nav a[href='#dashboard-review-quick-links']")).toContainText("上线审查");
  ...
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-review-summary-item]")).toHaveCount(3);
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-review-summary-item='blocked']")).toContainText("待签字");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-review-check]")).toHaveCount(4);
  await expect(page.locator("[data-dashboard-review-check='production-gates']")).toContainText("正式环境");
  await expect(page.locator("[data-dashboard-review-check='production-gates'] [data-dashboard-review-status='blocked']")).toContainText("待签字");
  await expect(page.locator("[data-dashboard-review-check='onsite-evidence'] [data-dashboard-review-status='watch']")).toContainText("待替换");
  await expect(page.locator("[data-dashboard-review-check='operation-boundary']")).toContainText("市、县两级");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-review-rule]")).toHaveCount(3);
  await expect(page.locator("[data-dashboard-review-rule='demo-release']")).toContainText("生产 ready");
  await expect(page.locator("[data-dashboard-review-rule='source-boundary']")).toContainText("只汇总");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-next-feature]")).toHaveCount(5);
  await expect(page.locator("[data-dashboard-next-feature='jurisdiction-drilldown']")).toContainText("市县辖区");
  await expect(page.locator("[data-dashboard-next-feature='realtime-warning']")).toContainText("小时级");
  await expect(page.locator("[data-dashboard-next-feature='evidence-signoff']")).toContainText("签字闭环");
  await expect(page.locator("[data-dashboard-next-feature='data-quality-lineage']")).toContainText("来源血缘");
  await expect(page.locator("[data-dashboard-next-feature='operation-handover']")).toContainText("市县两级");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-function-responsibility-summary-item]")).toHaveCount(4);
  await expect(page.locator("[data-dashboard-function-responsibility-summary-item='implemented']")).toContainText("5");
  await expect(page.locator("[data-dashboard-function-responsibility-summary-item='p1-plan']")).toContainText("预警、督办、证据闭环");
  await expect(page.locator("[data-dashboard-function-responsibility-summary-item='blocked']")).toContainText("生产审计留存");
  await expect(page.locator("[data-dashboard-function-responsibility-summary-item='owners']")).toContainText("责任部门");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-next-priority]")).toHaveCount(3);
  await expect(page.locator("[data-dashboard-next-priority='p0-production-gates']")).toContainText("统一身份");
  await expect(page.locator("[data-dashboard-next-priority='p0-production-gates']")).toContainText("项目办/安全牵头");
  await expect(page.locator("[data-dashboard-next-priority='p1-management-views']")).toContainText("小时级预警");
  await expect(page.locator("[data-dashboard-next-priority='p1-management-views']")).toContainText("卫生健康行政部门");
  await expect(page.locator("[data-dashboard-next-priority='p2-operation-quality']")).toContainText("来源血缘");
  await expect(page.locator("[data-dashboard-next-priority='p2-operation-quality']")).toContainText("统计/运维协同");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-gate]")).toHaveCount(5);
  await expect(page.locator("[data-dashboard-p0-gate='identity']")).toContainText("统一身份接入签字");
  await expect(page.locator("[data-dashboard-p0-gate='audit-retention']")).toContainText("AUDIT_EXPORT_PATH");
  await expect(page.locator("[data-dashboard-p0-gate='database']")).toContainText("生产主库");
  await expect(page.locator("[data-dashboard-p0-gate='monitoring-dr']")).toContainText("RTO/RPO");
  await expect(page.locator("[data-dashboard-p0-gate='interface-signoff']")).toContainText("真实报文");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-status='blocked']")).toHaveCount(5);
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-evidence]")).toHaveCount(5);
  await expect(page.locator("[data-dashboard-p0-evidence='identity']")).toContainText("OIDC/SAML");
  await expect(page.locator("[data-dashboard-p0-evidence='audit-retention']")).toContainText("哈希链导出样例");
  await expect(page.locator("[data-dashboard-p0-evidence='database']")).toContainText("迁移日志");
  await expect(page.locator("[data-dashboard-p0-evidence='monitoring-dr']")).toContainText("/api/metrics");
  await expect(page.locator("[data-dashboard-p0-evidence='interface-signoff']")).toContainText("联合签字单");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-evidence-status='missing']")).toHaveCount(5);
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-acceptance]")).toHaveCount(5);
  await expect(page.locator("[data-dashboard-p0-acceptance='identity']")).toContainText("统一认证管理员");
  await expect(page.locator("[data-dashboard-p0-acceptance='audit-retention']")).toContainText("不得解除生产上线阻断");
  await expect(page.locator("[data-dashboard-p0-acceptance='database']")).toContainText("演示或联调环境");
  await expect(page.locator("[data-dashboard-p0-acceptance='monitoring-dr']")).toContainText("RTO/RPO");
  await expect(page.locator("[data-dashboard-p0-acceptance='interface-signoff']")).toContainText("接口试运行状态");
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-p0-acceptance-status='pending']")).toHaveCount(5);
  await expect(page.locator("#dashboard-review-quick-links [data-dashboard-function-owner]")).toHaveCount(5);
  await expect(page.locator("[data-dashboard-function-owner='aggregate-entry']")).toContainText("规划信息处/信息中心");
  await expect(page.locator("[data-dashboard-function-owner='population-service-board']")).toContainText("日、周、月、年");
  await expect(page.locator("[data-dashboard-function-owner='risk-action-closure']")).toContainText("超期督办");
  await expect(page.locator("[data-dashboard-function-owner='interface-evidence']")).toContainText("真实报文归档");
  await expect(page.locator("[data-dashboard-function-owner='launch-policy']")).toContainText("生产审计留存");
  ...
  await expect(page.locator("#dashboard-function-list [data-function-report]")).toHaveCount(15);
  ...
  await expect(page.locator("#dashboard-indicator-center")).toHaveAttribute("data-indicator-dimension", "all");
  await expect(page.locator("#indicator-category-list [data-indicator-category]")).toHaveCount(7);
  await expect(page.locator("[data-indicator-category='resource-efficiency']")).toContainText("资源效率");
  await expect(page.locator("#indicator-aggregation-entrypoints [data-indicator-entry]")).toHaveCount(3);
  await expect(page.locator("[data-indicator-entry='smart-hospital']")).toContainText("智慧医院管理线汇聚入口");
  await expect(page.locator("#indicator-center-list [data-indicator]")).toHaveCount(8);
  await expect(page.locator("[data-indicator='performance-public-hospital']")).toContainText("公立医院绩效考核");
  await page.locator("#indicator-dimension-filter").selectOption("interface-launch");
  await expect(page.locator("#dashboard-indicator-center")).toHaveAttribute("data-indicator-dimension", "interface-launch");
  await expect(page.locator("#indicator-center-list [data-indicator]")).toHaveCount(1);
  await expect(page.locator("[data-indicator='interface-launch-readiness']")).toContainText("上线准备通过率");
  await page.locator("#indicator-status-filter").selectOption("blocked");
  await expect(page.locator("#dashboard-indicator-center")).toHaveAttribute("data-indicator-status", "blocked");
  await expect(page.locator("#indicator-center-list [data-indicator]")).toHaveCount(1);
  await expect(page.locator("#indicator-center-reset")).toBeEnabled();
  await page.locator("#indicator-center-reset").click();
  await expect(page.locator("#dashboard-indicator-center")).toHaveAttribute("data-indicator-dimension", "all");
  await expect(page.locator("#indicator-center-list [data-indicator]")).toHaveCount(8);
  ...
  await expect(page.locator("#production-readiness-board")).toHaveAttribute("data-backend-go-live-status", "blocked");
  await expect(page.locator("#production-readiness-list [data-production-gate]")).toHaveCount(5);
  await expect(page.locator("#production-acceptance-routing-summary")).toContainText("5 项 P0 接收判定");
  await expect(page.locator("#production-acceptance-routing-list [data-production-acceptance]")).toHaveCount(5);
  await expect(page.locator("#production-backend-go-live-summary")).toContainText("8 项后端上线准备");
  await expect(page.locator("#production-backend-go-live-list [data-production-backend-go-live]")).toHaveCount(8);
  await expect(page.locator("[data-production-backend-go-live='backend-database']")).toContainText("生产数据库");
  await expect(page.locator("[data-production-backend-go-live='backend-audit-retention']")).toContainText("AUDIT_EXPORT_PATH");
  await expect(page.locator("#production-backend-go-live-reset")).toBeDisabled();
  await page.locator("[data-production-backend-status-filter='blocked']").click();
  await expect(page.locator("#production-readiness-board")).toHaveAttribute("data-backend-go-live-active-status", "blocked");
  await expect(page.locator("#production-readiness-board")).toHaveAttribute("data-backend-go-live-filtered", "true");
  await expect(page.locator("#production-backend-go-live-list [data-production-backend-go-live]")).toHaveCount(4);
  await expect(page.locator("#production-backend-go-live-list [data-production-backend-status='blocked']")).toHaveCount(4);
  await expect(page.locator("#production-backend-go-live-reset")).toBeEnabled();
  await page.locator("#production-backend-go-live-reset").click();
  await expect(page.locator("#production-readiness-board")).toHaveAttribute("data-backend-go-live-active-status", "all");
  await expect(page.locator("#production-backend-go-live-list [data-production-backend-go-live]")).toHaveCount(8);
  await expect(page.locator("[data-production-acceptance='identity']")).toContainText("统一认证管理员");
  await expect(page.locator("[data-production-acceptance='audit-retention']")).toContainText("不得解除生产上线阻断");
  await expect(page.locator("[data-production-acceptance='database']")).toContainText("迁移日志");
  await expect(page.locator("[data-production-acceptance='interface-signoff']")).toContainText("接口试运行状态");
