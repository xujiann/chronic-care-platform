...
test("drug consumable readiness covers required supervision boundaries", () => {
  ...
  assert.equal(report.requiredBoundaries.includes("supply-alert"), true);
  ...
  assert.equal(report.summary.supplyAlerts >= 1, true);
  ...
  assert.equal(report.checks.some((item) => item.id === "drug-consumable:supply-alert" && item.passed), true);
  ...
  assert.equal(report.checks.some((item) => item.id === "drug-consumable:launch-readiness" && item.passed), true);
  assert.equal(report.launchReadiness.demoReviewReady, true);
  assert.equal(report.launchReadiness.productionCutoverBlocked, true);
  assert.equal(report.launchReadiness.implementedCapabilities.includes("traceability evidence submission and coverage"), true);
  assert.equal(report.launchReadiness.preLaunchGaps.includes("production identity, secrets, and audit-retention storage"), true);
  assert.equal(report.traceabilityEvidenceChecklist.some((item) => item.id === "trace-consumable-catalog" && item.ready), true);
  assert.equal(report.supplyAlerts.some((item) => item.id === "dcs-supply-mp3" && item.relatedPickupId === "mp3"), true);
  ...
  assert.match(markdown, /Supply assurance alerts: 1/);
  ...
  assert.match(markdown, /Launch readiness/);
  assert.match(markdown, /Production cutover blocked: yes/);
  ...
  assert.match(markdown, /dcs-supply-mp3/);
});
