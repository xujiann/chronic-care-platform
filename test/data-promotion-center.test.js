...
test("P0 data promotion registers twelve priority collections and reduces legacy authority debt", () => {
  const report = buildDataPromotionCenter(data, { now: "2026-08-17T01:00:00.000Z" });
  assert.equal(report.ok, true);
  assert.equal(report.summary.promotedP0, 12);
  assert.equal(report.summary.authoritative, 56);
  assert.equal(report.summary.legacyBlocked, 193);
