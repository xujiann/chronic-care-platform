...
test("real governance HTTP commands persist, replay, isolate roles and require independent approval", async (t) => {
  ...
  const clinicalCenter = await request(base, "/api/quality-safety/ai-cdss/center", institution);
  assert.equal(clinicalCenter.status, 200);
  const suspendedRule = clinicalCenter.body.ruleCards.find((item) => item.id === rule.id);
  assert.equal(suspendedRule.governanceStatus, "suspended");
  assert.equal(suspendedRule.decisionAvailable, false);
  assert.equal(suspendedRule.recommendedReview, "");
  assert.equal(JSON.stringify(clinicalCenter.body).includes("commandMetadata"), false);
  const platformCenter = await request(base, "/api/runtime/ai-governance/center", author);
  assert.equal(platformCenter.status, 200);
  assert.equal(platformCenter.body.schemaVersion, "platform-ai-governance-center-v1");
  assert.equal(platformCenter.body.useCases.length, 4);
