...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 48 tests", () => {
  ...
  assert.equal(rootTests.length, 32);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 48);
  assert.equal(union.size, 48);
