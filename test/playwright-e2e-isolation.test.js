...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 76 tests", () => {
  ...
  assert.equal(rootTests.length, 60);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 76);
  assert.equal(union.size, 76);
