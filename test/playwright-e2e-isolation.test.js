...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 51 tests", () => {
  ...
  assert.equal(rootTests.length, 35);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 51);
  assert.equal(union.size, 51);
