...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 46 tests", () => {
  ...
  assert.equal(rootTests.length, 30);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 46);
  assert.equal(union.size, 46);
