...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 57 tests", () => {
  ...
  assert.equal(rootTests.length, 41);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 57);
  assert.equal(union.size, 57);
