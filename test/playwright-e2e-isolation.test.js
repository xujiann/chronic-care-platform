...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 60 tests", () => {
  ...
  assert.equal(rootTests.length, 44);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 60);
  assert.equal(union.size, 60);
