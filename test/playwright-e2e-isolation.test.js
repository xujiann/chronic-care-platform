...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 54 tests", () => {
  ...
  assert.equal(rootTests.length, 38);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 54);
  assert.equal(union.size, 54);
