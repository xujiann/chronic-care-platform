...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 64 tests", () => {
  ...
  assert.equal(rootTests.length, 48);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 64);
  assert.equal(union.size, 64);
