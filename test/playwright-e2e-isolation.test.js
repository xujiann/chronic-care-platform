...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 63 tests", () => {
  ...
  assert.equal(rootTests.length, 47);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 63);
  assert.equal(union.size, 63);
