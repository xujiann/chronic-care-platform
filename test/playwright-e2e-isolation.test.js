...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 73 tests", () => {
  ...
  assert.equal(rootTests.length, 57);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 73);
  assert.equal(union.size, 73);
