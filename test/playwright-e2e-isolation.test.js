...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 71 tests", () => {
  ...
  assert.equal(rootTests.length, 55);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 71);
  assert.equal(union.size, 71);
