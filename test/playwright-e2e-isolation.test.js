...
test("online, resident and PWA Playwright suites form an exact disjoint partition of all 49 tests", () => {
  ...
  assert.equal(rootTests.length, 33);
  assert.equal(residentTests.length, 13);
  assert.equal(pwaTests.length, 3);
  assert.equal(declared, 49);
  assert.equal(union.size, 49);
