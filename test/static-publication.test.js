...
test("static publication contract exposes only the explicit browser graph", () => {
  ...
  assert.equal(contract.entrypoints.length, 55);
  assert.equal(contract.entrypoints.includes("ai-governance.html"), true);
...
test("static publication build writes a sanitized standalone Pages artifact", () => {
  ...
  try {
    ...
    assert.equal(result.manifest.browserSecurity.inventory.total, 841);
    assert.equal(result.manifest.browserSecurity.inventory.byPriority.P0, 799);
