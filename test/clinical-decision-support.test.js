...
test("replay rechecks rules without visible alerts before returning saved recommendations", async () => {
  const f = fixture({ rulePolicy: (rule) => ({ status: rule.governance.status, decisionAvailable: rule.governance.status === "approved" }) });
  f.data.phase2ClinicalAssistRules.push({ id: "other-rule", defaultAction: "withheld-recommendation", configStatus: "active", governance: { status: "approved" } });
  const command = { idempotencyKey: "rule-replay", expectedVersion: 0 };
  const committed = await f.command(command);
  assert.equal(committed.overview.rules[1].defaultAction, "withheld-recommendation");
  f.data.phase2ClinicalAssistRules[1].governance.status = "suspended";
  assert.equal(f.service.buildOverview(f.data, doctor).rules[1].defaultAction, "");
  const writes = f.calls.writes;
  await assert.rejects(f.command(command), { code: "CDSS_REPLAY_SCOPE_CHANGED" });
...
test("governed rules default to unavailable, suppress recommendations and reject acceptance", async () => {
  ...
  assert.equal(alert.governanceStatus, "unverified");
    ...
    assert.equal(f.service.buildOverview(f.data, doctor).rules[0].defaultAction, "");
    assert.equal(f.service.buildOverview(f.data, doctor).ruleConfig[0].defaultAction, "");
