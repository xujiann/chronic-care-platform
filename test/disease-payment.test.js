...
test("monthly settlement supports reconciliation and payment", () => {
  ...
  const created = Service.createSettlementBatch(formallyGroupAll(), { period: "2026-06", submissionDeadline: "2026-07-10", workingCalendar }, "insurance", { at: "2026-07-21T08:00:00.000Z" });
  ...
  assert.equal(created.batch.frozenAt, "2026-07-21T08:00:00.000Z");
  ...
  assert.throws(() => Service.applyInsuranceCoreSettlementCallback(created.state, created.batch.id, { action: "confirm-payment", receiptId: "PAY-TOO-EARLY", paidAmountFen: created.batch.standardAmountFen, at: "2026-07-21T09:00:00.000Z" }), /结算状态不允许/);
  const submitted = Service.reconcileBatch(created.state, created.batch.id, { action: "submit-core", externalRequestId: "CORE-REQ-001", idempotencyKey: "CORE-IDEM-001", at: "2026-07-22T08:00:00.000Z" }, "insurance");
  assert.equal(submitted.batch.settlementState, "CORE_SUBMITTED");
  const duplicate = Service.reconcileBatch(submitted.state, created.batch.id, { action: "submit-core", externalRequestId: "CORE-REQ-001", idempotencyKey: "CORE-IDEM-001", at: "2026-07-22T08:00:00.000Z" }, "insurance");
  assert.equal(duplicate.idempotent, true);
  assert.throws(() => Service.reconcileBatch(duplicate.state, created.batch.id, { action: "core-accepted", receiptId: "MANUAL-FAKE-RECEIPT", at: "2026-07-23T08:00:00.000Z" }, "insurance"), /只能由医保核心可信回调驱动/);
  const accepted = Service.applyInsuranceCoreSettlementCallback(duplicate.state, created.batch.id, { action: "core-accepted", receiptId: "CORE-ACCEPT-001", at: "2026-07-23T08:00:00.000Z" });
  const reconciling = Service.reconcileBatch(accepted.state, created.batch.id, { action: "start-reconciliation", idempotencyKey: "RECON-001", providerSummaryDigest: "a".repeat(64), at: "2026-07-24T08:00:00.000Z" }, "finance");
  const reconciled = Service.reconcileBatch(reconciling.state, created.batch.id, { action: "confirm-matched", idempotencyKey: "MATCH-001", providerAmountFen: created.batch.standardAmountFen, at: "2026-07-25T08:00:00.000Z" }, "finance");
  assert.equal(reconciled.batch.status, "已对账");
  assert.equal(reconciled.batch.settlementState, "RECONCILED");
  const requested = Service.reconcileBatch(reconciled.state, created.batch.id, { action: "request-payment", paymentRequestId: "PAY-REQ-001", at: "2026-07-26T08:00:00.000Z" }, "insurance");
  const paid = Service.applyInsuranceCoreSettlementCallback(requested.state, created.batch.id, { action: "confirm-payment", receiptId: "PAY-RECEIPT-001", paidAmountFen: created.batch.standardAmountFen, at: "2026-07-27T08:00:00.000Z" });
  assert.equal(paid.batch.settlementState, "PAID");
  assert.equal(paid.batch.sla.status, "completed-within-sla");
  assert.deepEqual(paid.batch.events.map((event) => event.at), [...paid.batch.events].sort((left, right) => Date.parse(left.at) - Date.parse(right.at)).map((event) => event.at));
...
test("readiness report distinguishes locally ready functions from external blockers", () => {
  ...
  assert.equal(report.checks.find((item) => item.id === "settlement-time-order")?.ok, true);
