...
test("release artifact manifest indexes reports templates commands and evidence", () => {
  ...
  assert.equal(report.artifacts.length >= 20, true);
  ...
  assert.equal(report.artifacts.some((item) => item.id === "quality-safety-joint-test" && item.command === "quality-safety:joint-test" && item.evidence === "/api/quality-safety/interface-joint-test-pack"), true);
