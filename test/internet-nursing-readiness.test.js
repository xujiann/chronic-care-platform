...
test("internet nursing readiness validates three-role workflow and policy evidence", () => {
  ...
  assert.equal(report.checks.some((item) => item.id === "nursing:p1Lifecycle" && item.passed), true);
  assert.equal(report.checks.some((item) => item.id === "nursing:writePathAdapter" && item.passed), true);
  ...
  assert.equal(report.summary.p1LifecycleTracks, 6);
  assert.equal(report.summary.writePathAdapter, true);
  assert.equal(report.summary.eventOutbox, true);
