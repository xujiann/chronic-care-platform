...
test("health dashboard summary aggregates the first seven applications without replacing them", () => {
  ...
  assert.equal(report.productionReadinessGate.acceptanceRouting.length, 5);
  assert.equal(report.productionReadinessGate.acceptanceRouting.some((item) => item.id === "identity" && item.receiver.includes("统一认证管理员")), true);
  assert.equal(report.productionReadinessGate.acceptanceRouting.some((item) => item.id === "audit-retention" && item.failedAction.includes("生产上线阻断")), true);
  assert.equal(report.productionReadinessGate.acceptanceRouting.some((item) => item.id === "database" && item.requiredPreparation.includes("迁移日志")), true);
  assert.equal(report.productionReadinessGate.acceptanceRouting.every((item) => item.receiver && item.requiredPreparation && item.passCondition && item.failedAction), true);
  assert.equal(report.productionReadinessGate.backendGoLiveChecklist.items.length, 8);
  assert.equal(report.productionReadinessGate.backendGoLiveChecklist.items.some((item) => item.id === "backend-runtime" && item.capability.includes("Node.js")), true);
  assert.equal(report.productionReadinessGate.backendGoLiveChecklist.items.some((item) => item.id === "backend-database" && item.status === "blocked"), true);
  assert.equal(report.productionReadinessGate.backendGoLiveChecklist.items.some((item) => item.id === "backend-audit-retention" && item.evidence.includes("AUDIT_EXPORT_PATH")), true);
  assert.equal(report.productionReadinessGate.backendGoLiveChecklist.items.every((item) => item.owner && item.requiredPreparation && item.evidence && item.nextAction), true);
  assert.equal(report.totals.productionReady, false);
  assert.equal(report.indicatorCenter.indicators.length, 8);
  assert.equal(report.indicatorCenter.dimensions.length, 8);
  assert.equal(report.indicatorCenter.reformCategories.length, 7);
  assert.equal(report.indicatorCenter.reformCategories.some((item) => item.id === "resource-efficiency" && item.name === "资源效率"), true);
  assert.equal(report.indicatorCenter.aggregationEntrypoints.length, 3);
  assert.equal(report.indicatorCenter.aggregationEntrypoints.some((item) => item.id === "smart-hospital" && item.name.includes("智慧医院管理线")), true);
  assert.equal(report.indicatorCenter.indicators.some((item) => item.dimension === "performance" && item.name.includes("绩效考核")), true);
  assert.equal(report.indicatorCenter.indicators.some((item) => item.dimension === "grade-review" && item.drilldown.href === "#site-evidence-package"), true);
  assert.equal(report.indicatorCenter.indicators.some((item) => item.dimension === "interface-launch" && item.status === "blocked"), true);
  assert.equal(report.indicatorCenter.indicators.every((item) => item.definition && item.source && item.owner && item.currentValue && item.targetValue && item.reformCategory && item.quality && item.confidence && item.blockers.length && item.drilldown.href), true);
  assert.equal(report.indicatorCenter.releaseEvidence.some((item) => item.evidence === "docs/health-dashboard-indicator-center-report.md"), true);
  assert.equal(report.functionalReport.functions.length, 15);
  ...
  assert.equal(report.functionalReport.functions.some((item) => item.id === "indicator-center" && item.status === "ready"), true);
  ...
  assert.equal(report.checks.some((item) => item.id === "dashboard:production-acceptance-routing" && item.passed), true);
  assert.equal(report.checks.some((item) => item.id === "dashboard:backend-go-live-checklist" && item.passed), true);
  assert.equal(report.checks.some((item) => item.id === "dashboard:indicator-center" && item.passed), true);
  ...
  assert.match(markdown, /P0 接收判定/);
  assert.match(markdown, /统一身份接收判定/);
  assert.match(markdown, /不得解除生产上线阻断/);
  assert.match(markdown, /生产后端上线清单/);
  assert.match(markdown, /Node\.js API 生产部署/);
  assert.match(markdown, /AUDIT_EXPORT_PATH/);
  assert.match(markdown, /指标中心/);
  assert.match(markdown, /公立医院绩效考核/);
  assert.match(markdown, /等级评审证据闭环率/);
  assert.match(markdown, /公立医院改革与高质量发展分类/);
  assert.match(markdown, /智慧医院管理线汇聚入口/);
  assert.match(markdown, /docs\/health-dashboard-indicator-center-report\.md/);
