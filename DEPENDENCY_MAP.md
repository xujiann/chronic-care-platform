...
- Inventory v2 在显式发布图精确锁定 854 处 `innerHTML=` 与 5 处 `insertAdjacentHTML`（合计 859，
  覆盖 41 个资产）；血液主工作台 25 处、急救生命链和医生工作台 controller 各 6 处 `innerHTML` 已迁到 DOM/text 节点，最高仍为 citizen 94、
...
- browser-e2e 本地与 CI 均由标准 `test:e2e` 顺序执行在线根 29、居民 13、PWA 3 项；三阶段各自分配
