...
- governance-api 先校验 custom auth 控制流/负向测试证据，再校验声明级授权矩阵、显式幂等行为证据合同和派生生产 API 目录；依赖方向为 `routeSourceFiles + authentication/idempotency 小型 evidence registry → authorization matrix v3 → production catalog v3`。只有显式标记的 SMS 外部 principal 从幂等合同派生 custom auth，平台 session/RBAC 合同继续复用授权矩阵。T07 financial dispatch/formal grouping 与 T03 highlight signal 依赖保持不变。新增五项依赖方向为 `T02/T06 route session/RBAC + organization/resource scope → src/http/api-command-behavior stable digest/resource lock → lock-in fresh read → owner route business/audit assembly → one writeDatabase → SQLite collection-version CAS`；共享 primitive 不读取数据库、不拥有领域集合、不导入 `server.js`，T02/T06 仍各自拥有业务规则和审计组装。精确重放在重新授权后从内部 receipt 返回，零写；失败无补写。没有增加集合、DDL、outbox、worker、PG adapter、跨域端口或外部依赖；50 条 receipt、单进程资源尾和 SQLite CAS 不等于分布式 exactly-once。`reviewedProofRequired` 为零，证据门禁仍不写数据库、报告或发布制品。
