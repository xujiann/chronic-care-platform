...
专用 runner → 隔离 fixture → 实际 SQLite wrapper/loader + 正式 pg 驱动 → CI PostgreSQL 16 服务。沿用现有 pg 依赖及既有 CI job，在 auth/shadow 验证后串行执行。
...
```text
session/RBAC
  -> emergency-signals route（body 前资源预检）
     -> t06-emergency-signal-write（聚合锁内、receipt 前重验）
        -> rowMatchesOrganizationScope + authOrganizations 父子关系
        -> DomainRepository -> emergencySignals + inbox/outbox + success audit
...
```text
/api/production-go-no-go/center
  -> production-go-no-go-ui.js
     -> 浏览器原生 DOM / textContent / className / dataset
     -> 既有 click 委托与 HealthCityAuth.authFetch
```

本切片未引入第三方依赖、HTML 净化器或第二套 API client；浏览器恶意响应测试继续使用标准 Playwright Chromium 与隔离临时服务。
...
- browser-e2e 本地与 CI 均由标准 `test:e2e` 顺序执行在线根 60、居民 13、PWA 3 项，共 76 项；三阶段各自分配
...
影像 dashboard 当前依赖方向为 `HTTP route → imaging-dashboard-query.v1 → 注入的 dashboard builder / 脱敏端口 → imaging/public-response`。share 写边界为 `imaging-cloud route → imaging-study-share-command.v1 → appendDataAccessLog / randomUUID ports`；QC 写边界为 `imaging-cloud route → imaging-study-quality-control-command.v2 → publishDiagnosticReportToFhir / randomUUID ports`。HTTP 层继续拥有鉴权、查找/404、body、FHIR 错误审计、单次持久化和响应投影；命令层验证回执，适配器将未知外部结果及外部成功后本地失败标为需对账。`publishDiagnosticReportToFhir` 仍由统一组合根注入；QC 仍是请求路径外调后本地写入，未增加 outbox、幂等或反向依赖。

体检 dashboard 当前依赖方向为 `HTTP route → physical-examination-dashboard-query.v1 → 注入的 Overview / readiness 端口`。专项分流写边界为 `blood-innovation HTTP adapter → physical-examination-specialized-intake-action-command.v2 → 注入的既有业务动作 / 访问审计 / 安全审计 / 时钟 / 规范化 / 持久化端口`；适配器继续拥有鉴权、body、读取/定位、进程锁和锁内居民范围，命令把业务变更、版本、显式幂等回执及两类审计交给单次持久化。两个体检端口均不导入根目录体检服务或 `server.js`，但仍接收宽 JSON 快照；进程锁/JSON 存储不提供分布式 exactly-once，混合路由和共享存储耦合尚未消除。
...
## 医保支付产品线依赖方向（2026-08-30）

组织依赖固定为 `T07 独立产品计划/工作树/测试 → T07 Owner 路由与领域端口 → 既有仓储/outbox/金融适配端口 → T00 main/共享门禁/统一部署`。T07 继续通过版本化或显式 Owner 端口访问身份、机构、审计、T08 外部集成等平台能力，不反向依赖 `server.js`，也不因产品线身份复制网关、账本或跨域写路径。共享运行时和统一部署保持不变。
# 2026-09-23 增量

隔离单批次 relay 仅复用现有 Node.js、SQLite 源 loader、正式 PostgreSQL 主存储合同与 checkpoint；无新增依赖或生产环境变量。
