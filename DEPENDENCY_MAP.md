...
- Inventory v2 在显式发布图精确锁定 846 处 `innerHTML=` 与 5 处 `insertAdjacentHTML`（合计 851，
  覆盖 40 个资产）；血液主工作台 25 处、急救生命链和医生工作台 controller 各 6 处、血液上线看板 8 处 `innerHTML` 已迁到 DOM/text 节点，最高仍为 citizen 94、
...
- E2E 依赖方向为 `npm test:e2e → root runner(30) / resident owned runner(13) / PWA runner(3) → 动态回环端口 + 独立临时
...
- browser-e2e 本地与 CI 均由标准 `test:e2e` 顺序执行在线根 30、居民 13、PWA 3 项；三阶段各自分配
...
- governance-api 先校验 custom auth 控制流/负向测试证据，再校验声明级授权矩阵、显式幂等行为证据合同和派生生产 API 目录；依赖方向为 `routeSourceFiles + authentication/idempotency 小型 evidence registry → authorization matrix v3 → production catalog v3`。只有显式标记的 SMS 外部 principal 从幂等合同派生 custom auth，平台 session/RBAC 合同继续复用授权矩阵，避免重复认证声明。幂等合同的 `endpoint` 覆盖可产生 `behavior-verified`，`action-slice` 只登记已验证 action 并保留完整 endpoint 阻断；字面 inventory 只在同一未结束 handler 范围配对 method/path。T07 退款合同复用 `route → refund transaction → versioned persistence command/outbox` 的既有依赖链，未创建第二协议；授权矩阵仍把相邻 refund action 的 `runtime-policy:routeRoles` variant 合并进创建入口，目录因此继续 fail closed。同一注册表的 `reviewedProofRequired` 只保存候选 key、owner、缺失证明类别和现有锚点，并以禁止和合同同 key 并存来防止误晋升；它不复制路由目录，也不是第二份行为真相。证据门禁不写数据库、报告或发布制品。
