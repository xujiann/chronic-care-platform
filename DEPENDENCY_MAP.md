...
- Inventory v2 在显式发布图精确锁定 839 处 `innerHTML=` 与 4 处 `insertAdjacentHTML`（合计 843，
  覆盖 39 个资产）；血液主工作台 25 处、急救生命链和医生工作台 controller 各 6 处、血液上线看板 8 处以及陪诊工作台 7 处 `innerHTML` 加 1 处 `insertAdjacentHTML` 已迁到 DOM/text 节点，最高仍为 citizen 94、
...
- E2E 依赖方向为 `npm test:e2e → root runner(31) / resident owned runner(13) / PWA runner(3) → 动态回环端口 + 独立临时
...
- browser-e2e 本地与 CI 均由标准 `test:e2e` 顺序执行在线根 31、居民 13、PWA 3 项；三阶段各自分配
