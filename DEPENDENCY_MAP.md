...
- governance-api 先校验 custom auth 控制流/负向测试证据，再校验声明级授权矩阵、显式幂等行为证据合同和派生生产 API 目录；依赖方向为 `routeSourceFiles + authentication/idempotency 小型 evidence registry → authorization matrix v3 → production catalog v3`。只有显式标记的 SMS 外部 principal 从幂等合同派生 custom auth，平台 session/RBAC 合同继续复用授权矩阵。T07 financial dispatch 的 reservation/finalize 依赖保持不变；formal grouping create 的新增方向为 `session/RBAC → insurance responsibility → city-level resource scope → body → per-idempotency-key command tail → fresh diseasePayment snapshot → createFormalGroupingJob → queued job + chained security audit → one state write/SQLite collection-version CAS`。精确重放在锁内重读后直接返回，不写状态。没有增加 schema、outbox、worker、PG adapter 或外部依赖；单进程命令尾和 SQLite CAS 不等于分布式 exactly-once。`reviewedProofRequired` 已归零，证据门禁仍不写数据库、报告或发布制品。
