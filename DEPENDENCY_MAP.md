...
AI/CDSS 的依赖为 HTTP/页面 → T01 治理策略或 T06 临床端口 → T00 注入的目录、既有 state 仓储和审计端口。T06 通过 rulePolicy 消费 T01 只读策略，不复制摘要算法；未注入策略时已治理规则不可用。未引入新依赖、模型 provider、worker 或独立数据库。
