...
- Inventory v2 在显式发布图精确锁定 822 处 `innerHTML=` 与 4 处 `insertAdjacentHTML`（合计 826，
  覆盖 33 个资产）；血液主工作台 25 处、急救生命链和医生工作台 controller 各 6 处、血液上线看板 8 处、陪诊工作台 7 处 `innerHTML` 加 1 处 `insertAdjacentHTML`、产品运行驾驶舱、产品区域运行驾驶舱与质量安全工作台各 1 处、区域切换工作台及血液召回面板各 2 处、血液创新指挥中心 10 处 `innerHTML` 已迁到 DOM/text 节点，最高仍为 citizen 94、
...
- E2E 依赖方向为 `npm test:e2e → root runner(38) / resident owned runner(13) / PWA runner(3) → 动态回环端口 + 独立临时
...
- browser-e2e 本地与 CI 均由标准 `test:e2e` 顺序执行在线根 38、居民 13、PWA 3 项；三阶段各自分配
