...
- HTML、静态资源、JSON/API、下载与错误响应由集中端口下发 `nosniff`、frame、referrer、
  ...
  严格目标只为 Report-Only；血液主工作台、急救生命链、医生工作台、血液上线看板、陪诊工作台、产品运行驾驶舱、产品区域运行驾驶舱、质量安全工作台、区域切换工作台与血液召回面板 API 字段已使用 DOM/text 节点，Inventory v2 已把
  836 个 DOM HTML、6 个动态 URL 和 45 个动态样式
...
scope、错误、幂等或审计语义。根 37 项与居民 13 项继续调用现有接口；动态回环端口和临时数据只用于
