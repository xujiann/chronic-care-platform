...
## 真实主存储验证（2026-09-22，实施待验证）

新增测试命令 `npm run postgres:primary-live-contract`，仅显式开关及专用隔离数据库配置可运行；缺配置或有跳过即失败。无新增 HTTP API、worker 接线或生产入口。

本轮基线为 PR #311 合并后的 main `3474f43c`；main CI35572286391 九项成功，Pages35572286399 成功（仅静态演示）。本机数据库不可连接，定向本地检查 62 项通过、23 项 live 用例跳过；新入口真实数据库、独立审查与保护集成证据仍待完成，不晋升生产能力。
