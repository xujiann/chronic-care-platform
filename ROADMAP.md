...
| 4 | 标准 build/lint/typecheck/unit/integration/smoke 入口 | 已合入 `main@fc42833`（PR #130）；TEST-006 再关闭 API test 的 `no-unreachable` 文件级例外、把类型边界由实际 9 个唯一文件扩大到 13 个，并隔离/观测本机约 294–371 秒 API 热点；内部边界覆盖另锁定 identity/audit/object storage/API governance 四组真实基线 / P1 | 保持 test:all 与原 server.js 85/85/55 语义；覆盖阈值只升不降，报告只留临时目录；先补行为保护再消除两个前端文件的 16 个重复键和 3 个显式 skip，依据多次 CI 耗时再决定是否需要新 ADR/拓扑分片 |
...
| 7C | 生产 API 机器目录 | v3 已分离 custom auth、源码观察与幂等行为证据；13 项认证合同保持未分类为 0。首批 existing-proof 形成 6 份行为合同：4 个完整 endpoint、2 个转诊 action-slice；329 个写接口仍缺 endpoint 级证明，330 项复核，593 项全部 NO-GO / P1 | 逐 owner 复核 7 个运行时策略和 329 个缺 endpoint 级行为证明的写接口；通用 action 路由只允许登记直接证明的 slice，不得用源码标记、局部 action 或猜测 owner 替代完整 endpoint 行为测试 |
...
| 9 | 前端可信渲染与 CSP | Accepted ADR；37 个脚本和 21 个静态样式风险已外移；血液主工作台完成可信 DOM/text 试点；29 个原模板 URL occurrence 已由 28 个真实 DOM 绑定迁移和 1 个扫描误报校正闭合。Inventory v2 锁定 871 个 DOM HTML、6 个动态 URL、45 个动态样式风险且禁止增加/替换 / P1 | 真实 OHIF exact-Origin 到位后迁移剩余 2 个导航；逐页治理 HTML 来源/编码/净化和动态 CSSOM，完成全角色恶意输入、真实托管头与独立安全评估后才移除兼容 `unsafe-inline` 并强制严格 CSP |
| 10 | CI/worker/部署依赖治理 | CI 风险域拆分和 Action 完整 SHA 固定已建立；12 个 worker profile 的 v1 脱敏兼容观测、9 个部署入口漂移门禁与部署包闭包已完成；TEST-005 已统一 Chromium 并将 40 项 E2E 分为独立 27 + 13 服务生命周期 / P2 | 不降低 required checks；持续按官方 tag 升级并核验 commit SHA；新增 worker 必须登记并复用 v1；PWA/离线缓存另立专项。真实采集器、告警路由、服务启用和现场验收继续外置 |
