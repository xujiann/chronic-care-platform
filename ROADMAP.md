...
| 7C | 生产 API 机器目录 | v3 当前 609 项与 13 项认证合同。现有 31 份幂等行为合同：29 个完整 endpoint、2 个转诊 action-slice；除首发范围 17 个写入口外，T06 急救信号死信重放 endpoint 已绑定 commission 范围、持久回放历史、冲突与原子审计证据，`reviewedProofRequired` 为零。347 个写接口中 318 个仍缺 endpoint 级证明，通用 action remainder 使总复核为 320，全部 NO-GO / P1 | T06 已关闭相邻急救信号更新的机构/区县运行时资源范围并补直接测试；下一步由 T00 独立登记机器证据后再减少目录复核数。真实 data owner、PG 多实例、长期留存/归档与现场证据继续外置，禁止把进程锁、SQLite CAS 或测试解释为 exactly-once/生产 GO |
...
| 15 | 当前工作流、Markdown 与跟踪 PDF 闭集治理 | GOV-001、DOC-001、REPO-001 仓库内缺口已关闭：开发默认 `origin/main`，固定 tag 仅作证据；273 份 Markdown 唯一分类；3 个 PDF 绑定来源与 digest / P2 | snapshot/superseded 保持只读；新增文档同步清单。两个历史 PDF 与一个现行校验 PDF 均无跟踪生成器，替换前必须先补可复现生成源，不得手工编辑 |
