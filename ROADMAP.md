...
| 7C | 生产 API 机器目录 | v3 保持 593 项与 13 项认证合同。existing-proof 现有 18 份行为合同：16 个完整 endpoint、2 个转诊 action-slice；首发 T02 operations dispatch/reconciliation review 与 T06 quality-safety dispatch/feedback/review 五项已关闭仓库内身份/组织或 assigned-resource scope、exact replay/conflict、expectedVersion、同资源并发、稳定错误与单次原子状态/审计证据缺口，`reviewedProofRequired` 为零。317 个写接口仍缺 endpoint 级证明，总复核 319；首发范围 API review 12，全部 NO-GO / P1 | 继续审计 8 个运行时策略及其余写接口；五项有界 receipt/进程锁/SQLite CAS 及公卫 200 条 ledger 都不是 PG 多实例、长期留存、外部回执或现场证据，禁止解释为 exactly-once/生产 GO |
