...
| 4 | 标准 build/lint/typecheck/unit/integration/smoke 入口 | 已合入 `main@fc42833`（PR #130）；TEST-006 已关闭 API test 的 `no-unreachable` 文件级例外，把类型边界由实际 9 个唯一文件扩大到 13 个，隔离/观测本机约 294–371 秒 API 热点，在 shadow-map/调用行为保护下消除两个前端文件的 16 个重复键和全部 lint 文件例外，完成 3 个 care skip 的 T05 owner/route 重验与恢复执行，并依次完成临时 seed/env/server 与单个 HIS hospital mock 两个共享生命周期夹具切片；TEST-002 将内部边界从 4 组扩展为 10 组，新增 worker、区域共享、转诊、科研导出及两个浏览器安全端口，并提高 API governance 基线 / P1 | 保持 test:all 与原 server.js 85/85/55 语义；API 的 43 个子测试数量/顺序摘要、单进程共享状态、断言和超时继续失败关闭；financial/alert/storage mock 后续仍一次只拆一个生命周期；覆盖阈值只升不降、源码不跨组重复、负向测试必须由所属组直接执行，报告只留临时目录；浏览器端口覆盖不替代 E2E/现场验证，外部 care/HIS 投递与现场验收仍 NO-GO，若改变测试拓扑再另立 ADR |
...
| 7C | 生产 API 机器目录 | v3 保持 593 项与 13 项认证合同。existing-proof 现有 9 份行为合同：7 个完整 endpoint、2 个转诊 action-slice；T01 security-control action 已关闭仓库内 role/step-up、精确 control scope、replay/conflict、同/异载荷并发、SQLite CAS、稳定错误和原子 control+receipt+audit 缺口。326 个写接口仍缺 endpoint 级证明，总复核 328，全部 NO-GO / P1 | 继续审计 financial dispatch、formal grouping、7 个运行时策略及其余写接口；security control 的 data owner、真实多实例/PG 与现场证据继续外置，禁止把进程锁、SQLite CAS 或测试解释为 exactly-once/生产 GO |
...
| 9 | 前端可信渲染与 CSP | Accepted ADR；37 个脚本和 21 个静态样式风险已外移；血液主工作台、急救生命链、医生工作台、血液上线看板、陪诊工作台与产品运行驾驶舱 controller 完成可信 DOM/text 切片；29 个原模板 URL occurrence 已闭合。Inventory v2 锁定 842 个 DOM HTML、6 个动态 URL、45 个动态样式风险且禁止增加/替换 / P1 | 真实 OHIF exact-Origin 到位后迁移剩余 2 个导航；逐页治理 HTML 来源/编码/净化和动态 CSSOM，完成全角色恶意输入、真实托管头与独立安全评估后才移除兼容 `unsafe-inline` 并强制严格 CSP |
| 10 | CI/worker/部署依赖治理 | CI 风险域拆分和 Action 完整 SHA 固定已建立；12 个 worker profile 的 v1 脱敏兼容观测、9 个部署入口漂移门禁与部署包闭包已完成；在线根 32 + 居民 13 项继续隔离 Worker，PWA 3 项已验证 v61 安装/更新/离线/缓存边界/清理，标准 E2E 共 48 项 / P2 | 不降低 required checks；持续按官方 tag 升级并核验 commit SHA；新增 worker 必须登记并复用 v1；真实 HTTPS、设备/浏览器策略、外部 Origin、现场缓存升级、采集器/告警和上线验收继续外置 |
