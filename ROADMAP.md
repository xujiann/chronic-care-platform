...
| 7C | 生产 API 机器目录 | v3 保持 593 项与 13 项认证合同。existing-proof 现有 8 份行为合同：6 个完整 endpoint、2 个转诊 action-slice；T07 financial reconciliation 已关闭仓库内身份/范围、digest replay/conflict、并发串行、SQLite CAS、稳定错误和原子 run+audit 缺口。327 个写接口仍缺 endpoint 级证明，总复核 329，全部 NO-GO / P1 | 继续审计 financial dispatch、formal grouping、7 个运行时策略及其余写接口；对账的 data owner、真实多实例/PG 与现场证据继续外置，禁止把进程锁、SQLite CAS 或测试解释为 exactly-once/生产 GO |
