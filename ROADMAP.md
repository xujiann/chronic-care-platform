...
| 9 | 前端可信渲染与 CSP | Accepted ADR；37 个脚本和 21 个静态样式风险已外移；血液主工作台与急救生命链 controller 完成可信 DOM/text 切片；29 个原模板 URL occurrence 已闭合。Inventory v2 锁定 865 个 DOM HTML、6 个动态 URL、45 个动态样式风险且禁止增加/替换 / P1 | 真实 OHIF exact-Origin 到位后迁移剩余 2 个导航；逐页治理 HTML 来源/编码/净化和动态 CSSOM，完成全角色恶意输入、真实托管头与独立安全评估后才移除兼容 `unsafe-inline` 并强制严格 CSP |
| 10 | CI/worker/部署依赖治理 | CI 风险域拆分和 Action 完整 SHA 固定已建立；12 个 worker profile 的 v1 脱敏兼容观测、9 个部署入口漂移门禁与部署包闭包已完成；在线根 28 + 居民 13 项继续隔离 Worker，PWA 3 项已验证 v61 安装/更新/离线/缓存边界/清理，标准 E2E 共 44 项 / P2 | 不降低 required checks；持续按官方 tag 升级并核验 commit SHA；新增 worker 必须登记并复用 v1；真实 HTTPS、设备/浏览器策略、外部 Origin、现场缓存升级、采集器/告警和上线验收继续外置 |
