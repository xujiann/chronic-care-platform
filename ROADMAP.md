...
| 7C | 生产 API 机器目录 | v3 已分离 custom auth、源码观察与幂等行为证据；13 项认证合同保持未分类为 0。existing-proof 形成 7 份行为合同：5 个完整 endpoint、2 个转诊 action-slice；T07 退款申请的真实 HTTP、机构范围、事务/CAS/outbox 已闭合。T07 第二批对 financial dispatch、reconciliation 和 formal grouping job 做了零晋升深审，以 3 条 `reviewedProofRequired` 锁定现存证明缺口。328 个写接口仍缺 endpoint 级证明；退款静态 runtime-role variant 使总复核仍为 330，593 项全部 NO-GO / P1 | 逐 owner 复核 7 个运行时策略和 328 个缺 endpoint 级行为证明的写接口；先解决真实同键异载荷冲突与 runtime role 解析边界，再登记 T08 event/dispatch 或 T07 refund action。通用 action 路由只允许登记直接证明的 slice，不得用源码标记、局部 action 或猜测 owner 替代完整 endpoint 行为测试 |
