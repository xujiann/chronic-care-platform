...
| 运营派单与结算 | 医院端、个人端 | `internet-nursing.html` | `dispatchRecommendations`, `settlement`, `satisfaction`, `qualityInspection` | 按护士资质、服务项目、区域、日容量和风险等级推荐派单，展示医保/自费预估、支付状态、满意度、投诉和质控抽查 | `nursing:phaseTwoOperations` |
| 监管扩展 | 卫健管理端、平台运维 | `internet-nursing.html` | `regulatoryMonthlyReport`, `regulatoryAlerts`, `regulatoryContract` | 形成服务量、风险处置、回访闭环、投诉率、轨迹完整率、机构质量评分、准入审核、目录审批和护士资质提醒 | `nursing:phaseThreeRegulation` |
...
- 手机预览：`mobile-preview.html?client=app&page=nursing` 在居民端手机服务页内切换到“护理”，用于验收居民手机预约和护士手机接单响应。
...
- `internetNursingOrders`：居民预约、首诊评估、知情同意、护士派单、位置轨迹、护理记录、质量回访、费用结算、满意度、投诉、质控抽查、不良事件和审计轨迹。
- `taskMessages`：新预约、派单、服务进展和质控待办的跨角色消息通道。
- `dispatchRecommendations`：按护士资质、专长、机构、服务项目、容量和风险等级生成的派单候选结果。
- `regulatoryMonthlyReport`：监管月报，汇总服务量、完成量、风险处置、投诉率、回访率、轨迹完整率、不良事件和机构质量评分。
- `regulatoryContract`：监管接口契约 `internet-nursing-regulatory-contract-v1`，定义对接院内护理管理系统、电子病历、医保结算和监管平台的数据对象与接口。
...
- 医院派单优先使用 `dispatchRecommendations`，候选护士必须满足资质、服务项目、机构和容量约束。
- 质控回访必须形成满意度、投诉、质控抽查和不良事件状态，纳入机构质量评分。
- 卫健监管端必须展示准入审核、目录变更审批、护士资质到期提醒和月报指标。
...
- 闭环摘要门禁：`nursing:closedLoopSummary`
- SLA 风险提示：`nursing-loop-risk-strip`, `slaOverdue`, `highRiskOpen`
- 阶段二运营门禁：`nursing:phaseTwoOperations`
- 阶段三监管门禁：`nursing:phaseThreeRegulation`
...
| 护士/护理服务人员 | 护士端手机工作站锁定本人身份；按接单、开始服务、完成护理记录顺序推进；写入 `locationTracePoints`；完成服务时写入结构化护理记录、耗材使用、照片/签名附件、异常事件状态和消息回执；移动卡片展示服务记录、附件数、回执状态、风险等级和轨迹点摘要。 | 增加排班日历、路线提醒；强化手机定位或定位设备核验。 |
...
上述开发计划的系统侧能力已完成，并纳入 `internet-nursing:readiness` 发布门禁：

- `nursing:serviceRecordClosure`：结构化护理记录、耗材、照片/签名附件、异常上报和消息回执。
- `nursing:closedLoopSummary`：页面汇总闭环完成率、待派单、待评估签署、待接单、高风险未闭环和超 24 小时 SLA 风险，并纳入发布门禁。
- `nursing:productionIntegration`：生产集成契约 `internet-nursing-production-integration-v1`，覆盖短信、院内消息、站内消息、电子签名附件存储、院内护理管理系统、电子病历和卫健监管平台连接器。
- `nursing:paymentIntegration`：支付对账契约 `internet-nursing-payment-v1`，覆盖医保电子凭证预核、移动自费支付、退费、电子发票和 T+1 日终对账。
- `nursing:deviceVerification`：设备核验契约 `internet-nursing-device-verification-v1`，覆盖手机 GPS、护士定位设备、服务记录仪、一键报警、照片附件和异常升级。
- `nursing:regulatorySubmission`：监管报送契约 `internet-nursing-regulatory-submission-v1`，覆盖机构、护士、订单、风险、轨迹、结算、质控、不良事件字段映射、月报、高风险实时、现场签字和压测证据。
- `nursing:siteCutoverPack`：现场切换签字包 `nursing-cutover`，把消息网关、电子签名存储、医院连接器、医保支付对账、设备实测、监管报送压测、审计保全和生产切换清单合并为上线签字证据。

剩余内容属于现场上线条件：替换生产短信/院内消息/电子签名/医保/监管平台地址，托管生产密钥，完成试点医院、卫健监管、医保经办和平台运维的现场签字、压测报告和灾备演练归档。
