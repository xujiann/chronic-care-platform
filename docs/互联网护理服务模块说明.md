...
| --- | --- | --- | --- | --- | --- |
| 居民预约 | 个人端 | `internet-nursing.html` | `internetNursingOrders`, `residents` | 提交服务对象、服务项目、期望时间、地址和风险等级 | `nursing-appointment-form`, `POST /api/internet-nursing/orders` |
...
| 护士接单服务 | 护士端 | `internet-nursing.html` | `internetNursingNurses`, `internetNursingOrders` | 锁定本人护士身份，接单、开始服务、完成护理记录并保留位置轨迹 | `nursing-nurse-queue`, `nurse / 123456` |
...
- 浏览器验收：使用 `nurse / 123456` 登录后，护士选择框应锁定 `inn-001`，护士队列应显示 `Accept`、`Start`、`Complete`，医院管理区应保持只读。
