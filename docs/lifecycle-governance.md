...
- 来源：用户于 2026-09-09 再次要求继续开发。T00 批准本中风险登记及下列两项精确 PLAN，基线 main `8e400c4c`。GOV-007 已由 #286 / head `a409bfb4` / CI 34339809407 九项成功及 14 批 legacy 退出 0、独立复审完成仓库集成，本次正常登记据此关闭，不创建递归自证任务。
- 中央只修改总账、本规范和 ROADMAP。已登记 GOV-008 与 OPS-029～OPS-032 四领域任务，WIP 5/5；同一核心范围只有一个 writer，其他候选排队，不再增加实施任务。
...
- 验收：两线先 RED 后 GREEN，以真实完整 controller/renderer 测试失败、迟到响应、正常与恶意输入；相关专项、页面、安全、process、diff、legacy test:all、独立 review 和最终 head CI 均满足后交 T00 串行集成。新测试提交前只引用已有可执行回归基线，不冒充已存在的新行为证据。
