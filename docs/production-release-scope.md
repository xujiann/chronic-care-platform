...
Repository verification currently identifies 12 scoped APIs needing behavior review. The 19 first-release legacy collections have completed owner/governance review, so `collectionReviewRequired=0`; those collections plus the `referrals` and `operationsReadiness` source bindings still make 21 scoped references without a production-write contract. Five first-release writes now have repository behavior contracts: operations dispatch/reconciliation review and quality-safety dispatch/feedback/review. Their in-process resource locks, record versions, hashed receipts and SQLite collection CAS are repository evidence only; they do not prove cross-instance exactly-once or production durability. `referrals` is validated as an exact `domain-data-ownership` owned contract for T05, while `operationsReadiness` is validated as a T00-owned exported derived read model; both remain fingerprinted data references but are not unowned top-level collections. All scoped APIs remain catalogued as production NO-GO. Worker activation, external joint-test results, signed cutover evidence, PostgreSQL primary selection and site acceptance remain external or release-specific work.
