# 路由模块化与并行开发边界（2026-08-03，2026-08-18 主线校准）
...
- 当前进程基线：`baseline/governance-20260817-enhancement-v1`
- 路由顺序清单：`src/http/routes/index.js`
- 服务依赖装配：`server.js`

业务进程从当前进程基线或明确指定的最新 `main` 创建短生命周期功能分支和独立 worktree。功能分支不是第二条集成线；完成专项验证后，通过 PR 进入 `main`。
...
```text
从当前进程基线或最新 main 创建 process/tNN-<topic>-YYYYMMDD
  -> 在独立 worktree 完成领域实现
  -> npm run routes:check
  -> npm run routes:test
  -> 运行所属领域专项测试
  -> 提交单一职责变更
  -> T00 审查跨域/保护路径，PR 合入 main
  -> npm run check
  -> npm test
  -> npm run test:all
...
所有权定义位于 `config/process-workstreams.json`，受保护文件由
`scripts/process-worktree.js` 校验。

```powershell
# 查看进程、领域路由与当前基线
npm.cmd run process:list

# 预览分支名和工作树路径，不修改 Git
npm.cmd run process:plan -- --process=T04 --topic=citizen-chronic-pilot --date=20260803

# 从当前进程基线创建分支和独立工作树；需最新 main 时显式传 --base=origin/main
npm.cmd run process:create -- --process=T04 --topic=citizen-chronic-pilot --date=20260803

# 在当前进程基线分支交付前检查是否越过其他进程的受保护文件
npm.cmd run process:verify

# 从最新 main 创建的分支必须显式使用同一比较基线
npm.cmd run process:verify -- --base=origin/main
```

分支固定使用 `process/tNN-<topic>-YYYYMMDD`。GitHub PR 模板要求填写进程、
基线、领域路由、运行时依赖及验证结果；CI 对 `process/**` PR 使用目标集成分支
（当前为 `main`）作为比较基线执行所有权检查。固定 `baselineTag` 继续用于可复现
工作树和发布证据，不得替代 PR 实际目标分支。
...
- 当前主线 76 个路由段标识唯一，13 个路由域完整登记；数字变化必须由路由检查验证。
- 顺序调度与响应短路契约测试通过。
