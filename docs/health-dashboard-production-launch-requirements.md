...
| 门禁 | 接收岗位 | 通过条件 | 未通过处理 |
|---|---|---|---|
| 统一身份 | 规划信息处、统一认证管理员 | 真实账号完成登录、角色映射和区县科室权限验证 | 退回身份源配置并保留审计记录 |
| 审计留存 | 安全管理岗、项目办 | 审计导出目录、哈希链校验和查询权限全部可复核 | 不得解除生产上线阻断 |
| 生产数据库 | 数据平台、运维负责人 | 迁移、备份、恢复、脱敏和回滚窗口均有演练记录 | 继续限定为演示或联调环境 |
| 监控灾备 | 运维、安全管理岗 | 健康检查、指标采集、告警路由、值班升级和 RTO/RPO 演练可追溯 | 不得转连续运行 |
| 接口联调 | 医疗机构接口负责人、项目办 | 真实报文、截图、问题清单、整改复测和联合签字单一致归档 | 保留接口试运行状态 |
