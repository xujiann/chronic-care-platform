...
- 缓存：首切片将 Service Worker 提升到 v60；PWA 专项于 2026-08-23 再提升到 v61，激活时删除 v60
  及其他旧缓存，并只把同源成功响应写入 Cache Storage。`data/public-demo.json` 可缓存，源快照 404、
  `/api/*` 与跨 Origin 请求不缓存。
...
专用 Playwright context 使用真实 `service-worker.js` 验证居民登录后的注册/激活、v60→v61 旧缓存清理、
受控 update、离线 mobile/citizen 导航回退、API/源快照拒绝缓存及逐项注销/清缓存。该自动化只运行于
动态 HTTP 回环地址；它不证明真实 HTTPS 终止、OS 安装提示、现场浏览器策略、外部 Origin 或生产验收。
