...
- 第七切片：`POST /api/imaging-cloud/studies/:id/qc` 已从 `clinical-blood` 迁入既有 `imaging-cloud` 路由，并通过 `imaging-study-quality-control-command.v2` 进入影像目标源码根；在既有协议上增加评分校验和 FHIR DiagnosticReport 资源关联/质控结论确认，区分明确拒绝、未知结果及外部成功后本地失败，后二者失败关闭为需对账，浏览器锁定进行中或需对账动作。未新增幂等、CAS、机构范围、schema、outbox 或生产授权，外调先于本地提交的风险继续 `NO-GO`。
- 第八切片：`POST /api/physical-exams/specialized-intakes/:id/actions` 已通过 `physical-examination-specialized-intake-action-command.v2` 接入体检目标源码根；显式命令键绑定 actor/resource/payload 并要求 `expectedVersion`，在进程锁内复核居民范围，将业务变更、版本、最多 50 条非淘汰回执、访问审计和安全审计纳入单次数据库写入，失败提交不污染调用方快照。旧客户端仍非幂等，进程锁/JSON 存储不构成分布式 exactly-once；三类动作尚无完整 HTTP 行为证据，故不进入中央 API 幂等证据白名单，生产仍 `NO-GO`。
...
- TEST-009：五个子域现各有由 `config/clinical-subdomains.json#subdomains[].testGate` 声明的确定性本地测试入口，统一由 `scripts/clinical-subdomain-test-gate.js` 失败关闭校验并执行；13 个子域测试文件与 1 个共享边界测试覆盖已登记用例、HTTP facade、鉴权拒绝和源码边界。质量安全的既有 `quality-safety-dashboard-query.v1` 同步登记为已实现用例。`independent-domain-tests` 因五门可独立执行提升为 `met`；T00 已把统一 runner 接入 package 与治理 CI，机器校验同时确认两处接线，因此 `independent-ci-gates` 提升为 `met`。该切片不改变共享 Node 运行时、统一部署、数据 Owner、`productionReady=false` 或独立部署未授权。
