...
- 状态：Accepted
- 接受范围：仅 Recommendation 第 1 切片；其余为后续建议。
- 日期：2026-09-21
- Owner：T00 存储组合与迁移；T02 数据质量问题集合；T09 兼容路由
- 依据：用户于 2026-09-21 对上线前建议计划明确回复“按照建议执行”。本次接受第 1 切片的版本化 receipt schema、受控事务包装器、只读 loader 及必要兼容测试；正式编码前仍须登记独立任务和精确单写范围。不授权服务端接线、relay、checkpoint 改造、真实数据迁移或 worker/生产激活。
- 实施基线：`70521f29` 已通过 PR #308 集成只读样本 `17df11d7`，PR/main CI 均成功。2026-09-21 再次核验主线未漂移，原 head=17、无 v18 预留；本切片追加 v18，不改 v1–v17。

## 接受范围与实施前置

用户批准按顺序先完成只读候选集成，再实施提交凭证第一切片；不是对后续所有迁移、目标主库或上线的概括授权。第 2、3 切片仍须独立 PLAN、审查和准入，第 4 项只是每个已准入切片的验证流程。

2026-09-21 只读复核显示 schema head 为 17，未发现 v18 预留；实际追加前仍须重新核对最新主线。已有对象存储 worker 和架构治理把当前 head 与历史 v17 等同，追加迁移时允许最小兼容修复及对应负测：判断已应用的原 v17，而非要求当前 head 永远为 17。`reservedSqliteMigrationVersion=17`、原指纹、既有 worker 权限与生产禁止状态保持不变。迁移测试的未来非法版本探针改用当前 head + 1，不改历史 ledger/checksum。

所有新增能力只在合成隔离测试库调用。当前请求路径仍使用旧 outbox 写入，不会自动生成 receipt；不得将新模块存在表述为真实提交链已接通。回滚保留 receipt/outbox 及业务事实，禁止自动 DROP；已升级库不能直接交给旧 schema runtime，恢复须采用经验证的前滚修复或受控备份恢复。
