...
Launch command brief delivery receipts are now recorded on the same `publicHealthLaunchCommandBriefs` rows. After a brief is published, a commission user can call `POST /api/public-health/launch-command-briefs/:id/actions` with `acknowledge-launch-command-brief` or `escalate-launch-command-brief-receipt`, a configured `acknowledgementTarget`, and a `note`. The API rejects receipts before publication, targets outside the configured audience, and blank notes; every receipt or escalation is retained in the brief history and the `public-health-launch-command-brief-action` audit trail. The workbench shows receipt totals and pending recipients so a broadcast can be followed through to delivery receipt without treating a delivery receipt as site evidence, a P0/P1 blocker closure, or a production approval.
...
## Public health five-suite highlight center

The public health workbench now exposes a five-suite operating center covering multi-source trigger rules, GIS-oriented signal review, AI-assisted investigation, command dispatch, and acceptance evidence. `GET /api/public-health/highlights` returns the consolidated board; signal intake and alert, command task, AI review, and evidence action APIs retain the operator, role, timestamp, note, and evidence references in the existing audit trail.

The feature is functionally runnable and is checked by `publicHealth:highlightCapabilities`, `api:highlight-actions`, `frontend:highlight-center`, `deploy:check`, and the dedicated model tests. AI output always requires human review. The formal state remains `blocked-until-site-evidence-signed`: demo signals, rules, resources, and evidence summaries do not represent a completed CDC, HIS, EMR, LIS, immunization, supervision, security, or production-site acceptance.
