...
The workbench does not replace the change advisory board, hospital-side authorization, security assessment, disaster-recovery certification or government approval. It does not set environment signoffs, upload signed files, run migrations, switch traffic or execute rollback.

## Verification

```powershell
node --test test/production-go-no-go.test.js
