...
| --- | --- | --- | --- | --- | --- |
| 出生人口统计与监管 | 卫健管理端 | `index.html` | `birthStatistics`, `birthCertificates` | 展示出生证明、签发/上报、电子证照、公安共享、妇幼入册、公安待共享、妇幼待入册、低体重儿、质控补正和风险清单 | `renderBirthStatistics`, `renderMaternalChildCare`, `mch-risk-list`, `role:commission-risk-metrics` |
| 出生医学证明办理 | 医疗机构端 | `institution.html` | `birthCertificates`, `birthCertificateForms`, `birthCertificateDocuments` | 登记新生儿和父母信息，办理首次签发、换发、补发、签发、上报入册、材料核验，并展示公安待共享、妇幼待入册、空白证件、废证销毁和质控补正风险卡片 | `birth-certificate-form`, `/api/birth-certificates`, `submitBirthCertificate`, `actionButton`, `role:institution-risk-metrics`, `data:certificate-document-controls` |
...
- `birthCertificateDocuments`：空白证件年度申领、季度配发、废证销毁和旧版清理台账，作为 `data:certificate-document-controls` 上线门禁证据。
- `birthStatistics`：出生人口统计指标，支撑卫健管理端统计看板、服务清单和风险监测；`metrics.pendingPublicSecuritySync`、`metrics.pendingMaternalChildSync` 和 `metrics.qualityPending` 分别沉淀公安共享待办、妇幼入册待办和质控待办数量，作为 `data:risk-metrics` 发布门禁。
...
- 风险指标摘要：`maternal-child:readiness` 在 `summary.riskMetrics` 中输出公安待共享、妇幼待入册和质控补正数量，供发布审阅和卫健管理端验收复核。
- Release 门禁：`release:report` 增加 `maternalChild:riskMetrics`，要求三项风险指标和 `data:risk-metrics` 检查同时通过后才允许上线报告为绿。

## 上线运行保障

- 出生医学证明通过 `POST /api/birth-certificates` 新增后，服务端立即调用 `refreshBirthStatistics`，保证卫健管理端 `birthStatistics.metrics.total` 与证照个案数量一致。
- 出生医学证明通过 `/api/workflow-actions` 签发、上报、公安共享或妇幼入册后，服务端同步刷新 `pendingPublicSecuritySync`、`pendingMaternalChildSync`、`qualityPending` 等风险指标，避免医疗机构端办理成功但卫健统计仍停留旧值。
- API 回归用例覆盖“机构端新增出生证 -> 工作流上报入册 -> 管理端重新读取统计”的闭环，作为上线运行门槛的一部分。
