...
急救、血液与影像首个切片已完成：急救组合血液协调投影；血液统一角色范围并保留内存规范化；影像统一构建、脱敏、公开响应净化并保留居民访问审计。下一切片进入体检子域，先锁定 `GET /api/physical-exams` 的居民范围、角色差异、审计和响应脱敏，再从 `blood-innovation` 混合路由迁出一个体检查询用例。
