...
2026-08-26 的当前源码产生 601 条授权声明和 371 个字面条件路由；两者并集形成 593 个唯一目录条目：585 个字面路由、8 个运行时策略路由；585 个 required、7 个 no-auth public、1 个 optional-session，custom auth 未分类为 0。13 项认证证据包括既有 SMS callback，以及原 13 个未分类 key 中由控制流与可执行负向测试共同证明的 12 个真实入口。认证合同只记录 credential source 的类别名称，不记录真实 credential。
...
`production-api-catalog-v3` 另行统计幂等行为证据：333 个写接口中，107 个观察到幂等相关标记、226 个未观察到。`api-idempotency-evidence-v1` 现登记 18 份直接行为合同，其中 16 个完整 endpoint 为 `behavior-verified`，另有 `workflow-actions[collection=referrals]` 与 `tasks[referrals:*]` 2 个 action-slice；317 个写接口仍为 `behavior-proof-required`，总计 319 项 `review-required`。新增首发 T02 operations dispatch/reconciliation review 与 T06 quality-safety dispatch/feedback/review 五项合同证明身份/组织或 assigned-resource scope、actor scope key hash、同资源串行和锁内重读、显式 expectedVersion、精确重放/冲突、业务+既有审计单次提交和稳定失败；内部 receipt 每资源 50 条且不出公开响应。首发范围 API review 由 17 降为 12；19 个 legacy 集合的 owner review 已闭合，`collectionReviewRequired=0`，但 21 个范围引用仍无生产写合同。两个 action-slice 仍只进入 `verifiedActionContracts`，通用 endpoint 继续阻断。所有合同都明确不证明多实例 exactly-once，593 项仍全部 `NO-GO`。
...
3. 由 T01–T09 owner 分批为 317 个未验证写接口提供 endpoint 行为合同与负向测试；226 个“未观察到标记”只是优先排查子集，只读接口不要求幂等键。
