...
`production-api-catalog-v3` 另行统计幂等行为证据：333 个写接口中，111 个观察到幂等相关标记、222 个未观察到。`api-idempotency-evidence-v1` 现登记 17 份直接行为合同，其中 15 个完整 endpoint 为 `behavior-verified`，另有 `workflow-actions[collection=referrals]` 与 `tasks[referrals:*]` 2 个 action-slice；318 个写接口仍为 `behavior-proof-required`，总计 320 项 `review-required`。最新 T03 highlight signal 合同直接证明身份/RBAC 先于 body/state、city/health-admin 旧 payload 兼容、district canonical 来源机构 allowlist、actor 组织命名空间 key hash、同 key 命令串行和锁内重读、精确重放/冲突、bounded signal ledger、signal+audit 单次提交、稳定错误与真实 SQLite API 持久化；独立 highlight GET 和 `GET /api/public-health/system` 的嵌入式 highlights 都是 safe method，不新增幂等合同，直接路由与真实 SQLite API 测试已保护内部摘要脱敏、district allowlist、混合 alert 失败关闭和外层 highlights 摘要重算。T00 高风险授权注册表现在以 `city and health-admin platform; district own organization or explicit public-health hospital allowlist` 描述独立 highlight GET 范围，授权矩阵与派生生产目录测试同时锁定该值；system 其余字段仍保留遗留响应。`GET /api/state` 已先由 T02 投影移除 `authUsers.password/passwordHash`，但 commission 其余全状态读取范围仍未最小化，不能据此关闭该接口的 `NO-GO` 债务。两个 action-slice 仍只进入 `verifiedActionContracts`，通用 endpoint 继续阻断。所有合同都明确不证明多实例 exactly-once。 新增四份合同对应 T04 慢病计划 PATCH/随访反馈和 T05 远程会诊创建/动作，均保持旧成功响应、范围重验、精确 replay、冲突/CAS、一次状态审计提交及存储失败无补写。
...
3. 由 T01–T09 owner 分批为 318 个未验证写接口提供 endpoint 行为合同与负向测试；222 个“未观察到标记”只是优先排查子集，只读接口不要求幂等键。
