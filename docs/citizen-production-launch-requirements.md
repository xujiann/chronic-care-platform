...
- 配置 `NODE_ENV=production`、正式 `SESSION_SECRETS`、`INTEGRATION_GATEWAY_SECRET`、`SMS_GATEWAY_URL`、`SMS_TEMPLATE_ID`、`SMS_DELIVERY_CALLBACK_SECRET`、`OIDC_*` 和审计保全目标。
...
| 依赖项 | 生产状态 | 责任角色 | 上线阻断口径 | 上线前必须归档的证据 | 现场验收动作 |
| --- | --- | --- | --- | --- | --- |
| 短信网关 `SMS_GATEWAY_URL` | `required-before-production` | `platform-ops` | 未取得发送回执和限流策略前，手机号验证码登录不得向真实居民开放 | 已签署短信网关合同、发送回调地址、限流策略和上线测试回执 | 试发验证码、核验回调、归档脱敏居民回执 |
| 实名身份核验 `OIDC_*` | `required-before-production` | `identity-integration` | 未完成 OIDC 签发方、凭据和 claim 映射前，居民账号绑定仅限试点演示 | OIDC 签发方、客户端密钥、声明映射和实名核验验收记录 | 完成实名登录样例并核对 `residentId/personIndex` 审计日志 |
| 监护/家庭关系核验 | `required-before-production` | `resident-master-index` | 未取得关系来源和人工复核队列前，家庭成员代办入口保持关闭 | 关系来源、人工复核队列和家庭绑定审计样本 | 完成一条通过和一条驳回样例，保留复核人和复核时间 |
| HTTPS、备案和隐私协议 | `required-before-production` | `security-compliance` | 未完成备案、TLS、隐私协议和渗透测试前，不发布移动端生产入口 | 域名备案、TLS 证书、隐私协议地址和渗透测试验收 | 现场打开 HTTPS 居民入口、隐私协议 URL 和渗透测试验收单 |
...
平台已实现 `POST /api/auth/oidc/exchange`、`GET /api/auth/adapters`、生产短信验证码运行时适配、`POST /api/auth/sms-delivery-callback` 签名送达回调和 `GET /api/auth/sms-deliveries` 脱敏运维台账。生产验证码使用随机六位码并仅保存 HMAC 摘要；回调校验时间窗、nonce 重放、事件幂等、乱序和终态冲突；OIDC 仅允许已绑定且启用的本地账号签发会话。该进展属于“身份与短信回调代码就绪”；供应商专有字段与签名差异、真实密钥托管、网络白名单、机构目录同步、真实凭据联调、移动发布和现场签字仍是上线阻断项，详见 `docs/production-identity-message-adapters.md`。
